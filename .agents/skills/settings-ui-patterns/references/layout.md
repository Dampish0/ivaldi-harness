...
- **Ivaldi** (`general`): General, Appearance, Chat, Sessions, Voice, Notifications, Shortcuts.
- **Workspace** (`projects`): Projects, Git, Integrations, Remote Instances, External Tunnel.
- **AI & Agents** (`opencode`): Models & Providers, Agents, Agent Behavior, Commands, Lifecycle Hooks.
- **Tools** (`tools`): MCP, Plugins, Skills. Skills Catalog is reached from Skills rather than duplicated in the top-level nav.
- **Prompts** (`content`): Magic Prompts, Snippets.
- **System** (`system`): Usage and About. Developer-only low-level runtime settings belong here when they have a dedicated page.

Placement rules:

- Work mode orders its groups as Ivaldi, Workspace, Tools, Advanced. Advanced stays last so normal tool configuration remains easier to reach than low-level options.
