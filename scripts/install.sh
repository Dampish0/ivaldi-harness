#!/usr/bin/env bash
# Ivaldi Install Script
# Usage: curl -fsSL https://raw.githubusercontent.com/Dampish0/ivaldi-harness/main/scripts/install.sh | bash

set -euo pipefail

PACKAGE_NAME="@ivaldi/web"
BIN_NAME="ivaldi"
MIN_NODE_VERSION=22
...
main() {
  ...
  # If Ivaldi is already installed, hand off to its own updater instead
  # of guessing a package manager. `ivaldi update` detects which manager
  ...
  if command_exists "$BIN_NAME"; then
    info "Ivaldi is already installed — updating via 'ivaldi update'..."
    echo ""
    if ivaldi update; then
      echo ""
      success "Ivaldi is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "Ivaldi installed successfully!"
    echo ""
    ...
    if command_exists "$BIN_NAME"; then
      echo "  Get started:"
      echo "    ivaldi              # Start server on port 3000"
      echo "    ivaldi --help       # Show all options"
    else
      ...
      if [ -n "$bin_dir" ]; then
        ...
        echo "  then restart your terminal and run: ivaldi"
      fi
    fi
    echo ""
    echo "  Prerequisites:"
