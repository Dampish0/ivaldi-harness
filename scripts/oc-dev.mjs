#!/usr/bin/env node
import { resolveIvaldiDataDirectory } from '../packages/web/server/lib/ivaldi-data-dir.js';
/**
...
const configPath = path.join(resolveIvaldiDataDirectory(), 'oc-dev.json');
...
function printReleaseNextSteps(version) {
  ...
  console.log('');
  console.log('Build, validate, and publish release artifacts manually. See docs/RELEASE_READINESS.md.');
...
function installedWebCli(directory) {
  const cliPath = path.join(directory, 'node_modules', '@ivaldi', 'web', 'bin', 'cli.js');
...
async function deployWeb(options, config) {
  ...
  step('Removing old global package', () => {
    run('bun', ['remove', '-g', '@ivaldi/web'], { allowFail: true, label: 'remove @ivaldi/web' });
...
async function deployRemoteWeb(options, config) {
  ...
  step(`Stopping remote instance on ${host}:${port}`, () => run('ssh', [host, `set -e; ${REMOTE_RUNTIME_ENV}; cd ~/${dir} 2>/dev/null || exit 0; PORT=${quote(port)}; TMPDIR=$(node -p "require('os').tmpdir()" 2>/dev/null || echo /tmp); PIDFILE="$TMPDIR/openchamber-${port}.pid"; INSTANCEFILE="$TMPDIR/openchamber-${port}.json"; if [ -f ./node_modules/@ivaldi/web/bin/cli.js ]; then bun ./node_modules/@ivaldi/web/bin/cli.js stop --port "$PORT" >/dev/null 2>&1 || node ./node_modules/@ivaldi/web/bin/cli.js stop --port "$PORT" >/dev/null 2>&1 || true; fi; if command -v lsof >/dev/null 2>&1; then lsof -ti :"$PORT" | xargs -r kill >/dev/null 2>&1 || true; sleep 0.5; lsof -ti :"$PORT" | xargs -r kill -9 >/dev/null 2>&1 || true; fi; rm -f "$PIDFILE" "$INSTANCEFILE"`], { label: 'stop remote instance' }));
...
function prepareOpenCodeCli() {
  step('Preparing bundled OpenCode CLI', () => run('bun', ['--filter', '@ivaldi/electron', 'prepare:opencode-cli']));
}
...
function startVsCodeExtension() {
  ...
  removeFilesByPrefixSuffix(vscodeDir, 'ivaldi-', '.vsix');
...
async function installVsCodeExtensionLocal(options) {
  ...
  step('Removing found VSIX package(s) before install flow', () => removeFilesByPrefixSuffix(vscodeDir, 'ivaldi-', '.vsix'));
  step('Packaging VSIX', () => run('bunx', ['vsce', 'package', '--no-dependencies'], { cwd: vscodeDir }));
  step('Installing VSIX locally', () => {
    run('code', ['--uninstall-extension', 'dampish0.ivaldi'], { label: 'uninstall old extension', allowFail: true });
    run('code --install-extension packages/vscode/ivaldi-*.vsix', [], { shell: true, label: 'install VSIX' });
  });
  if (cleanup === 'delete') {
    step('Removing local VSIX package(s) after install', () => removeFilesByPrefixSuffix(vscodeDir, 'ivaldi-', '.vsix'));
  }
}
