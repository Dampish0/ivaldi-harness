...
Electron desktop runtime for Ivaldi on macOS, Windows, and Linux.
...
`main.mjs` imports `@ivaldi/web/server/index.js` and calls `startWebUiServer()`. The Electron window then loads the UI from the local server in development, or from packaged `resources/web-dist` assets in packaged builds.
...
| `scripts/build-computer-use.mjs` | Builds the Windows Computer Use helper |
...
`electron:dev` keeps Vite's dependency cache so repeated HMR launches stay
warm. Use `electron:dev:clean` when dependency optimization needs to be rebuilt
from scratch; it clears the Vite cache and starts Vite with `--force`.

The official HMR launcher defaults `OPENCHAMBER_FAST_HMR=1`. On this path Vite
uses its esbuild TSX transform instead of React Fast Refresh's Babel transform,
which cuts Windows cold-start time substantially. React source edits reload the
page instead of preserving component state. Set `OPENCHAMBER_FAST_HMR=0` before
launching when state-preserving React Fast Refresh matters more than startup
time. Direct Vite usage keeps React Fast Refresh unless this variable is set.

The HMR launcher checks whether its paired API port still has a listener. It
does not use the HTTP `/health` response as a process-liveness signal, because a
busy backend may temporarily delay that route while still being alive. Initial
startup and nodemon restarts both get a recovery window; if the API port stays
closed, the HMR stack exits instead of leaving a live Vite window backed by a
dead API.
...
1. `build:computer-use` to publish the Windows Computer Use helper when building on Windows.
...
| `OPENCHAMBER_HMR_CLEAN_CACHE` | Set to `1` to clear Vite's dependency cache and force dependency re-optimization for HMR startup |
| `OPENCHAMBER_FAST_HMR` | Official HMR launchers default this to `1` to skip the React Babel/Fast Refresh transform for faster cold startup; set `0` for state-preserving React Fast Refresh |
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs use the platform log path for the `Ivaldi` app name.

Development builds use a separate user data directory named `Ivaldi Dev`, so dev state does not overwrite normal packaged app state. Backend data uses `~/.config/ivaldi` for fresh installs and reuses an existing `~/.config/openchamber` directory in place. `IVALDI_DATA_DIR` takes precedence over the legacy environment override.
