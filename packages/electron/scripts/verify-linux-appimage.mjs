...
export const verifyExtractedPayload = ({
  ...
  const desktopPath = path.join(root, 'ivaldi.desktop');
  ...
  for (const entry of ['Name=Ivaldi', 'Icon=ivaldi', 'StartupWMClass=ivaldi']) {
  ...
  assertElfArchitecture(path.join(root, 'ivaldi'), targetArchitecture, 'Electron executable');
...
const findAppImage = (version, architecture) => {
  ...
  const expected = path.join(electronRoot, 'dist', `Ivaldi-${version}-linux-${suffix}.AppImage`);
