...
const createPayload = () => {
  ...
  fs.writeFileSync(path.join(root, 'ivaldi.desktop'), [
    '[Desktop Entry]', 'Name=Ivaldi', 'Exec=AppRun --no-sandbox %U', 'Icon=ivaldi', 'StartupWMClass=ivaldi', '',
  ].join('\n'));
  writeElf(path.join(root, 'ivaldi'), 'x64');
