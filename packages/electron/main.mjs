import '@ivaldi/web/server/lib/ivaldi-bootstrap.js';
import { app, BrowserWindow, dialog, globalShortcut, ipcMain, Menu, nativeTheme, net as electronNet, Notification, powerMonitor, powerSaveBlocker, protocol, screen, session, shell, webContents } from 'electron';
...
// ~/Library/Logs/Ivaldi/ (not ~/Library/Logs/@ivaldi/electron/).
app.setName('Ivaldi');
if (process.platform === 'linux') {
  app.setDesktopName('ivaldi.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Ivaldi Dev'));
}
...
// that output never lands in ~/Library/Logs/Ivaldi/main.log and we
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@ivaldi/electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@ivaldi/electron', version: app.getVersion() };
};
...
const quitConfirmationMessage = () => {
  ...
  return `Ivaldi detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
// session expires) reuses the same "Ivaldi Desktop" record instead of
...
/**
 * The browser panel's own session, kept separate from Ivaldi's.
 *
...
import { pathLooksUserConfigured, mergePathValues } from '@ivaldi/web/server/lib/opencode/path-utils.js';
import { clearAppImageArgv0FromProcessEnv } from '@ivaldi/web/server/lib/inherited-env.js';
...
const spawnLocalServer = async () => {
  ...
  const desktopModuleDirectory = isDev && path.basename(__dirname) === 'dist-bundle'
  ...
  const { startWebUiServer } = await import('@ivaldi/web/server/index.js');

  if (process.platform === 'win32' && !state.computerUse) {
    state.computerUse = createWindowsComputerUse({
      packaged: app.isPackaged,
      resourcesPath: process.resourcesPath,
      moduleDirectory: desktopModuleDirectory,
      protectedProcessId: process.pid,
      onStatus: (status) => emitToAllWindows('ivaldi:computer-use-status', status),
    });
  }
...
const buildStartupSplashHtml = () => {
  const settings = readSettingsRoot();
  const splashBgLight = typeof settings.splashBgLight === 'string' ? settings.splashBgLight.trim() : '#f7f7f5';
  const splashFgLight = typeof settings.splashFgLight === 'string' ? settings.splashFgLight.trim() : '#1a1a1a';
  const splashBgDark = typeof settings.splashBgDark === 'string' ? settings.splashBgDark.trim() : '#0c0a09';
  const splashFgDark = typeof settings.splashFgDark === 'string' ? settings.splashFgDark.trim() : '#e7e7e7';
  ...
  <head>
    ...
    <style>
      ...
      .ivaldi-splash-mark > :not(.ivaldi-mark) {
        display: none;
      }
    </style>
  </head>
  <body>
    <div class="stack">
      <svg class="ivaldi-splash-mark" width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ivaldi loading icon">
        ...
        <path class="ivaldi-mark" d="M24 10h52l14 14v52L76 90H24L10 76V24L24 10Z" fill="var(--splash-face-fill)"/>
        <path class="ivaldi-mark" d="M29 27h42v12H58v22h13v12H29V61h13V39H29V27Z" fill="var(--splash-stroke)"/>
      </svg>
    </div>
  </body>
  </html>`;
...
const waitForStartupSplashVisible = (browserWindow, timeoutMs = 250) => new Promise((resolve) => {
  ...
  const finish = () => {
    ...
    settled = true;
    clearTimeout(timer);
    browserWindow.removeListener('ready-to-show', finish);
    resolve();
  };
  const timer = setTimeout(finish, timeoutMs);
  timer.unref?.();
  browserWindow.once('ready-to-show', finish);
});
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'Ivaldi',
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to Ivaldi server?',
...
const setupAutoUpdater = () => {
  if (!app.isPackaged || !IVALDI_DESKTOP_UPDATES_ENABLED) {
    return;
  }
...
const getDevTunnelClient = async () => {
  if (!devTunnelClientPromise) {
    devTunnelClientPromise = import('@ivaldi/web/server/lib/dev-tunnel/client.js')
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_get_computer_use_status':
      return state.computerUse?.status() ?? { supported: process.platform === 'win32', available: false, active: false, target: null };

    case 'desktop_stop_computer_use':
      await state.computerUse?.stop();
      return state.computerUse?.status() ?? { supported: process.platform === 'win32', available: false, active: false, target: null };
    ...
    // on the remote Ivaldi host, so the browser panel loads a real origin
     ...
     *
     * nativeTheme.themeSource is app-wide and would drag Ivaldi's own
     ...
     * to third-party hosts from Ivaldi itself. The bytes come back as a
    ...
    // touch Ivaldi's session or any other window's storage.
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/openchamber/credentials', '.config/ivaldi/credentials'];
    ...
    case 'desktop_set_window_theme': {
      ...
      if (canUseTitleBarOverlay(browserWindow)) {
        ...
        browserWindow.setTitleBarOverlay({
          color: useDark ? '#151515' : '#f7f7f5',
          symbolColor: useDark ? '#e7e7e7' : '#1a1a1a',
          height: 48,
        });
      }
      return null;
    ...
    case 'desktop_check_for_updates': {
      if (!IVALDI_DESKTOP_UPDATES_ENABLED) {
    ...
    case 'desktop_download_and_install_update':
      if (!IVALDI_DESKTOP_UPDATES_ENABLED) throw new Error('Ivaldi desktop updates are not configured for this fork');
      assertUpdaterCapability({ packaged: app.isPackaged });
      if (!state.pendingUpdate) {
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires Ivaldi.app to be installed in /Applications');
          }
        } catch (error) {
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About Ivaldi', click: () => dispatchAction('about') },
        {
...
app.whenReady().then(async () => {
  ...
  // The backend runs in this same Electron main process. Let Chromium paint
  // the tiny splash before synchronous server module evaluation occupies the
  // main thread, otherwise startup looks frozen despite creating the window.
  await waitForStartupSplashVisible(state.mainWindow);
