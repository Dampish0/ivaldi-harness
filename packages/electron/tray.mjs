...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'Ivaldi — no active sessions';
  const bits = [];
  ...
  return `Ivaldi — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show Ivaldi', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit Ivaldi', click: () => onAction({ type: 'quit' }) },
      );
    }
