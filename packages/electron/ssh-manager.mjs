...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded Ivaldi health');
};
...
export class ElectronSshManager {
  ...
  async issueClientToken(localUrl, openchamberPassword) {
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'Ivaldi Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured Ivaldi UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'Ivaldi Desktop SSH' }),
    });
  ...
  async installOpenChamberManaged(parsed, controlPath, version, preferred) {
    ...
    const bunCommand = bunPath ? `${shellQuote(bunPath)} add -g @ivaldi/web@${version}` : null;
    const npmCommand = npmPath
      ? `mkdir -p "${REMOTE_USER_PREFIX}" && ${shellQuote(npmPath)} install -g --prefix "${REMOTE_USER_PREFIX}" @ivaldi/web@${version}`
      : null;
    ...
    throw lastError || new Error('Failed to install Ivaldi on remote host');
  }
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (openchamberPassword && authStatus !== 200) {
          throw new Error(`Remote Ivaldi requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote Ivaldi requires UI authentication on /api/system/info; configure the Ivaldi UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote Ivaldi probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === 'external') {
      if (!instance.remoteOpenchamber.preferredPort) {
        throw new Error('External mode requires a preferred remote Ivaldi port');
      }
      const port = instance.remoteOpenchamber.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external Ivaldi server', null, null, port, false, 0, false);
    ...
    this.setStatus(instance.id, 'remote_probe', 'Checking remote Ivaldi installation');
    ...
    if (!binary) {
      ...
      if (existing) {
        this.setStatus(instance.id, 'updating', `Updating remote Ivaldi from ${existing.version || 'unknown'} to ${this.appVersion}`);
      } else {
        this.setStatus(instance.id, 'installing', 'Installing Ivaldi on remote host');
      }
      ...
      if (!binary) {
        throw new Error('Ivaldi was installed on the remote host but no openchamber binary could be found');
      }
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed Ivaldi server');
    ...
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed Ivaldi server');
    ...
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredOpenChamberPassword(instance)))) {
      throw new Error('Managed Ivaldi server failed to become reachable');
    }
