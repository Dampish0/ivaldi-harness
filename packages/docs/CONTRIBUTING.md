...
This package is docs content source-of-truth for Ivaldi.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `openchamber tunnel` to expose a running Ivaldi instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run Ivaldi behind...").
...
You should land on the Ivaldi session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local Ivaldi)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access Ivaldi from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the Ivaldi app ships in.
...
---
title: Встановлення
description: Встановіть Ivaldi для десктопа, вебу або VS Code.
---
```
...
- brand and product nouns: Ivaldi, OpenCode, VS Code, PWA, GitHub, Discord,
  macOS, SSH
...
## Manual website integration

For a separately maintained Starlight website, the upstream
`openchamber-website` layout is a reference for integrating this content.
...
Copy and publish content manually to the intended Ivaldi website. This repository
does not dispatch updates to the upstream website. See [DEPLOYMENT.md](DEPLOYMENT.md)
