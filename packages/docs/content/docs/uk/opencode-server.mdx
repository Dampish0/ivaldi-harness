...
description: Підключіть Ivaldi до локального або віддаленого сервера OpenCode.
---

# OpenCode Server

Ivaldi працює поверх сервера OpenCode. За замовчуванням він сам запускає такий сервер, тож вам нічого робити не доводиться. Ця сторінка потрібна лише тоді, коли ви хочете спрямувати Ivaldi на сервер, який ви вже запускаєте, або керувати тим, що він запускає.

## Як Ivaldi знаходить сервер

Коли Ivaldi стартує, він шукає сервер у такому порядку:
...
Задайте це перед запуском Ivaldi:
...
- `OPENCODE_SKIP_START=true` — каже Ivaldi не запускати власний сервер.

Якщо вам потрібно лише змінити порт, задайте `OPENCODE_PORT` замість `OPENCODE_HOST`.

Якщо в `OPENCODE_HOST` бракує порту або є шлях, Ivaldi ігнорує його й повертається до запуску власного сервера. Слідкуйте за журналами запуску на предмет попередження `[config]`, якщо очікуване підключення не відбулося.
...
## Запуск Ivaldi при вході в систему

Скористайтеся `startup enable`, щоб встановити нативний користувацький сервіс. Ivaldi використовує `launchd` на macOS, `systemd --user` на Linux і Task Scheduler на Windows.
...
Щоб створити link підключення для іншого застосунку Ivaldi, використайте:
...
Поки сервер запускається або перезапускається, Ivaldi показує стан «OpenCode перезапускається» і призупиняє запити, доки той не буде готовий. Це нормально одразу після запуску чи перезапуску. Якщо стан так і не зникає, див. [Підключення до OpenCode](/uk/troubleshooting/opencode-connection/).
