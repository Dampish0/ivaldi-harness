---
title: Tunnels
description: Exposez Ivaldi en sécurité pour l’accès distant et mobile.
---

# Tunnels

Un tunnel est un lien public vers votre Ivaldi, pour y accéder depuis un navigateur ordinaire sur un autre réseau. Utilisez `openchamber tunnel` pour en créer un pour une instance en cours d’exécution.
...
Ivaldi démarre la CLI du fournisseur de tunnel sur votre machine. Installez d’abord le fournisseur que vous voulez utiliser :
...
Quand le tunnel est actif, `status` affiche une URL publique. Ouvrez-la, ou scannez le QR code, pour joindre Ivaldi depuis n’importe où.

Par défaut, Ivaldi affiche un QR code dans les sessions TTY interactives. Utilisez `--qr` pour forcer la sortie QR, ou `--no-qr` pour la désactiver.
...
- un tunnel actif par instance Ivaldi (port)
...
- [PWA et mobile](/mobile/) — joindre Ivaldi depuis votre téléphone
