---
title: Reverse proxy
description: Configurez correctement Ivaldi derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez Ivaldi derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord qu’Ivaldi fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si Ivaldi compresse déjà les réponses.
...
- Ivaldi joignable directement sur le LAN
...
- Ivaldi compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par Ivaldi, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
