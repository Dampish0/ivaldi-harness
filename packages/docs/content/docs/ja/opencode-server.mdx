---
title: OpenCode サーバー
description: Ivaldi をローカルまたはリモートの OpenCode サーバーに接続します。
---

# OpenCode サーバー

Ivaldi は OpenCode サーバーの上で動作します。デフォルトでは Ivaldi がサーバーを起動するため、何もする必要はありません。このページが必要なのは、すでに実行しているサーバーに Ivaldi を向けたい場合、または Ivaldi が起動するサーバーを管理したい場合だけです。

## Ivaldi がサーバーを見つける順序

Ivaldi の起動時、次の順序でサーバーを探します。
...
- `OPENCODE_SKIP_START=true` — Ivaldi に自分のサーバーを起動しないよう伝えます。

ポートだけを変えたい場合は、`OPENCODE_HOST` ではなく `OPENCODE_PORT` を設定します。

`OPENCODE_HOST` にポートがない、またはパスが含まれている場合、Ivaldi はそれを無視し、自分のサーバー起動にフォールバックします。期待した接続が行われなかった場合は、起動ログの `[config]` 警告を確認してください。
...
`startup enable` を使うと、ネイティブのユーザーサービスをインストールできます。Ivaldi は macOS では `launchd`、Linux では `systemd --user`、Windows では Task Scheduler を使います。
...
別の Ivaldi アプリ向けに接続リンクを作成するには、次を使います。
...
サーバーの起動中または再起動中、Ivaldi は "OpenCode is restarting" 状態を表示し、準備が整うまでリクエストを一時停止します。これは起動直後や再起動直後には正常です。いつまでも消えない場合は、[OpenCode 接続](/troubleshooting/opencode-connection/) を参照してください。
