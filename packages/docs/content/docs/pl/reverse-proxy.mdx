---
title: Reverse proxy
description: Skonfiguruj Ivaldi poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Ivaldi za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Ivaldi działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Ivaldi już kompresuje odpowiedzi.
...
- Ivaldi dostępny bezpośrednio w LAN
...
- Ivaldi kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Ivaldi, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
