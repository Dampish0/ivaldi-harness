...
description: Napraw brak połączenia Ivaldi z serwerem OpenCode.
---

# Połączenie z OpenCode

Jeśli Ivaldi się ładuje, ale nigdy nie przechodzi poza „OpenCode uruchamia się ponownie”, albo czat nie odpowiada, serwer, z którym się komunikuje, jest nieosiągalny. Przejdź przez te kroki.
...
Jeśli ustawiłeś Ivaldi tak, aby używał istniejącego serwera, sprawdź ponownie konfigurację z [OpenCode Server](/pl/opencode-server/):

- `OPENCODE_HOST` musi zawierać port, bez ścieżki — na przykład `http://localhost:4096`
- ustaw `OPENCODE_SKIP_START=true`, aby Ivaldi nie uruchamiał też własnego
- jeśli adres jest nieprawidłowy, Ivaldi go ignoruje i uruchamia własny serwer — poszukaj ostrzeżenia `[config]` w logach
...
- [OpenCode Server](/pl/opencode-server/) — jak Ivaldi znajduje serwer i nim zarządza
