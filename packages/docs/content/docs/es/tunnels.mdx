---
title: Túneles
description: Expón Ivaldi de forma segura para acceso remoto y móvil.
---

# Túneles

Un túnel es un enlace público a tu Ivaldi, para que puedas acceder a él desde un navegador corriente en otra red. Usa `openchamber tunnel` para crear uno para una instancia en marcha.
...
Ivaldi inicia el CLI del proveedor de túnel en tu máquina. Instala primero el proveedor que quieras usar:
...
Cuando el túnel está activo, `status` muestra una URL pública. Ábrela, o escanea el código QR, para acceder a Ivaldi desde cualquier lugar.

Por defecto, Ivaldi imprime un código QR en sesiones TTY interactivas. Usa `--qr` para forzar la salida del QR, o `--no-qr` para desactivarla.
...
- un único túnel activo por instancia de Ivaldi (puerto)
...
- [PWA y acceso móvil](/es/mobile/) — accede a Ivaldi desde tu teléfono
