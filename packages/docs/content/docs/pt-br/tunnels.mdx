---
title: Túneis
description: Exponha o Ivaldi com segurança para acesso remoto e móvel.
---

# Túneis

Um túnel é um link público para o seu Ivaldi, para que você possa acessá-lo de um navegador comum em outra rede. Use `openchamber tunnel` para criar um para uma instância em execução.
...
O Ivaldi inicia o CLI do provedor de túnel na sua máquina. Instale primeiro o provedor que você quer usar:
...
Quando o túnel estiver no ar, o `status` mostra uma URL pública. Abra-a, ou escaneie o QR code, para acessar o Ivaldi de qualquer lugar.

Por padrão, o Ivaldi imprime um QR code em sessões TTY interativas. Use `--qr` para forçar a saída do QR, ou `--no-qr` para desativá-la.
...
- um único túnel ativo por instância do Ivaldi (porta)
...
- [PWA e Acesso Móvel](/pt-br/mobile/) — acesse o Ivaldi pelo celular
