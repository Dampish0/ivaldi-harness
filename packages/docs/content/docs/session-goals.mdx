...
A goal turns one prompt into a finish line. Instead of nudging the agent with "continue" after every reply, you set a goal once — and Ivaldi keeps the session working toward it automatically, checking progress with an independent auditor after every turn. It keeps running even while you are away.
...
After the agent stops and the session stays quiet for a moment, Ivaldi:
...
- After the Ivaldi server starts or reconnects, it discovers saved active goals and checks live session activity before continuing. Paused, blocked, completed, and budget-limited goals stay untouched. Failed reads retry without assuming the session is idle.
- If recovery finds an interrupted turn or uncertain continuation delivery, the goal becomes blocked with a reason. Inspect the session before taking action. For an interrupted turn, send a message to continue the conversation, then select **Resume** for the goal. For uncertain delivery, confirm no work is running before selecting **Resume**. Recovery does not replay unfinished tool actions.
- The goal loop runs in the Ivaldi server, not in your browser tab. Close the tab, lock the phone — the agent keeps working, and you get a notification when the goal settles. The server (desktop app or `openchamber` process) must stay running.
