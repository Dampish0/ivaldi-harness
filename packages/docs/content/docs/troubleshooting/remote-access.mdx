---
title: Remote Access
description: Fix tunnels, remote instances, and reaching Ivaldi from another device.
---

# Remote Access

When you can't reach Ivaldi from your phone or another machine, the fix depends on how you're connecting.
...
- for **Anywhere** pairing, check **Settings → Remote Instances → Ivaldi Relay** on the server: it should say **Connected**; if not, disable and re-enable it
...
When a [remote instance](/remote-instances/) stalls, Ivaldi names the step that failed:

- **auth** — your SSH or UI password was rejected; re-enter it
- **install / start** — Ivaldi couldn't set up or start the server on the remote machine; check that machine's requirements
- **forwarding** — the connection is up but the port isn't reaching you; try a different local port

## Behind your own server

If you put Ivaldi behind a reverse proxy and it loads oddly or won't connect, see [Reverse Proxy](/reverse-proxy/).
