---
title: Reverse Proxy
description: Configure Ivaldi correctly behind Nginx, Nginx Proxy Manager, or another reverse proxy.
---

# Reverse Proxy

Use this page if you run Ivaldi behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm Ivaldi works directly first.
...
- Disable gzip on the proxy if Ivaldi is already compressing responses.
...
- Ivaldi reachable directly on LAN
...
- Ivaldi compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by Ivaldi, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
