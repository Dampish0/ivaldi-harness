---
title: Tunnel
description: Stelle Ivaldi sicher für Remote- und Mobilzugriff bereit.
---

# Tunnel

Ein Tunnel ist ein öffentlicher Link zu deinem Ivaldi, damit du ihn von einem normalen Browser in einem anderen Netzwerk aus erreichen kannst. Nutze `openchamber tunnel`, um einen für eine laufende Instanz zu erstellen.
...
Ivaldi startet das CLI des Tunnel-Anbieters auf deinem Rechner. Installiere zuerst den Anbieter, den du verwenden möchtest:
...
Sobald der Tunnel aktiv ist, zeigt `status` eine öffentliche URL. Öffne sie oder scanne den QR-Code, um Ivaldi von überall aus zu erreichen.

Standardmäßig druckt Ivaldi in interaktiven TTY-Sitzungen einen QR-Code. Mit `--qr` erzwingst du die QR-Ausgabe, mit `--no-qr` deaktivierst du sie.
...
- ein aktiver Tunnel pro Ivaldi-Instanz (Port)
...
- [PWA & Mobile](/mobile/) — Ivaldi vom Telefon aus erreichen
