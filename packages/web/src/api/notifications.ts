import type { NotificationPayload, NotificationsAPI } from '@ivaldi/ui/lib/api/types';
...
const notifyWithServiceWorker = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    await registration.showNotification(payload?.title ?? 'Ivaldi', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
...
const notifyWithWebAPI = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    ...
    new Notification(payload?.title ?? 'Ivaldi', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
