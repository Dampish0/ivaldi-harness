...
vi.mock('@ivaldi/ui/lib/runtime-auth', () => ({
...
vi.mock('@ivaldi/ui/lib/runtime-fetch', () => ({ installRuntimeFetchBridge: vi.fn() }));
vi.mock('@ivaldi/ui/lib/runtime-switch', () => ({
  getRuntimeApiBaseUrl: vi.fn(() => ''),
  getRuntimeKey: vi.fn(() => 'local'),
  initializeRuntimeEndpoint: vi.fn(),
  switchRuntimeEndpoint: vi.fn(),
}));
vi.mock('@ivaldi/ui/lib/desktopRelayRestore', () => ({ restoreDesktopRelayRuntime: vi.fn(() => Promise.resolve()) }));
vi.mock('@ivaldi/ui/lib/runtime-url', () => ({ configureRuntimeUrlResolver: vi.fn(() => ({})) }));
vi.mock('@ivaldi/ui/lib/opencode/client', () => ({ opencodeClient: { reconnectToRuntimeBaseUrl: vi.fn() } }));
vi.mock('./api', () => ({ createWebAPIs: vi.fn() }));

import { setRuntimeBearerToken, setRuntimeExtraHeaders } from '@ivaldi/ui/lib/runtime-auth';
import { initializeRuntimeEndpoint, switchRuntimeEndpoint } from '@ivaldi/ui/lib/runtime-switch';
import { restoreDesktopRelayRuntime } from '@ivaldi/ui/lib/desktopRelayRestore';
