...
function showHelp() {
  console.log(`
 Ivaldi - Web interface for the OpenCode AI coding agent
...
ENVIRONMENT:
  ...
  OPENCHAMBER_DATA_DIR         Override Ivaldi data directory
...
EXAMPLES:
  ...
  openchamber startup enable     # Start Ivaldi at user login
...
function showControlHelp() {
  console.log(`
 Ivaldi Control Commands

USAGE:
  openchamber <COMMAND> [OPTIONS]

COMMANDS:
  status                         Show running Ivaldi runtimes
...
COMMON OPTIONS:
  ...
  -p, --port <port>              Target a specific Ivaldi runtime
...
DESCRIPTION:
  ...
  Ivaldi app. If no server is running on the selected port, it starts one.
...
COMMON OPTIONS:
  -p, --port              Target Ivaldi instance port
...
BEHAVIOR NOTES:
  - One active tunnel per Ivaldi instance.
