...
async function discoverUnconfirmedRegistryInstanceOnPort(port, options = {}) {
  ...
  // A free recorded port proves this registry entry is stale regardless of
  // whether process identity is readable. Removing the registry files is safe
  // here because we do not signal the PID, which may have been recycled.
  ...
  // An occupied port alone is not enough to kill a PID. Only return a process
  // for PID-based recovery after command-line identity confirms it is ours.
