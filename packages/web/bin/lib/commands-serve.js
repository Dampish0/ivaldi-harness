...
async function serveCommand(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'Ivaldi serve' });
    }
    ...
    if (targetPort !== 0) {
      ...
      if (existingInstance?.runtime === 'desktop') {
        throw new Error(
          `Port ${targetPort} is used by Ivaldi Desktop app. Choose another port or stop the desktop app.`
        );
      }
      if (existingInstance) {
        const pidSuffix = Number.isFinite(existingInstance.pid) ? ` (PID: ${existingInstance.pid})` : '';
        if (existingInstance.source === 'probe') {
          throw new Error(`Ivaldi is already running on port ${targetPort}. Use \`openchamber status\` or \`openchamber stop --port ${targetPort}\`.`);
        }
        throw new Error(`Ivaldi is already running on port ${targetPort}${pidSuffix}`);
      }
    ...
    if (options.foreground) {
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting Ivaldi on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    child.unref();
    serveSpin?.start(`Starting Ivaldi on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`Ivaldi daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`Ivaldi daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start Ivaldi');
