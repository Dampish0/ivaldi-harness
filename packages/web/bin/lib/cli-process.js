...
function readProcessCmdline(pid) {
  try {
    ...
    if (process.platform === 'win32') {
      // Windows has no /proc equivalent. A stale PID file cannot be trusted
      // on liveness alone because Windows can recycle that PID for an
      // unrelated process. Query the process command line through CIM so
      // lifecycle recovery can distinguish our process before it ever sends a
      // termination signal. Keep this hidden because it runs from normal CLI
      // status/stop flows.
      ...
      const script = [
        `$process = Get-CimInstance Win32_Process -Filter 'ProcessId = ${Math.trunc(pid)}' -ErrorAction SilentlyContinue`,
        "if ($null -ne $process -and $null -ne $process.CommandLine) { [Console]::Out.Write($process.CommandLine) }",
      ].join('; ');
  ...
  // Unknown platforms: identity cannot be established, so callers fall back
  // to the conservative unknown state.
  return null;
}
...
function isOpenchamberCmdline(cmdline) {
  ...
  // Accept Ivaldi installs and existing OpenChamber process records.
  const normalized = cmdline.toLowerCase();
  return normalized.includes('ivaldi') || normalized.includes('openchamber');
}
