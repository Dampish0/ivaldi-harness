...
The primary executable is `ivaldi`, with `openchamber` retained as an alias. Bootstrap resolves `IVALDI_DATA_DIR`, then the legacy override, then an existing Ivaldi or legacy directory. Fresh installs default to `~/.config/ivaldi`. Both environment names are set before modules capture paths, so legacy runtime consumers share the resolved location.
...
- `commands-connect-url.js`
  ...
  - Emits an `ivaldi://connect?v=2&p=<base64url>` pairing link with a one-time pairing id, secret, and transport candidates from `client-pairing-sessions.json`. Clients redeem the secret through `/api/client-auth/pairing/redeem`. Updated clients also read legacy `openchamber://connect` links. Keep the emitted prefix in sync with `packages/ui/src/lib/connectionPayload.ts`.
