...
function spawnUnrelatedIdleProcess() {
  return spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'unrelated-idle'], { stdio: 'ignore' });
}
...
describe('cli API target resolution', () => {
  ...
  it('fails when multiple non-default instances are running', async () => {
    await expect(resolveTargetPort({}, {
      ...
      isServerHealthReady: async () => false,
    })).rejects.toThrow('Multiple Ivaldi instances are running');
  });
});
...
describe('isOpenchamberCmdline', () => {
  it('accepts OpenChamber CLI and daemon cmdlines', () => {
    expect(isOpenchamberCmdline('node /x/@ivaldi/web/bin/cli.js serve')).toBe(true);
    expect(isOpenchamberCmdline('node /x/@ivaldi/web/server/index.js --port 9090')).toBe(true);
...
describe('isOpenchamberProcessRunning', () => {
  ...
  // A live but unrelated process (a recycled stale PID) must read as not-running
  // on every platform where command-line identity is available.
  it.skipIf(!['linux', 'darwin', 'win32'].includes(process.platform))(
    'returns false for a live non-OpenChamber PID',
    async () => {
      const child = process.platform === 'win32'
        ? spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'unrelated-process'], { stdio: 'ignore' })
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  ...
  it('plain stop ignores a stale CLI registry entry that resolves to desktop runtime', async () => {
    await withTempOpenChamberDataDir(async () => {
      ...
      const child = spawnUnrelatedIdleProcess();
