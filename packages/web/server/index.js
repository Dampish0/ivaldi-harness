import './lib/ivaldi-bootstrap.js';
import { resolveIvaldiDataDirectory } from './lib/ivaldi-data-dir.js';
...
import { ensureIvaldiOriginalSkills } from './lib/opencode/ivaldi-original-skills.js';
...
import { createExecutionBoundaryPolicyRuntime } from './lib/execution-boundary/policy.js';
import { createExecutionBoundaryRuntime } from './lib/execution-boundary/runtime.js';
...
import { createLifecycleHookRuntime } from './lib/lifecycle-hooks/runtime.js';
import { createOpenCodeLifecycleEventAdapter } from './lib/lifecycle-hooks/event-adapter.js';
import { createLifecycleHookManagementService } from './lib/lifecycle-hooks/management.js';
...
const OPENCHAMBER_USER_CONFIG_ROOT = resolveIvaldiDataDirectory();
...
const settingsRuntime = createSettingsRuntime({
  ...
  onSettingsChanged: (settings) => {
    lifecycleHookRuntime?.replaceHooks(settings?.lifecycleHooks);
  },
});
...
sessionGoalRuntime.start(globalMessageStreamHub);
const lifecycleEventAdapter = createOpenCodeLifecycleEventAdapter();
const executionBoundaryPolicyRuntime = createExecutionBoundaryPolicyRuntime({ path, crypto });
const executionBoundaryRuntime = createExecutionBoundaryRuntime({
  platform: process.platform,
  policyRuntime: executionBoundaryPolicyRuntime,
  spawnImpl: spawn,
...
const permissionAutoAcceptRuntime = createPermissionAutoAcceptRuntime({
  ...
  getModeCapabilities: () => executionBoundaryRuntime.getPermissionModeCapabilities(),
});
...
notificationTriggerRuntime.setGetShouldAutoAcceptPermission(
  (permission, directory) => permissionAutoAcceptRuntime.shouldAutoAcceptPermission(permission, directory),
);
...
globalMessageStreamHub.subscribeEvent((event) => {
  ...
  if (lifecycleHookRuntime?.hasPassiveHooks()) {
    for (const lifecyclePayload of lifecycleEventAdapter.map(event)) {
      lifecycleHookRuntime.dispatch(lifecyclePayload);
    }
  }
});
...
const serverUtilsRuntime = createServerUtilsRuntime({
  ...
  runLifecycleHooks: (payload) => {
    const configuration = lifecycleHookRuntime?.getConfigurationStatus();
    ...
    if (!lifecycleHookRuntime?.hasHooksFor(payload?.event)) {
      return Promise.resolve({ ok: true, status: 200, results: [] });
    }
    return lifecycleHookRuntime.execute(payload);
  },
...
const openCodeLifecycleRuntime = createOpenCodeLifecycleRuntime({
  ...
  getManagedOpenCodeEnv: async () => {
    ...
    const includeComputer = process.platform === 'win32' && process.env.OPENCHAMBER_RUNTIME === 'desktop' && Boolean(desktopComputerUse);
...
const openChamberSessionService = createOpenChamberSessionService({
  ...
  sessionKnowledgeRuntime,
  emitLifecycleEvent: (payload) => lifecycleHookRuntime?.dispatch(payload),
});
...
const openChamberControlService = createOpenChamberControlService({
  ...
  computerUse: {
    execute: (...args) => {
      if (!desktopComputerUse) throw new OpenChamberControlError('Computer Use is unavailable', 503);
      return desktopComputerUse.execute(...args);
    },
  },
  ...
  lifecycleHooks: lifecycleHookManagementService,
});
...
async function main(options = {}) {
  desktopComputerUse = options.computerUse ?? null;
  ...
  const getActiveServerPort = () => {
    ...
    return Number.isInteger(address?.port) ? address.port : null;
  };
  agentToolRuntime = createAgentToolRuntime({
    ...
    getActivePort: getActiveServerPort,
  });
  lifecycleHookRuntime = createLifecycleHookRuntime({
    env: process.env,
    launchProcess: (request) => executionBoundaryRuntime.launchProcess(request),
  });
  notificationEmitterRuntime.setOnNotification((notification) => {
    lifecycleHookRuntime?.dispatch({
      event: 'Notification',
      sessionId: notification.sessionId,
      directory: notification.directory ?? null,
      notification,
    });
  });
  try {
    const lifecycleSettings = await readSettingsFromDiskMigrated();
    lifecycleHookRuntime.replaceHooks(lifecycleSettings?.lifecycleHooks);
  } catch (error) {
    lifecycleHookRuntime.markConfigurationUnavailable(error);
    console.warn('[lifecycle-hooks] Failed to load initial configuration:', error?.message ?? error);
  }
  ...
  console.log(`Starting Ivaldi on port ${port === 0 ? 'auto' : port}`);

  const originalSkills = ensureIvaldiOriginalSkills();
  if (originalSkills.conflicts.length > 0) {
    console.warn(
      `[Ivaldi Skills] Kept user-owned skills that conflict with Ivaldi originals: ${originalSkills.conflicts.join(', ')}`,
    );
  }
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    ...
    getServerLabel: () => {
      try {
        ...
        return typeof name === 'string' && name.trim().length > 0 ? name.trim() : 'Ivaldi';
      } catch {
        return 'Ivaldi';
      }
    },
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    lifecycleHookManagementService,
  });
  ...
  void scheduledTasksRuntime.start().catch((error) => {
  ...
  return {
    ...
    getQuitRiskStatus: async () => {
      await scheduledTasksRuntime.whenReady?.();
      return {
        tunnel: {
