...
describe('OpenCode proxy lifecycle hook preflight', () => {
  let upstreamServer;
  let proxyServer;
  ...
  it('runs UserPromptSubmit before forwarding the parsed message request', async () => {
    let upstreamBody = null;
    let hookPayload = null;
    let hookCalls = 0;
    const upstream = express();
    upstream.use(express.json());
    upstream.post('/session/:sessionId/message', (req, res) => {
    ...
    registerOpenCodeProxy(app, {
      ...
      runLifecycleHooks: async (payload) => {
        hookCalls += 1;
        hookPayload = payload;
    ...
    const message = { parts: [{ type: 'text', text: 'review this' }] };

    const response = await fetch(`http://127.0.0.1:${proxyPort}/api/session/session-1/message`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        'x-opencode-directory': encodeURIComponent('C:/work/project'),
    ...
    expect(upstreamBody).toEqual(message);
  ...
  it('returns 409 without forwarding when a blocking hook rejects the prompt', async () => {
    let upstreamCalls = 0;
    const upstream = express();
    upstream.use(express.json());
    upstream.post('/session/:sessionId/message', (_req, res) => {
      upstreamCalls += 1;
      res.json({ ok: true });
    });
    upstreamServer = await listen(upstream);
    ...
    registerOpenCodeProxy(app, {
      ...
      runLifecycleHooks: async () => ({
        ...
        hookId: 'policy',
        error: 'Lifecycle hook "policy" blocked the prompt: Hook exited with code 3',
        results: [],
      }),
    });
    proxyServer = await listen(app);
    ...
    const response = await fetch(`http://127.0.0.1:${proxyPort}/api/session/session-2/message`, {
      method: 'POST',
  ...
  it('runs the same preflight when proxying to an external OpenCode base URL', async () => {
    let hookCalls = 0;
    let upstreamCalls = 0;
    const upstream = express();
    upstream.use(express.json());
    upstream.post('/session/:sessionId/message', (_req, res) => {
      upstreamCalls += 1;
      res.json({ ok: true, source: 'external' });
    });
    upstreamServer = await listen(upstream);
    const upstreamPort = upstreamServer.address().port;
    ...
    const response = await fetch(`http://127.0.0.1:${proxyPort}/api/session/external-session/message`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ parts: [{ type: 'text', text: 'external' }] }),
    });

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ ok: true, source: 'external' });
    expect(hookCalls).toBe(1);
    expect(upstreamCalls).toBe(1);
  });

  it('returns a stable infrastructure error contract when preflight evaluation throws', async () => {
    let upstreamCalls = 0;
    const upstream = express();
    upstream.use(express.json());
    upstream.post('/session/:sessionId/message', (_req, res) => {
    ...
    registerOpenCodeProxy(app, {
      ...
      ensureOpenCodeApiPrefix: () => {},
      runLifecycleHooks: async () => {
        throw new Error('settings observer exploded');
      },
    });
    proxyServer = await listen(app);
    const proxyPort = proxyServer.address().port;

    const response = await fetch(`http://127.0.0.1:${proxyPort}/api/session/session-3/message`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    await expect(response.json()).resolves.toEqual({
      code: 'LIFECYCLE_HOOK_EVALUATION_FAILED',
      error: 'Lifecycle hook evaluation failed',
    });
    expect(upstreamCalls).toBe(0);
  });
});
