...
describe('checkForUpdates', () => {
  ...
  beforeEach(() => {
    vi.stubEnv('IVALDI_UPDATE_API_URL', 'https://updates.ivaldi.test/check');
    vi.stubEnv('OPENCHAMBER_UPDATE_API_URL', '');
  ...
  it('does not contact an upstream update service without explicit configuration', async () => {
    vi.stubEnv('IVALDI_UPDATE_API_URL', '');
    fetchMock.when('registry.npmjs.org', { ok: false, status: 404 });
    const result = await checkForUpdates({ currentVersion: '1.20.0' });
    expect(result.available).toBe(false);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(fetchMock.mock.calls[0][0]).toBe('https://registry.npmjs.org/@ivaldi/web');
  });
  ...
  it('returns available=false when npm only has a prerelease of the current version', async () => {
    fetchMock
      .when('updates.ivaldi.test', Promise.reject(new Error('Network error')))
      .when('registry.npmjs.org', {
        ok: true,
        json: async () => ({
    ...
    fetchMock
      .when('updates.ivaldi.test', {
        ...
        json: async () => ({
          latestVersion: '1.10.0',
          updateAvailable: true,
          downloadUrl: 'https://github.com/Dampish0/ivaldi-harness/releases/download/v1.10.0/OpenChamber-1.10.0-42-android.aab',
        }),
      })
      .when('api.github.com/repos/Dampish0/ivaldi-harness/releases/tags/v1.10.0', {
        ok: true,
        json: async () => ({
          assets: [
            {
  ...
  it('keeps a direct Android APK URL from the update API', async () => {
    const apkUrl = 'https://github.com/Dampish0/ivaldi-harness/releases/download/v1.10.0/OpenChamber-1.10.0-42-android.apk';
    fetchMock.when('updates.ivaldi.test', {
      ok: true,
      json: async () => ({
        latestVersion: '1.10.0',
        updateAvailable: true,
