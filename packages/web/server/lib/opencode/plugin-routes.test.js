...
const testUnlessRootOrWindows = process.platform === 'win32' || (typeof process.getuid === 'function' && process.getuid() === 0)
  ? test.skip
  : test;
...
describe('opencode plugin routes', () => {
  ...
  testUnlessRootOrWindows('GET /registry reports unreadable path plugin', async () => {
    createRegistryApp(mock(async () => ({ ok: true, latest: '1.0.0', versions: ['1.0.0'], distTags: { latest: '1.0.0' } })));
