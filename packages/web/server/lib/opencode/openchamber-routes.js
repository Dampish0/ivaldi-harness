  ...
  app.post('/api/openchamber/update-install', async (_req, res) => {
    try {
      ...
      if (isForegroundService) {
        ...
        return res.json({
          success: true,
          message: 'Update queued; Ivaldi will restart after installation completes',
          version: updateInfo.version,
          packageManager: pm,
          autoRestart: true,
      ...
      const logPreamble = [
        '',
        `=== Ivaldi update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting Ivaldi...
              ${restartCmd || 'echo Service manager will restart Ivaldi.'}
            ) else (
              echo Update failed
              exit /b 1
            )
