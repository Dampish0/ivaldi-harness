...
export const registerCommonRequestMiddleware = (app, dependencies) => {
  ...
  app.use((req, res, next) => {
    if (req.path.startsWith('/api/behavior')) {
      ...
      (req.method === 'POST' && /^\/api\/session\/[^/]+\/message(?:\/|$)/.test(req.path)) ||
      ...
      req.path.startsWith('/api/permission-policy') ||
      req.path.startsWith('/api/lifecycle-hooks') ||
