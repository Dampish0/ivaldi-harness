...
- `packages/web/server/lib/lifecycle-hooks/runtime.js`: user-level lifecycle hook command execution. `proxy.js` owns the authoritative `UserPromptSubmit` preflight before forwarding message requests to OpenCode.
...
- Registers OpenChamber-owned lifecycle hook management routes through `lib/lifecycle-hooks/routes.js` before generic OpenCode proxy forwarding.
...
## Ivaldi original skills
- Canonical Ivaldi-authored skills live under `packages/web/server/ivaldi-original-skills/` so they ship with the web package and Electron runtime.
- `ensureIvaldiOriginalSkills()` installs them into the user's global OpenCode skills directory before managed OpenCode starts, making the same skill available across projects and visible in Settings.
- Each original skill carries `ivaldi_original: true` and an integer `ivaldi_version`. A newer bundled version updates an older Ivaldi-managed copy. A user-owned skill with the same name is preserved and reported as a conflict instead of being overwritten.
...
- Owns:
  ...
  - Session message forwarder: `POST /api/session/:sessionId/message`
    - Runs the OpenChamber `UserPromptSubmit` lifecycle preflight exactly once before forwarding. The same boundary applies to managed and external OpenCode targets. Blocking hooks return HTTP 409 and infrastructure failures return HTTP 500 before any upstream message request is made.
