...
export const registerOpenCodeProxy = (app, deps) => {
  ...
  const SESSION_MESSAGE_PATH = /^\/session\/([^/]+)\/message(?:\/|$)/;
  const getSessionMessageId = (req) => {
    if (req.method !== 'POST') return null;
    const match = SESSION_MESSAGE_PATH.exec(req.path);
    return match?.[1] || null;
  };
  const getRequestDirectory = (req) => {
    const headers = normalizeForwardedDirectoryHeaders({ ...req.headers });
    const headerDirectory = headers['x-opencode-directory'];
    if (typeof headerDirectory === 'string' && headerDirectory.trim()) return headerDirectory.trim();
  ...
  app.use('/api', async (req, res, next) => {
    const sessionId = getSessionMessageId(req);
    if (!sessionId || typeof runLifecycleHooks !== 'function') return next();

    try {
      const result = await runLifecycleHooks({
        event: 'UserPromptSubmit',
        sessionId,
        directory: getRequestDirectory(req),
        message: req.body,
      });
      if (result?.ok !== false) return next();
      return res.status(result.status === 409 ? 409 : 500).json({
        code: result.code || 'LIFECYCLE_HOOK_EVALUATION_FAILED',
        error: result.error || 'Lifecycle hook blocked the prompt',
        ...(result.hookId ? { hookId: result.hookId } : {}),
      });
    } catch (error) {
      console.error('[proxy] Lifecycle hook preflight failed:', error?.message ?? error);
  ...
  if (process.platform === 'win32') {
    app.get('/api/session', async (req, res, next) => {
      ...
      try {
        ...
        const settingsPath = path.join(resolveIvaldiDataDirectory(), 'settings.json');
        let projectDirs = [];
        try {
