...
import { sanitizeLifecycleHooks } from '../lifecycle-hooks/config.js';
...
export const createSettingsHelpers = (dependencies) => {
  ...
  const sanitizeSettingsUpdate = (payload) => {
    ...
    if (candidate.permissionAutoAccept && typeof candidate.permissionAutoAccept === 'object' && !Array.isArray(candidate.permissionAutoAccept)) {
      const modes = {};
      const sourceModes = candidate.permissionAutoAccept.modes;
      if (sourceModes && typeof sourceModes === 'object' && !Array.isArray(sourceModes)) {
        for (const [sessionId, mode] of Object.entries(sourceModes)) {
          if (sessionId && (mode === 'manual' || mode === 'auto' || mode === 'full-access')) {
      ...
      if (sourceSessions && typeof sourceSessions === 'object' && !Array.isArray(sourceSessions)) {
        for (const [sessionId, enabled] of Object.entries(sourceSessions)) {
          ...
          if (!Object.hasOwn(modes, sessionId)) modes[sessionId] = enabled ? 'full-access' : 'manual';
        }
      }
      for (const [sessionId, mode] of Object.entries(modes)) {
        sessions[sessionId] = mode === 'full-access';
      }
    ...
    if (Array.isArray(candidate.lifecycleHooks)) {
      result.lifecycleHooks = sanitizeLifecycleHooks(candidate.lifecycleHooks) ?? [];
    }
