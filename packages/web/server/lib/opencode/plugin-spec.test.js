...
describe('parsePathSpec', () => {
  ...
  test('tilde home shorthand with subpath', () => {
    ...
    expect(spec.parsePathSpec('~/x.js', { homedir, cwd: path.resolve('/p') })).toEqual({
      absolutePath: path.join(homedir, 'x.js'),
    });
  });

  test('bare tilde = homedir', () => {
    const homedir = path.resolve('/home/u');
    expect(spec.parsePathSpec('~', { homedir, cwd: path.resolve('/p') })).toEqual({
  ...
  test('relative ./', () => {
    ...
    expect(spec.parsePathSpec('./x.js', { homedir: path.resolve('/home/u'), cwd })).toEqual({
      absolutePath: path.resolve(cwd, './x.js'),
    });
  });

  test('relative ../', () => {
    const cwd = path.resolve('/p/a');
    expect(spec.parsePathSpec('../x.js', { homedir: path.resolve('/home/u'), cwd })).toEqual({
      absolutePath: path.resolve(cwd, '../x.js'),
    });
  });
