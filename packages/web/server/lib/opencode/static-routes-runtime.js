...
<head>
  ...
  <title>Ivaldi API-only mode</title>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  <main>
    <svg class="logo" viewBox="108 105 302 302" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ivaldi logo">
      <defs>
        <mask id="ivaldi-gap" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" x="0" y="0" width="512" height="512">
          <rect width="512" height="512" fill="#fff"/>
          <polygon points="252.50,256.00 422.50,133.00 459.50,220.00 454.50,314.00" fill="#000"/>
        </mask>
      </defs>
      <circle cx="252.50" cy="256.00" r="118.00" stroke="currentColor" stroke-width="48.00" mask="url(#ivaldi-gap)"/>
      <polygon points="374.50,207.00 401.50,234.00 374.50,261.00 347.50,234.00" fill="currentColor"/>
    </svg>
    <h1>Ivaldi is running in headless mode</h1>
    <p>This server is ready. Open it from the Ivaldi desktop or mobile app to use it.</p>
    <div class="command">
  ...
  <script>
    ...
    button?.addEventListener('click', async () => {
      ...
      try {
        ...
        json: () => {
          res.json({ ok: true, mode: 'api-only', message: 'Ivaldi is running in API-only mode' });
        },
        default: () => {
          res.type('text/plain').send('Ivaldi is running in API-only mode');
        },
      });
    });
  };
