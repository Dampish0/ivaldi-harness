...
<title>${escapeHtml(title)} — Ivaldi</title>
<style>
...
<script>window.location.href = 'openchamber://focus/mcp-auth';</script>` : ''}
      ...
      if (!capability.supported) {
        return res.status(409).json({
          ...
          error: capability.reason === 'bundled'
            ? 'OpenCode is bundled with Ivaldi Desktop and updates with the app.'
            : 'This OpenCode runtime cannot be upgraded by Ivaldi.',
        });
      }
  ...
  app.get('/mcp/oauth/callback', async (req, res) => {
    ...
    if (!context?.name) {
      return finish(400, {
        title: 'Authorization Failed',
        message: 'This authorization session has expired or is unknown to the running app. Return to Ivaldi and click Authorize again.',
      });
    }
    ...
    try {
      ...
      return finish(200, {
        ...
        message: 'You can close this tab and return to Ivaldi.',
      });
    } catch (error) {
      return finish(502, {
        title: 'Authorization Failed',
