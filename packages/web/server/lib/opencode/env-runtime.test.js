...
const isolateWindowsOpencodeFallbacks = () => {
  const root = createTempDir('openchamber-windows-discovery-');
  ...
  process.env.OPENCHAMBER_BUNDLED_OPENCODE_CLI_DIR = path.join(root, 'bundled-cli');
...
afterEach(() => {
  ...
  restoreEnv('OPENCODE_PATH', originalOpencodePath);
  restoreEnv('OPENCHAMBER_OPENCODE_PATH', originalOpenChamberOpencodePath);
  restoreEnv('OPENCHAMBER_OPENCODE_BIN', originalOpenChamberOpencodeBin);
...
describe('OpenCode env runtime', () => {
  ...
  it('does not auto-detect the Windows OpenCode desktop app as a CLI', () => {
    setPlatform('win32');
    isolateWindowsOpencodeFallbacks();
    const localAppData = createTempDir('openchamber-localappdata-');
    const desktopBinary = path.join(localAppData, 'Programs', 'OpenCode', 'OpenCode.exe');
    fs.mkdirSync(path.dirname(desktopBinary), { recursive: true });
    fs.writeFileSync(desktopBinary, '');
