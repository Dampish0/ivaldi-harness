...
describe('settings helpers', () => {
  ...
  it('sanitizes the persisted permission auto-accept policy', () => {
    ...
    expect(helpers.sanitizeSettingsUpdate({
      permissionAutoAccept: {
        modes: { root: 'sandbox', child: 'auto', invalid: 'anything' },
        sessions: { legacy: true },
        revision: 4,
      },
    })).toEqual({
      permissionAutoAccept: {
        modes: { child: 'auto', legacy: 'full-access' },
        sessions: { child: false, legacy: true },
        revision: 4,
      },
    });
  });

  it('sanitizes lifecycle hook command settings', () => {
    const helpers = createTestHelpers();
    ...
    expect(helpers.sanitizeSettingsUpdate({ lifecycleHooks: [] })).toEqual({ lifecycleHooks: [] });
    expect(helpers.sanitizeSettingsUpdate({ lifecycleHooks: 'node hook.mjs' })).toEqual({});
  });
