...
import { IVALDI_COMPUTER_ACTION_DEFINITIONS, OPENCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS, OPENCHAMBER_CONTROL_ACTION_DEFINITIONS } from '../openchamber-control/actions.js';
...
describe('agent tool action allowlist', () => {
  it('defines a short title and agent description for every action', () => {
    ...
    expect(IVALDI_COMPUTER_ACTION_DEFINITIONS.every(({ action, title, description }) => action && title && description)).toBe(true);
  });
  ...
  it('forwards the current agent session identity to lifecycle actions', async () => {
    const { runtime, executeAction } = await createRuntime();
    await runtime.execute({
      input: { action: 'lifecycle.test', hookId: 'check' },
    ...
    expect(executeAction).toHaveBeenCalledWith(
      'lifecycle.test',
      { action: 'lifecycle.test', hookId: 'check' },
      '/work/project',
      { sessionId: 'ses_agent' },
    );
  });
...
describe('managed agent tool runtime', () => {
  it('materializes the plugin and preserves configured plugin entries', async () => {
    ...
    expect(hooks.tool.openchamber.args.parameters.properties.command).toMatchObject({
      type: 'array',
      minItems: 1,
    });
    expect(hooks.tool.openchamber.args.parameters.properties.event.enum).toContain('UserPromptSubmit');
    expect(hooks.tool.openchamber.args.parameters.properties.enabled.description).toContain('defaults to false');
    expect(hooks.tool.openchamber.description).toContain('Lifecycle hooks execute local programs');
    expect(hooks.tool.openchamber.description).toContain('Full access host execution');
    expect(hooks.tool.openchamber.description).toContain('lifecycle.test executes it once');
  ...
  it('exposes Computer Use as an isolated permission surface', async () => {
    const { runtime, dataDir } = await createRuntime();
    await runtime.prepareManagedOpenCodeEnv({ includeControl: false, includeWeb: false, includeMemory: false, includeComputer: true });
    const pluginPath = path.join(dataDir, 'agent-tool', 'openchamber-plugin.js');
    const pluginModule = await import(`${pathToFileURL(pluginPath).href}?computer=${Date.now()}`);
    const { tool } = await pluginModule.OpenChamberPlugin();

    expect(Object.keys(tool)).toEqual(['ivaldi_computer']);
    expect(tool.ivaldi_computer.args.action.enum).toContain('computer.snapshot');
    expect(Object.keys(tool.ivaldi_computer.args.parameters.properties).sort()).toEqual([
      'amount', 'direction', 'elementId', 'fromX', 'fromY', 'key', 'modifiers', 'snapshotId',
      'text', 'toX', 'toY', 'windowId', 'x', 'y',
    ]);
  });

  it('returns Computer Use screenshots as tool attachments and scopes its lease to the session', async () => {
    const attachment = { type: 'file', mime: 'image/png', url: 'data:image/png;base64,AA==', filename: 'snapshot.png' };
