...
- `openchamber` — projects, sessions, worktrees, scheduled tasks, and lifecycle
  hooks. Enabled
...
5. The route delegates the fixed action allowlist directly to the shared
   ...
   partial-failure reporting, scheduled-task contracts, and lifecycle-hook
   management have one owner.
...
- Lifecycle hook creation defaults to disabled. `enabled: true` is reserved for
  a user request to activate or install the executable hook. `lifecycle.test`
  executes the saved command once and therefore requires an explicit request to
  test or run it. Hook execution currently uses the shared execution boundary's
  Full access host mode through the normal host process launcher.
...
- Inputs map to a fixed action and parameter allowlist. Session, schedule,
  browser, and project actions do not expose arbitrary CLI, shell, route, or URL
  forwarding. Lifecycle actions may store a validated argv command because that
  is the lifecycle-hook contract; agent-created hooks default disabled and use
  the same validation and persistence path as Settings.
...
The managed `openchamber` tool exposes five user-level lifecycle operations:

- `lifecycle.list` returns saved hooks, supported events, and recent runs.
- `lifecycle.create` appends a validated hook. `hookId`, `event`, and argv-array
  `command` are required; `enabled` defaults to `false` for agent-created hooks.
- `lifecycle.update` changes only supplied fields on the exact `hookId` and
  preserves its list position.
- `lifecycle.delete` removes the exact `hookId`.
- `lifecycle.test` runs a saved hook once without changing configuration. The
  current agent session and directory are carried into the dry-run payload and
  recent execution record.

All mutations pass through the lifecycle management service, which validates
the complete ordered list before persisting it. The agent tool never edits
`settings.json` directly. Project-local executable hooks remain outside this
surface until a project trust policy exists.
