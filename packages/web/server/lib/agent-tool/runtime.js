...
import {
  ...
  IVALDI_COMPUTER_ACTION_DEFINITIONS,
...
import { LIFECYCLE_HOOK_EVENTS, LIFECYCLE_HOOK_FAILURE_MODES } from '../lifecycle-hooks/config.js';
...
const ACTIONS = new Set([...OPENCHAMBER_AGENT_TOOL_ACTIONS, ...OPENCHAMBER_WEB_ACTIONS, ...OPENCHAMBER_MEMORY_ACTIONS, ...IVALDI_COMPUTER_ACTIONS]);
...
const COMPUTER_PARAMETER_NAMES = ['windowId', 'snapshotId', 'elementId', 'x', 'y', 'text', 'key', 'modifiers', 'direction', 'amount', 'fromX', 'fromY', 'toX', 'toY'];
...
const ALL_PARAMETER_PROPERTIES = {
  ...
  hookId: { type: 'string', description: 'Exact lifecycle hook ID. Use letters, numbers, dot, underscore, or dash' },
  event: { type: 'string', enum: LIFECYCLE_HOOK_EVENTS, description: 'Lifecycle event that runs the hook' },
  command: { type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 32, description: 'Command argv array: executable first, then separate arguments. Never pass a shell command string' },
  enabled: { type: 'boolean', description: 'Whether the lifecycle hook is active. lifecycle.create defaults to false; set true only when the user asked to activate or install the executable hook' },
  timeoutMs: { type: 'integer', minimum: 100, maximum: 120000, description: 'Lifecycle hook timeout in milliseconds' },
  failureMode: { type: 'string', enum: LIFECYCLE_HOOK_FAILURE_MODES, description: 'warn continues after failure. block is only valid for UserPromptSubmit and stops that message' },
  ...
  windowId: { type: 'string', description: 'Window ID returned by window.list' },
  snapshotId: { type: 'string', description: 'Current snapshot ID; required for every input action so stale observations cannot act' },
  elementId: { type: 'string', description: 'Accessibility element ID from the current snapshot' },
  x: { type: 'integer', minimum: 0, description: 'Horizontal client coordinate from the current snapshot' },
  y: { type: 'integer', minimum: 0, description: 'Vertical client coordinate from the current snapshot' },
  key: { type: 'string', description: 'Key name such as ENTER, TAB, ESCAPE, LEFT, RIGHT, UP, DOWN, HOME, END, DELETE, BACKSPACE, or a single character' },
  modifiers: { type: 'array', items: { type: 'string', enum: ['CTRL', 'ALT', 'SHIFT', 'META'] }, description: 'Optional key modifiers' },
  amount: { type: 'integer', minimum: 1, maximum: 20, description: 'Scroll notch count (default 3)' },
...
const CONTROL_PARAMETER_PROPERTIES = pickParameters(
  Object.keys(ALL_PARAMETER_PROPERTIES).filter((name) => (
    !WEB_PARAMETER_NAMES.includes(name) && !MEMORY_ONLY_PARAMETER_NAMES.includes(name) && !COMPUTER_PARAMETER_NAMES.includes(name)
  )),
);
...
const COMPUTER_TOOL_DESCRIPTION = "Inspect and operate one visible Windows desktop window through Ivaldi. This is local desktop control, not web automation. Use one action per call. Start with window.list, select a safe target, then use the returned snapshotId for every click, type, key, scroll, or drag. A stale snapshot is rejected: inspect again instead of guessing. Prefer accessibility elementId over coordinates. Never type secrets or interact with password managers, security tools, authentication prompts, elevated windows, UAC, or the Ivaldi window itself. computer.stop releases control immediately.";
...
const createResult = ({ ok, action, data, error, exitCode, attachments }) => ({
...
const createToolEntry = ({ name, description, actions, definitions, parameters }) => String.raw`    ${name}: {
      ...
      args: {
        action: { type: "string", enum: ${JSON.stringify(actions)}, oneOf: ${JSON.stringify(definitions.map((entry) => ({ const: entry.action, description: entry.description })))}, description: "Ivaldi action to perform" },
      ...
      async execute(input, context) {
        ...
        if (!endpoint || !token) {
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "Ivaldi managed tool connection is unavailable" } })
        }
        ...
        try {
          const response = await fetch(endpoint, {
            ...
            body: JSON.stringify({ input: args, contextDirectory: context.directory, sessionId: context.sessionID, tool: ${JSON.stringify(name)} }),
          ...
          if (valid) {
            const { attachments, ...printable } = result
            return {
              title,
              output: JSON.stringify(printable),
              ...(Array.isArray(attachments) ? { attachments } : {}),
              metadata: { openchamber: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: result.ok === true } },
            }
          }
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "Ivaldi returned an invalid response", kind: "runtime", status: response.status } })
        } catch (error) {
...
const createPluginSource = ({ includeControl, includeWeb, includeMemory, includeComputer }) => {
  const entries = [];
  if (includeControl) {
  ...
  if (includeComputer) {
    entries.push(createToolEntry({
      name: 'ivaldi_computer',
      description: COMPUTER_TOOL_DESCRIPTION,
      actions: IVALDI_COMPUTER_ACTIONS,
      definitions: IVALDI_COMPUTER_ACTION_DEFINITIONS,
...
const mergePluginConfig = (rawConfig, pluginUrl) => {
  ...
  if (errors.length > 0 || !parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('OPENCODE_CONFIG_CONTENT must contain a valid JSON object before Ivaldi can inject its managed tool');
  }
  if (parsed.plugin !== undefined && !Array.isArray(parsed.plugin)) {
    throw new Error('OPENCODE_CONFIG_CONTENT plugin must be an array before Ivaldi can inject its managed tool');
  }
...
export const createAgentToolRuntime = (dependencies) => {
  ...
  const prepareManagedOpenCodeEnv = async ({ includeControl = true, includeWeb = true, includeMemory = true, includeComputer = false } = {}) => {
    const port = getActivePort();
    if (!Number.isInteger(port) || port <= 0) {
      throw new Error('Ivaldi listener port is unavailable for managed tool injection');
    }
    if (!includeControl && !includeWeb && !includeMemory && !includeComputer) {
      throw new Error('At least one Ivaldi managed tool must be enabled to inject the plugin');
    }
    await fsPromises.mkdir(pluginDirectory, { recursive: true });
    await fsPromises.writeFile(pluginPath, createPluginSource({ includeControl, includeWeb, includeMemory, includeComputer }), { mode: 0o600 });
  ...
  const execute = async (payload = {}, options = {}) => {
    ...
    if (!ACTIONS.has(action)) {
      return createResult({ ok: false, action, error: { message: `Unsupported Ivaldi action: ${action}`, kind: 'usage' } });
    }
    if (typeof executeAction !== 'function') {
      return createResult({ ok: false, action, error: { message: 'Ivaldi control service is unavailable', kind: 'runtime' } });
    }
    try {
      let actionOptions = options;
      if (action.startsWith('computer.') || action.startsWith('window.')) {
        actionOptions = {
          ...options,
          ownerId: asNonEmptyString(payload.sessionId) || asNonEmptyString(payload.contextDirectory) || 'managed-session',
        };
      } else if (action.startsWith('lifecycle.')) {
        actionOptions = { ...options };
        const sessionId = asNonEmptyString(payload.sessionId);
        if (sessionId) actionOptions.sessionId = sessionId;
      }
      const result = await executeAction(action, { ...payload.input, action }, payload.contextDirectory, actionOptions);
      if (result?.__openchamberToolResult === true) {
        return createResult({ ok: true, action, data: result.data, attachments: result.attachments });
      }
      return createResult({ ok: true, action, data: result });
    } catch (error) {
      return createResult({
        ok: false,
        action,
  ...
  const registerRoutes = (app, express) => {
    app.post('/api/openchamber/agent-tool', express.json({ limit: '8mb' }), async (req, res) => {
