...
//
// Stream connections reconcile persisted goals once, then idle events drive
// progress. Failed discovery and failed authoritative reads retry.
...
import { createSessionGoalRecovery } from './recovery.js';
...
const continuationCursorSchema = z.string().catch('');
const liveStatusesSchema = z.record(z.string(), z.object({ type: z.enum(['idle', 'busy', 'retry']) }));
const childrenSchema = z.array(z.object({ id: z.string().min(1) }));
...
const parseGoalMetadata = (session) => {
  ...
  return {
    ...
    continuationAfterMessageID: continuationCursorSchema.parse(goal.continuationAfterMessageID),
...
export const createSessionGoalRuntime = ({
  ...
  const cancelTick = (sessionId) => {
    inflight.get(sessionId)?.abort();
    inflight.delete(sessionId);
  };

  const openCodeFetch = async (fetchPath, { directory, method = 'GET', body, query, signal = connection.signal } = {}) => {
  ...
  const fetchSessionStatuses = async (directory) => {
    ...
    const parsed = liveStatusesSchema.safeParse(statuses);
  ...
  const writeGoal = async (sessionId, directory, expectedGoal, mutate, signal = connection.signal) => {
    const session = await openCodeFetch(`/session/${encodeURIComponent(sessionId)}`, { directory, signal });
    const currentGoal = parseGoalMetadata(session);
    if (!currentGoal || currentGoal.id !== expectedGoal.id || currentGoal.updatedAt !== expectedGoal.updatedAt
      || currentGoal.status !== 'active' || session.time?.archived) return null;
  ...
  const settleGoal = async ({ sessionId, directory, goal, status, statusReason, note, tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID, evaluationProviderID, evaluationModelID, signal }) => {
  ...
  const sendContinuation = async ({ sessionId, directory, goal, lastAssistantInfo, signal }) => {
  ...
  const tick = async (sessionId, directory, signal) => {
    if (!isEnabled()) return;
    ...
    const header = sessionHeaderSchema.safeParse(session);
    if (!header.success || header.data.id !== sessionId) {
      armTimer(sessionId, directory, idleQuietMs);
      return;
    }
    if (header.data.time?.archived) return;
    // Sub-agent/task sessions never carry user goals — skip them.
    if (header.data.parentID) return;
    ...
    if (goal.objectiveFile) {
      ...
      if (fileObjective) {
        ...
        await settleGoal({ sessionId, directory, goal, signal, status: 'blocked', statusReason: 'objective file unavailable; restore the objective before resuming' });
        return;
      } else {
    ...
    const interruptedTail = lastMessageInfo?.role === 'user'
      || (lastAssistantInfo && !(lastAssistantInfo.time?.completed > 0) && !lastAssistantInfo.error);
    if (interruptedTail) {
      if (recovered.has(sessionId)) {
        await settleGoal({ sessionId, directory, goal, signal, status: 'blocked', statusReason: 'interrupted turn; inspect session and send a message before resuming goal' });
        recovered.delete(sessionId);
      }
      return;
    }
    ...
    // The marker is written before dispatch. A restart cannot distinguish a
    // lost response from a request that never arrived, so never replay it.
    if (goal.continuationAfterMessageID === lastAssistantInfo.id && goal.statusReason !== 'resumed') {
      await settleGoal({ sessionId, directory, goal, signal, status: 'blocked', statusReason: 'continuation delivery uncertain; inspect session before resuming' });
      return;
    }
    ...
    if (!abortedTail && lastAssistantInfo.error && typeof lastAssistantInfo.error === 'object') {
      ...
      await settleGoal({
        sessionId, directory, goal, signal, status: 'blocked', statusReason: reason, tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID,
      });
      return;
    }
    ...
    if (typeof goal.tokenBudget === 'number' && tokensUsed >= goal.tokenBudget) {
      await settleGoal({
        sessionId, directory, goal, signal, status: 'budgetLimited', statusReason: 'token budget reached', tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID,
      });
      return;
    }
    ...
    if (goal.turnsUsed >= maxAutoTurns) {
      await settleGoal({
        sessionId, directory, goal, signal, status: 'blocked', statusReason: 'auto-continuation limit reached', tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID,
      });
      return;
    }
    ...
    if (lastAssistantInfo.summary === true || abortedTail) {
      ...
      if (!audit) {
        ...
        if (auditFailStreak >= AUDIT_FAIL_LIMIT) {
          await settleGoal({
            sessionId, directory, goal, signal, status: 'blocked', statusReason: 'progress audit unavailable', tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID,
          });
          return;
        }
      ...
      if (audit?.verdict === 'complete') {
        await settleGoal({
          sessionId, directory, goal, signal, status: 'complete', statusReason: 'verified by audit', note: audit.note, tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID,
      ...
      if (audit?.verdict === 'blocked') {
        ...
        if (blockedStreak >= BLOCKED_STREAK_LIMIT) {
          await settleGoal({
            sessionId, directory, goal, signal, status: 'blocked', statusReason: audit.note || 'blocked per audit', note: audit.note, tokensUsed, tokensBaseline, tokensCommitted, lastAccountedMessageID,
            evaluationProviderID: audit.evaluationProviderID, evaluationModelID: audit.evaluationModelID,
          });
          return;
        }
    ...
    // Audit may take time. Recheck the transcript and live activity before
    // reserving a continuation, then persist its marker before dispatch.
    signal.throwIfAborted();
    const latest = await fetchRecentMessages(sessionId, directory);
    if (!latest) throw new Error('Cannot recheck goal transcript');
    const latestLastInfo = latest.at(-1)?.info;
    if (!latestLastInfo || latestLastInfo.id !== lastMessageInfo?.id) return;
    const currentStatuses = await fetchSessionStatuses(directory);
    const currentChildren = await fetchSessionChildren(sessionId, directory);
    if (!currentStatuses || !currentChildren) throw new Error('Cannot recheck goal activity');
    if (isWorkingStatus(currentStatuses[sessionId]) || currentChildren.some((child) => isWorkingStatus(currentStatuses[child.id]))) return;
    ...
    const written = await writeGoal(sessionId, directory, goal, (current) => ({
      ...
      continuationAfterMessageID: lastAssistantInfo.id,
      blockedStreak,
      auditFailStreak,
      statusReason: '',
    ...
    await sendContinuation({ sessionId, directory, goal: { ...written, objective: effectiveObjective }, lastAssistantInfo: executionInfo ?? lastAssistantInfo, signal });
  };

  const armTimer = (sessionId, directory, quietMs) => {
    if (stopped || connection.signal.aborted) return;
    ...
    const timer = setTimeout(() => {
      ...
      tick(sessionId, directory, signal)
        .catch((error) => {
  ...
  const processPayload = (payload, directoryHint = '') => {
    ...
    if (aborted) {
      ...
      cancelTick(aborted.sessionId);
      void pauseAfterAbort(aborted.sessionId, directoryHint).catch((error) => {
    ...
    if (update && update.goal?.status !== 'active') {
      clearTimer(update.sessionId);
      if (!update.goal || update.goal.status === 'paused') cancelTick(update.sessionId);
      return;
    }
    if (
      update
  ...
  const recovery = createSessionGoalRecovery({
    buildOpenCodeUrl,
    getOpenCodeAuthHeaders,
    onConnect: () => { connection = new AbortController(); },
    onDisconnect: disconnect,
    onSession: (session) => {
      if (parseGoalMetadata(session)?.status === 'active' && !timers.has(session.id)) {
        recovered.add(session.id);
        armTimer(session.id, session.directory, idleQuietMs);
      }
    },
  });
  ...
  return {
    processPayload,
    start: (hub) => { if (!stopped) recovery.start(hub); },
    stop: () => {
      stopped = true;
      recovered.clear();
      recovery.stop();
