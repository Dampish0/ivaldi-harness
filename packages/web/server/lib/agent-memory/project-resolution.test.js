...
describe('resolving a session directory to its project', () => {
  ...
  test('a directory outside any repository keys by itself', async () => {
    ...
    const looseDirectory = path.resolve('/tmp/loose');
    expect(await resolve(looseDirectory)).toBe(createProjectIdFromPath(looseDirectory));
  });
  ...
  test('managed chat session directories share the Chats root store', async () => {
    ...
    expect(await resolve(path.join(chatsRoot, '2026-08-21', 'session-a'))).toBe(createProjectIdFromPath(chatsRoot));
    expect(await resolve(path.join(chatsRoot, '2026-08-21', 'session-b'))).toBe(createProjectIdFromPath(chatsRoot));
