...
- **`readContext` never takes the write lock.** Every mutator calls it while
  already holding that lock, so taking it again would deadlock. Legacy migration
  has a separate per-project in-flight task. Concurrent readers share that task,
  and the migration owner rechecks `context.json` before writing. This avoids
  duplicate rename attempts on Windows while keeping ordinary reads lock-free.
...
failure simply leaves the migration to run again on the next read. Concurrent
reads share one in-process migration task per project, and a later reader
rechecks `context.json` before starting another migration. Repeat and concurrent
reads therefore converge on identical content on Windows as well as POSIX filesystems.
