...
describe('legacy migration', () => {
  ...
  test('concurrent reads converge on the same migrated content', async () => {
    ...
    let contextRenameCount = 0;
    const migrationFsPromises = {
      ...fsPromises,
      rename: async (source, destination) => {
        if (destination === contextPath()) {
          contextRenameCount += 1;
          // Keep the first migration in-flight long enough that duplicate
          // migrations would overlap deterministically on every platform.
          await new Promise((resolve) => setTimeout(resolve, 25));
          if (contextRenameCount > 1) {
            const error = new Error('simulated Windows concurrent rename collision');
            error.code = 'EPERM';
            throw error;
          }
        }
    ...
    const concurrentRuntime = createProjectContextRuntime({
      fsPromises: migrationFsPromises,
      path,
      projectsDirPath,
      createId: () => `plan-${++idCounter}`,
    });
    ...
    const results = await Promise.all([
      concurrentRuntime.readContext(PROJECT_ID),
      concurrentRuntime.readContext(PROJECT_ID),
      concurrentRuntime.readContext(PROJECT_ID),
    ]);
    ...
    expect(contextRenameCount).toBe(1);
