...
export const createProjectContextRuntime = (deps) => {
  ...
  const legacyMigrationTasks = new Map();
  ...
  const migrateFromLegacyConfigOnce = async (projectId, now) => {
    const key = sanitizeProjectId(projectId);
    const existing = legacyMigrationTasks.get(key);
    if (existing) {
      return existing;
    }

    const task = (async () => {
      // Another reader may have completed migration after this caller observed
      // a missing context file. Re-read before touching either persisted file.
      const current = await readJson(contextPathFor(projectId));
      if (!current.missing) {
        if (!current.value) {
      ...
      return migrateFromLegacyConfig(projectId, now);
    })();

    legacyMigrationTasks.set(key, task);
    try {
      return await task;
    } finally {
      if (legacyMigrationTasks.get(key) === task) {
        legacyMigrationTasks.delete(key);
      }
    }
  };
   ...
   * already holding it, so locking here would deadlock. Legacy migration uses
   * its own per-project in-flight task so concurrent readers share one
   * migration without contending on Windows rename semantics.
   */
  const readContext = async (projectId) => {
    ...
    if (stored.missing) {
      const migrated = await migrateFromLegacyConfigOnce(projectId, now);
      if (migrated) {
        return {
          version: PROJECT_CONTEXT_VERSION,
