export const createNotificationTriggerRuntime = (deps) => {
  ...
  let getShouldAutoAcceptPermission = deps.getShouldAutoAcceptPermission;
  const setGetIsSessionAutoAccepting = (resolver) => {
    getIsSessionAutoAccepting = typeof resolver === 'function' ? resolver : undefined;
  };
  const setGetShouldAutoAcceptPermission = (resolver) => {
    getShouldAutoAcceptPermission = resolver;
  };
  ...
  const maybeSendPushForTrigger = async (payload) => {
    ...
    if (payload.type === 'permission.asked' && sessionId) {
      ...
      const shouldAutoAccept = async () => {
        if (getShouldAutoAcceptPermission) {
          return getShouldAutoAcceptPermission(payload.properties, notificationDirectory);
        }
        return getIsSessionAutoAccepting?.(sessionId, notificationDirectory)
          ?? isSessionAutoAccepting(sessionId, notificationDirectory);
      };
      if (await shouldAutoAccept()) {
  ...
  return {
    ...
    setGetShouldAutoAcceptPermission,
