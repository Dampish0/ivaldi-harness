...
describe('scheduled-tasks runtime initialization', () => {
  const createRuntimeDeps = (overrides = {}) => ({
    projectConfigRuntime: {},
    buildOpenCodeUrl: () => 'http://localhost',
    getOpenCodeAuthHeaders: () => ({}),
  ...
  it('exposes initialization without reporting readiness early', async () => {
    let releaseProjects;
    const projectsReady = new Promise((resolve) => {
      releaseProjects = resolve;
    });
    const runtime = createScheduledTasksRuntime(createRuntimeDeps({
      listProjects: () => projectsReady,
    }));

    const startPromise = runtime.start();
    expect(runtime.getStatus().initializationState).toBe('initializing');
  ...
  it('preserves initialization failure for authoritative callers', async () => {
    const startupError = new Error('scheduled task initialization failed');
    const runtime = createScheduledTasksRuntime(createRuntimeDeps({
      listProjects: async () => {
        throw startupError;
      },
    ...
    expect(runtime.getStatus().initializationState).toBe('error');
    await expect(runtime.whenReady()).rejects.toBe(startupError);
  });
});
