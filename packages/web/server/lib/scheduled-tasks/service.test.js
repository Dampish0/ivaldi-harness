...
describe('scheduled-task service status', () => {
  it('waits for runtime initialization before returning authoritative counts', async () => {
    let releaseReady;
    ...
    let resolved = false;
    const statusPromise = service.status().then((status) => {
      resolved = true;
      return status;
    });
  ...
  it('surfaces runtime initialization failure instead of returning empty status', async () => {
    ...
    await expect(service.status()).rejects.toBe(startupError);
    expect(getStatus).not.toHaveBeenCalled();
  });
});
