...
`start()` begins project and loop reconciliation and returns one shared
initialization promise. Desktop startup does not wait for that work before it
shows the main UI. Callers that require authoritative task counts call
`whenReady()` first. The status route and Desktop quit-risk check do this so an
initializing runtime is never mistaken for an authoritative empty schedule.
