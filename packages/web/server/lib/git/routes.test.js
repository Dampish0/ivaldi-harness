...
describe('git routes index mutations', () => {
  ...
  it('emits lifecycle events only after successful worktree mutations', async () => {
    const created = { path: '/repo/.worktrees/feature', branch: 'feature' };
    ...
    registerGitRoutes(app, { emitLifecycleEvent });

    const createResponse = createMockResponse();
    await getRoute('POST', '/api/git/worktrees')(
      { query: { directory: '/repo' }, body: { name: 'feature' } },
      createResponse,
    );
    ...
    const removeResponse = createMockResponse();
    await getRoute('DELETE', '/api/git/worktrees')(
      { query: { directory: '/repo' }, body: { directory: '/repo/.worktrees/feature', deleteLocalBranch: true } },
      removeResponse,
    );
    expect(removeResponse.statusCode).toBe(200);
    expect(emitLifecycleEvent).toHaveBeenLastCalledWith({
      event: 'WorktreeRemove',
      directory: '/repo',
      worktree: { path: '/repo/.worktrees/feature', deleteLocalBranch: true },
    });
    expect(emitLifecycleEvent).toHaveBeenCalledTimes(2);
  });

  it('does not emit worktree lifecycle events when the mutation fails', async () => {
    gitLibraries.createWorktree.mockRejectedValue(new Error('create failed'));
    const emitLifecycleEvent = vi.fn();
    const { app, getRoute } = createRouteRegistry();
    registerGitRoutes(app, { emitLifecycleEvent });
