export function registerGitRoutes(app, dependencies = {}) {
  const emitLifecycleEvent = dependencies.emitLifecycleEvent || (() => {});
  ...
  app.get('/api/git/status', async (req, res) => {
    ...
    try {
      ...
      // sessions (e.g. sidebar discovery). It is an expected capability result,
      // not a server warning; callers already receive an explicit soft payload.
      if (isNonRepoGitError(error)) {
        return res.json(nonRepoStatusPayload());
      }
