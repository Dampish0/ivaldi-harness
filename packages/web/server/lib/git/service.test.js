...
const runGit = (cwd, args) => {
  ...
  if (args[0] === 'init') {
    // Keep fixtures independent of the developer's global Git line-ending
    // policy. Otherwise checkout/reset assertions change from LF to CRLF on
    // Windows when core.autocrlf=true globally.
    execFileSync('git', ['config', '--local', 'core.autocrlf', 'false'], {
      cwd,
      stdio: 'ignore',
    });
  }
...
const normalizeServicePath = (value) => String(value || '').replace(/\\/g, '/');
...
describe('worktree root resolution', () => {
  it('resolves the git toplevel for a repository subdirectory', async () => {
    ...
    await expect(resolveWorktreeTopLevel(subdirectory)).resolves.toEqual({ root: normalizeServicePath(fs.realpathSync(repo)) });
  });
  ...
  it('resolves the primary worktree root from a linked worktree', async () => {
    ...
    await expect(resolvePrimaryWorktreeRoot(worktree)).resolves.toEqual({ root: normalizeServicePath(fs.realpathSync(repo)) });
  });
});
...
describe('createWorktree', () => {
  ...
  it('runs the post-checkout hook after populating a created worktree', async () => {
    ...
    try {
      ...
      installPostCheckoutHook(
        repo,
        `#!/bin/sh\nprintf '%s|%s|%s|%s' "$1" "$2" "$3" "$(${process.platform === 'win32' ? 'pwd -W' : 'pwd -P'})" > ${JSON.stringify(hookLog)}\n`,
      );
      ...
      expect(flag).toBe('1');
      expect(normalizeServicePath(cwd)).toBe(normalizeServicePath(fs.realpathSync(created.path)));
    } finally {
      if (previousXdgDataHome === undefined) {
        delete process.env.XDG_DATA_HOME;
      } else {
  ...
  it('preflights fast create branch-in-use failures before creating the candidate directory', async () => {
    ...
    try {
      ...
      const canonicalWorktree = normalizeServicePath(fs.realpathSync(worktree));
