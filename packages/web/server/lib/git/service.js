...
const isFilenameTooLongError = (message) => {
  const text = String(message || '');
  if (/file ?name too long|filename or extension is too long|path(?:name)? too long/i.test(text)) {
    return true;
  }

  // Git for Windows reports an overlong individual path component as
  // "cannot create directory ...: Invalid argument" instead of the POSIX
  // ENAMETOOLONG wording. Only classify that generic error as a length error
  // when the path itself proves a filesystem component exceeds 255 chars.
  if (process.platform === 'win32') {
    const match = text.match(/cannot create directory at ['"]([^'"]+)['"]:\s*Invalid argument/i);
    if (match && match[1].split(/[\\/]/).some((component) => component.length > 255)) {
      return true;
    }
  }
  return false;
...
const formatWorktreePopulateError = (message) => {
  ...
  return [
    ...
    'Ivaldi enables Git `core.longpaths` for worktree population; if this still fails on Windows, enable OS long paths (LongPathsEnabled) or open the repository from a shorter absolute path.',
  ].join('\n');
};
...
const runPostCheckoutHook = async (directory) => {
  ...
  // Let Git invoke the hook instead of executing the hook file directly.
  // Extensionless shell hooks cannot be launched with Win32 CreateProcess,
  // while `git hook run` uses Git for Windows' normal hook runner and matches
  // Git's missing/non-executable semantics on every platform.
  const hookResult = await runGitCommand(directory, [
    'hook',
    'run',
    '--ignore-missing',
    'post-checkout',
  ...
  if (!hookResult.success) {
    ...
    console.warn(`[GitService] post-checkout hook failed in worktree ${directory}: ${hookResult.message || hookResult.stderr || hookResult.stdout || 'hook failed'}`);
  }
};
...
const createIntegrateTempWorktree = async (repoRoot, targetBranch) => {
  const tmpParent = path.join(resolveIvaldiDataDirectory(), 'tmp');
...
const runWorktreeStartCommand = async (directory, command) => {
  ...
  if (process.platform === 'win32') {
    // `execFile(cmd.exe, ['/c', text])` double-escapes a command that begins
    // with a quoted executable path, e.g. "C:\\Program Files\\nodejs\\node.exe".
    // `exec` owns the shell command string and preserves cmd.exe quoting.
    const result = await execAsync(text, {
      cwd: directory,
      env: await buildGitEnv(),
...
export async function stashPush(directory, options = {}) {
  ...
  const message = typeof options.message === 'string' && options.message.trim()
    ...
    : `Ivaldi stash ${new Date().toISOString()}`;
