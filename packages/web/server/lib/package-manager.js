...
const PACKAGE_NAME = '@ivaldi/web';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/Dampish0/ivaldi-harness/main/CHANGELOG.md';
const GITHUB_RELEASES_URL = 'https://github.com/Dampish0/ivaldi-harness/releases';
const GITHUB_RELEASES_API_URL = 'https://api.github.com/repos/Dampish0/ivaldi-harness/releases';
...
function getOpenChamberConfigDir() {
  return resolveIvaldiDataDirectory();
}
...
async function checkForUpdatesFromApi(currentVersion, options = {}) {
  const updateCheckUrl = process.env.IVALDI_UPDATE_API_URL?.trim() || process.env.OPENCHAMBER_UPDATE_API_URL?.trim();
