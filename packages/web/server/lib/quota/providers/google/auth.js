...
export const resolveGoogleOAuthClient = (sourceId, environment = process.env) => {
  const prefix = sourceId === 'gemini' ? 'GEMINI' : 'ANTIGRAVITY';
  const clientId = asNonEmptyString(environment[`IVALDI_${prefix}_GOOGLE_CLIENT_ID`]);
  const clientSecret = asNonEmptyString(environment[`IVALDI_${prefix}_GOOGLE_CLIENT_SECRET`]);
