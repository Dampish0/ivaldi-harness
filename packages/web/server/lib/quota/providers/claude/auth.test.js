...
describe('Claude credential discovery', () => {
  ...
  it('honours CLAUDE_CONFIG_DIR when locating the credentials file', () => {
    process.env.CLAUDE_CONFIG_DIR = path.resolve('/tmp/claude-home');
    files.set(path.join(process.env.CLAUDE_CONFIG_DIR, '.credentials.json'), claudeCodeBlob('custom-dir-token'));
