...
describe('quota credential store', () => {
  it('uses owner-only permissions and rejects arbitrary provider paths', () => {
    ...
    const quotaDirectoryStat = fs.statSync(path.join(temporaryDirectory, 'quota'));
    const credentialFileStat = fs.statSync(path.join(temporaryDirectory, 'quota', 'ollama-cloud.json'));
    if (process.platform !== 'win32') {
      expect(quotaDirectoryStat.mode & 0o777).toBe(0o700);
      expect(credentialFileStat.mode & 0o777).toBe(0o600);
    }
    expect(quotaDirectoryStat.isDirectory()).toBe(true);
    expect(credentialFileStat.isFile()).toBe(true);
