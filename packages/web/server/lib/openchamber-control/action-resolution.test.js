...
describe('a namespace the tool name already implies', () => {
  test('resolves a bare action inside the calling tool', () => {
    ...
    expect(resolveAgentToolAction('snapshot', 'ivaldi_computer')).toEqual({ action: 'computer.snapshot' });
  });

  test('resolves a bare name only when it is unique inside the calling tool', () => {
    expect(resolveAgentToolAction('delete', 'openchamber_memory')).toEqual({ action: 'memory.delete' });
    const unresolved = resolveAgentToolAction('delete', 'openchamber');
    expect(unresolved.action).toBeUndefined();
    expect(unresolved.error).toContain('schedule.delete');
    expect(unresolved.error).toContain('lifecycle.delete');
  });
