...
export const createOpenChamberControlService = (dependencies) => {
  ...
  const wait = (duration, signal) => {
    ...
    if (signal.aborted) return Promise.reject(new OpenChamberControlError('Ivaldi action was cancelled', 499));
    return new Promise((resolve, reject) => {
      const onAbort = () => {
        signal.removeEventListener('abort', onAbort);
        reject(new OpenChamberControlError('Ivaldi action was cancelled', 499));
      };
  ...
  const waitForIdle = async ({ client, sessionID, directory, timeoutMs, requireActivity, baselineMessageID, startedAt, signal }) => {
    ...
    while (true) {
      if (signal?.aborted) throw new OpenChamberControlError('Ivaldi action was cancelled', 499);
  ...
  const execute = async (action, input = {}, contextDirectory, options = {}) => {
    try {
      if (!CONTROL_ACTIONS.has(action)) {
        throw new OpenChamberControlError(`Unsupported Ivaldi action: ${action || 'missing'}`, 400);
      }
      ...
      if (action.startsWith('computer.') || action.startsWith('window.')) {
        if (!computerUse) {
          throw new OpenChamberControlError('Computer Use is only available in Ivaldi Desktop on Windows', 503);
        }
        const result = await computerUse.execute(action, input, {
      ...
      if (action.startsWith('lifecycle.')) {
        if (!lifecycleHooks) {
          throw new OpenChamberControlError('Lifecycle hook management is unavailable on this server', 503);
        }
        const hookId = asNonEmptyString(input.hookId);
        const lifecycleFailure = (error) => {
          if (error?.code === 'LIFECYCLE_HOOK_CONFIG_INVALID' && Array.isArray(error.issues) && error.issues[0]?.error) {
            return new OpenChamberControlError(error.issues[0].error, 400);
          }
          return error;
        };
        try {
          if (action === 'lifecycle.list') return lifecycleHooks.list();
          if (!hookId) throw new OpenChamberControlError('hookId is required', 400);
          if (action === 'lifecycle.create') {
            ...
            if (input.timeoutMs !== undefined) hook.timeoutMs = input.timeoutMs;
            if (input.failureMode !== undefined) hook.failureMode = input.failureMode;
            const hooks = await lifecycleHooks.create(hook);
            return { created: true, hook: hooks.find((entry) => entry.id === hookId), hooks };
          }
          if (action === 'lifecycle.update') {
            const patch = {};
            for (const field of ['event', 'command', 'enabled', 'timeoutMs', 'failureMode']) {
              if (Object.hasOwn(input, field)) patch[field] = input[field];
            }
            if (Object.keys(patch).length === 0) {
              throw new OpenChamberControlError('Provide at least one lifecycle hook field to update', 400);
            }
            const hooks = await lifecycleHooks.update(hookId, patch);
            return { updated: true, hook: hooks.find((entry) => entry.id === hookId), hooks };
          }
          if (action === 'lifecycle.delete') return lifecycleHooks.remove(hookId);
          if (action === 'lifecycle.test') {
            const payload = {};
            ...
            return { hookId, result: await lifecycleHooks.testSaved(hookId, payload) };
          }
        } catch (error) {
      ...
      throw asControlError(error, `Failed to execute ${action || 'Ivaldi action'}`);
    }
  };
