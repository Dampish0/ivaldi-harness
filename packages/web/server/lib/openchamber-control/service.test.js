...
describe('OpenChamber control service', () => {
  it('delegates Computer Use with the current lease owner and preserves attachments', async () => {
    const computerUse = { execute: vi.fn(async () => ({
      data: { snapshotId: 'snap-1' },
      attachments: [{ type: 'file', mime: 'image/png', url: 'data:image/png;base64,AA==' }],
    })) };
    const { service } = createService({ computerUse });
    await expect(service.execute('computer.snapshot', {}, undefined, { ownerId: 'ses_1' })).resolves.toEqual({
      __openchamberToolResult: true,
      data: { snapshotId: 'snap-1' },
      attachments: [{ type: 'file', mime: 'image/png', url: 'data:image/png;base64,AA==' }],
    });
    expect(computerUse.execute).toHaveBeenCalledWith('computer.snapshot', {}, { signal: undefined, ownerId: 'ses_1' });
  });
  ...
  it('creates lifecycle hooks disabled by default and maps explicit activation', async () => {
    const lifecycleHooks = {
      create: vi.fn(async (hook) => [{ ...hook, timeoutMs: 10_000, failureMode: 'warn' }]),
    };
    const { service } = createService({ lifecycleHooks });

    await expect(service.execute('lifecycle.create', {
      hookId: 'check-prompt',
      event: 'UserPromptSubmit',
      command: ['node', 'check.mjs'],
    })).resolves.toMatchObject({
      created: true,
      hook: { id: 'check-prompt', enabled: false },
    });
    expect(lifecycleHooks.create).toHaveBeenLastCalledWith({
      id: 'check-prompt',
      event: 'UserPromptSubmit',
      command: ['node', 'check.mjs'],
      enabled: false,
    ...
    await service.execute('lifecycle.create', {
      hookId: 'active',
      event: 'ChatEnd',
    ...
    expect(lifecycleHooks.create).toHaveBeenLastCalledWith(expect.objectContaining({
      id: 'active',
      enabled: true,
      timeoutMs: 5000,
      failureMode: 'warn',
  ...
  it('updates, tests, lists, and deletes lifecycle hooks through the shared manager', async () => {
    const saved = { id: 'check', event: 'ChatEnd', command: ['node'], enabled: false, timeoutMs: 1000, failureMode: 'warn' };
    const lifecycleHooks = {
      list: vi.fn(() => ({ hooks: [saved], supportedEvents: ['ChatEnd'] })),
      update: vi.fn(async (_id, patch) => [{ ...saved, ...patch }]),
      testSaved: vi.fn(async () => ({ hookId: 'check', ok: true })),
      remove: vi.fn(async () => ({ deleted: true, hookId: 'check', hooks: [] })),
    };
    const { service } = createService({ lifecycleHooks });

    await expect(service.execute('lifecycle.list')).resolves.toMatchObject({ hooks: [saved] });
    await expect(service.execute('lifecycle.update', { hookId: 'check', enabled: true })).resolves.toMatchObject({
      updated: true,
      hook: { id: 'check', enabled: true },
    });
    expect(lifecycleHooks.update).toHaveBeenCalledWith('check', { enabled: true });
    await expect(service.execute('lifecycle.test', { hookId: 'check' }, 'C:/repo', { sessionId: 'ses_agent' })).resolves.toEqual({
      hookId: 'check',
      result: { hookId: 'check', ok: true },
    });
    expect(lifecycleHooks.testSaved).toHaveBeenCalledWith('check', { directory: 'C:/repo', sessionId: 'ses_agent' });
    await expect(service.execute('lifecycle.delete', { hookId: 'check' })).resolves.toMatchObject({ deleted: true });
  });

  it('requires an actual lifecycle update and surfaces validator details as usage errors', async () => {
    const lifecycleHooks = {
      update: vi.fn(),
      create: vi.fn(async () => {
        throw Object.assign(new Error('Lifecycle hook configuration is invalid'), {
          statusCode: 400,
          code: 'LIFECYCLE_HOOK_CONFIG_INVALID',
          issues: [{ error: 'Hook ID must use only letters, numbers, dot, underscore, or dash' }],
        });
      }),
    };
    const { service } = createService({ lifecycleHooks });

    await expect(service.execute('lifecycle.update', { hookId: 'check' })).rejects.toThrow('Provide at least one lifecycle hook field to update');
    expect(lifecycleHooks.update).not.toHaveBeenCalled();
    await expect(service.execute('lifecycle.create', {
      hookId: 'bad id',
      event: 'ChatEnd',
      command: ['node'],
    })).rejects.toThrow('Hook ID must use only letters, numbers, dot, underscore, or dash');
  });
  ...
  it('rejects actions outside the fixed contract', async () => {
    const { service } = createService();
    await expect(service.execute('session.delete')).rejects.toThrow('Unsupported Ivaldi action');
  });
});
