...
import { createFastHmrBundlePlugin } from '../../scripts/fast-hmr-bundle.mjs';
...
export default defineConfig(async ({ command }) => {
  const fastHmr = command === 'serve' && process.env.OPENCHAMBER_FAST_HMR === '1';
  const fastHmrBundlePlugin = fastHmr
    ? await createFastHmrBundlePlugin({
  ...
  plugins: [
    ...(fastHmrBundlePlugin ? [fastHmrBundlePlugin] : []),
    ...(fastHmr ? [] : [react({
      babel: {
        // The React Compiler is a production optimization. Development still
        // uses React Fast Refresh unless the official fast-HMR launcher opts
        // into Vite's esbuild TSX path with OPENCHAMBER_FAST_HMR=1.
  ...
  resolve: {
    alias: [
      ...
      { find: '@ivaldi/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  optimizeDeps: {
    ...(fastHmr ? { noDiscovery: true } : {}),
    include: fastHmr ? [] : ['@opencode-ai/sdk/v2'],
  },
  server: {
    port: 5173,
    proxy: {
