...
`useProductModeStore.ts` owns Ivaldi's device-local Work/Developer presentation mode. The mode changes which product chrome and developer-oriented actions are exposed, but it must never select a weaker agent, reduce model/tool capability, or change execution permissions. Developer mode preserves the complete OpenChamber/Ivaldi interface; Work mode is a quieter view over the same underlying agent.
