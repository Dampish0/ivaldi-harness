...
`useProfileStore.ts` owns the web/desktop client's profile name, independently of GitHub and runtime hosts. The versioned `ivaldi-profile` localStorage record accepts a trimmed name of 1–64 characters. Saving writes synchronously before publishing state, so failed writes preserve the old name and cannot complete onboarding. Missing or invalid records require explicit name entry; unreadable records remain untouched until a successful user save. App asks before both the first-launch chooser and a ready workspace. Recovery, embedded chats, OAuth callbacks, the separate mobile app and VS Code retain their existing entry flows. Existing installations without a name get the same one-time prompt. Edit profile uses the same form and store, with two square mode choices. Mode selection stays local to the form until the profile name saves successfully; closing the editor discards the selection. Onboarding retains its name-only form.

`useSessionTabsStore.ts` owns the desktop/web header's per-client open-tab working set. It persists tab ids/order plus a bounded title display snapshot for those open and recently closed tabs. The snapshot exists only so a cold launch can paint the previous tab strip before the global session request finishes. It is not session authority: unresolved restored tabs stay non-navigable and expose no session actions until live/global session metadata arrives, at which point the store refreshes the snapshot. Authoritative removal prunes both the id and its snapshot.
...
Skills detail reads follow the same authority rule. `404` is an authoritative
missing skill; transport and server failures are errors and must not be
collapsed into `null` or blank description/instruction fields. The Skills
surface keeps an already loaded detail visible when refresh fails and exposes a
retry state when no authoritative detail is available.
