...
interface SkillsStore {
  ...
  /** Failed authoritative list refreshes, keyed by the same directory identity. */
  skillsLoadErrorByDirectory: Record<string, boolean>;
  isLoading: boolean;
...
export const selectSkillsLoadErrorForDirectory = (
  state: Pick<SkillsStore, 'skillsLoadErrorByDirectory'>,
  directory?: string | null,
): boolean => {
  const cacheKey = getSkillsCacheKey(resolveDirectory(directory));
  return state.skillsLoadErrorByDirectory[cacheKey] === true;
};
    ...
    persist(
      (set, get) => ({
        ...
        loadSkills: async (requestedDirectory?: string | null) => {
          ...
          const request = (async () => {
            set((state) => ({
              isLoading: true,
              skillsLoadErrorByDirectory: {
                ...state.skillsLoadErrorByDirectory,
                [cacheKey]: false,
              },
            }));
        ...
        readSupportingFile: async (skillName: string, filePath: string, requestedDirectory?: string | null) => {
          ...
          if (!response.ok) {
            throw new Error(`Failed to load supporting file: ${response.status}`);
          }
