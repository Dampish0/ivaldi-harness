...
interface CommandsStore {
  ...
  loadErrorsByDirectory: Record<string, string | null>;
  isLoading: boolean;
  commandDraft: CommandDraft | null;

  setSelectedCommand: (name: string | null) => void;
  setCommandDraft: (draft: CommandDraft | null) => void;
  loadCommands: (directory?: string | null, options?: { force?: boolean }) => Promise<boolean>;
...
export const selectCommandsLoadErrorForDirectory = (
  state: Pick<CommandsStore, 'loadErrorsByDirectory'>,
  directory?: string | null,
): string | null => {
  const cacheKey = getCommandsCacheKey(resolveDirectory(directory));
  return state.loadErrorsByDirectory[cacheKey] ?? null;
};
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        loadCommands: async (requestedDirectory?: string | null, options?: { force?: boolean }) => {
          ...
          if (!options?.force && hasCachedCommands && now - loadedAt < COMMANDS_LOAD_CACHE_TTL_MS) {
            return true;
          }
          ...
          const request = (async () => {
                ...
                if (previousSignature !== nextSignature) {
                  set((state) => {
                    const next: Partial<CommandsStore> = {
                      ...
                      loadErrorsByDirectory: { ...state.loadErrorsByDirectory, [cacheKey]: null },
                      isLoading: false,
                    };
            ...
            set((state) => {
              const next: Partial<CommandsStore> = {
                ...
                loadErrorsByDirectory: {
                  ...state.loadErrorsByDirectory,
                  [cacheKey]: lastError instanceof Error ? lastError.message : 'Failed to load commands',
                },
                isLoading: false,
              };
