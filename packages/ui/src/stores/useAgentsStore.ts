...
interface AgentsStore {
  ...
  loadAgents: (directory?: string | null, options?: { force?: boolean }) => Promise<boolean>;
...
export const selectAgentsLoadErrorForDirectory = (
  state: Pick<AgentsStore, 'loadErrorsByDirectory'>,
  directory?: string | null,
): string | null => {
  const cacheKey = getAgentsCacheKey(resolveDirectory(directory));
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        loadAgents: async (requestedDirectory?: string | null, options?: { force?: boolean }) => {
          ...
          if (!options?.force && hasCachedAgents && now - loadedAt < AGENTS_LOAD_CACHE_TTL_MS) {
            return true;
          }
                ...
                if (attempt === 2) {
                  set((state) => ({
                    ...
                    loadErrorsByDirectory: {
                      ...
                      [cacheKey]: error instanceof Error ? error.message : 'Failed to load agents',
                    },
                  }));
                }
              }
