    ...
    (set, get) => ({
        ...
        const fetchPromise = (async () => {
          ...
          try {
            ...
            if (isRequestCurrent(token, directory)) {
              ...
              newDirectories.set(directory, {
                ...currentDirState,
                statusError: error instanceof Error ? error.message : 'Failed to fetch Git status',
              });
              set({ directories: newDirectories });
            }
          } finally {
      ...
      fetchBranches: async (directory, git) => {
        ...
        try {
          ...
          newDirectories.set(directory, {
            ...d,
            branchesError: error instanceof Error ? error.message : 'Failed to fetch Git branches',
            isLoadingBranches: false,
          });
          set({ directories: newDirectories });
        }
