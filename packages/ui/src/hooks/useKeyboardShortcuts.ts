import { reopenLastClosedSessionTab } from '@/lib/sessionTabs';
...
export const useKeyboardShortcuts = () => {
  ...
  useKeybinds({
    ...
    new_chat_worktree: () => {
      if (useProductModeStore.getState().mode === 'work') return false;
      useUIStore.getState().setSessionSwitcherOpen(false);
      if (!isVSCodeRuntime()) {
    ...
    toggle_terminal: () => {
      if (useUIStore.getState().isMobile || useProductModeStore.getState().mode === 'work') return false;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (
        ...
        if (!state.isMobile && effectiveDirectory) {
          ...
          productMode: useProductModeStore.getState().mode,
