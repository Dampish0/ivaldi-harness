...
type RouteSessionStatus = 'none' | 'known' | 'missing' | 'unavailable';

const findGlobalSession = (sessionId: string) => {
  const global = useGlobalSessionsStore.getState();
  return [...global.activeSessions, ...global.archivedSessions].find((session) => session.id === sessionId) ?? null;
};

const resolveRouteSession = async (sessionId: string): Promise<{
  status: Exclude<RouteSessionStatus, 'none'>;
  directory: string | null;
}> => {
  const sessionState = useSessionUIStore.getState();
  const resolvedDirectory = sessionState.getDirectoryForSession(sessionId);
  const syncedSession = getAllSyncSessions().find((session) => session.id === sessionId) ?? null;
  if (syncedSession) {
    return { status: 'known', directory: resolvedDirectory };
  }

  let globalSession = findGlobalSession(sessionId);
  if (globalSession) {
    return {
      status: 'known',
      directory: resolvedDirectory ?? resolveGlobalSessionDirectory(globalSession),
    };
  }

  if (useGlobalSessionsStore.getState().status !== 'ready') {
    await ensureGlobalSessionsLoaded(getAllSyncSessions());
    if (useGlobalSessionsStore.getState().status !== 'ready') {
      return { status: 'unavailable', directory: null };
    }
    globalSession = findGlobalSession(sessionId);
  }

  if (!globalSession) {
    return { status: 'missing', directory: null };
  }
...
export function useRouter(): void {
  ...
  const applyRoute = React.useCallback(
    async (route: RouteState) => {
      if (isApplyingRouteRef.current) {
        return { sessionRouteStatus: 'unavailable' as RouteSessionStatus };
      }

      isApplyingRouteRef.current = true;
      let sessionRouteStatus: RouteSessionStatus = 'none';

      try {
        // 1. Apply session first (may trigger async operations)
        if (route.sessionId) {
          const routeSession = await resolveRouteSession(route.sessionId);
          sessionRouteStatus = routeSession.status;
          if (routeSession.status === 'known') {
            const currentSessionId = useSessionUIStore.getState().currentSessionId;
            if (route.sessionId !== currentSessionId) {
              setCurrentSession(route.sessionId, routeSession.directory);
            }
          }
        }
        ...
        if (route.tab && route.tab !== 'chat') {
          ...
          if (directory) {
            ...
            useUIStore.getState().openContextPanelTab(directory, { mode: targetMode });
          }
        }

        // 4. Apply diff file (only if going to diff tab)
        if (useProductModeStore.getState().mode === 'developer' && route.diffFile && (route.tab === 'diff' || !route.tab)) {
  ...
  React.useEffect(() => {
    ...
    const initializeRoute = async () => {
      ...
      if (!isVSCode && !isEmbeddedChat) {
        updateBrowserURL({
          ...getCurrentAppState(),
          sessionId: route.sessionId && result.sessionRouteStatus !== 'missing'
            ? route.sessionId
  ...
  React.useEffect(() => {
    ...
    const handlePopState = () => {
      ...
      if (hasRouteParams()) {
        void applyRoute(route).then((result) => {
          if (result.sessionRouteStatus === 'missing') {
            updateBrowserURL(getCurrentAppState(), { replace: true, force: true });
          }
        });
      } else {
  ...
  }, [applyRoute, getCurrentAppState, isVSCode, isEmbeddedChat, setSettingsDialogOpen]);
}
