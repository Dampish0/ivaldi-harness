...
describe("VS Code permission auto-accept runtime", () => {
  test("Auto accepts routine permissions and leaves risky permissions pending", async () => {
    const replied: string[] = []
    const decisions: Array<{ permissionID: string; effect: string; reasonCode: string }> = []
    const runtime = createVSCodePermissionAutoAcceptRuntime({
      getPolicy: () => ({ child: "auto" }),
      getSessions: () => new Map(),
    ...
    expect(await runtime.processPermission(routine, "/repo")).toBe(true)
    expect(await runtime.processPermission(risky, "/repo")).toBe(false)
    expect(replied).toEqual(["routine"])
    expect(decisions.map(({ permissionID, effect, reasonCode }) => ({ permissionID, effect, reasonCode }))).toEqual([
      { permissionID: "routine", effect: "allow", reasonCode: "auto.allow.git-read" },
      { permissionID: "risky", effect: "ask", reasonCode: "auto.ask.protected-git" },
    ])
  })
