...
import { classifyAutoPermission, type AutoPermissionDecision } from "@/lib/permissionAutoPolicy"
...
type Dependencies = {
  getPolicy: () => Record<string, PermissionMode | boolean>
...
export function createVSCodePermissionAutoAcceptRuntime(dependencies: Dependencies) {
  ...
  const getMode = async (sessionId: string, directory?: string): Promise<PermissionMode> => {
    ...
    while (current && !seen.has(current)) {
      if (Object.prototype.hasOwnProperty.call(policy, current)) {
        const configured = policy[current]
        if (configured === true) return 'full-access'
        if (configured === false) return 'manual'
        return configured
      }
      seen.add(current)
  ...
  const evaluate = async (permission: PermissionRequest, directory?: string): Promise<AutoPermissionDecision & { mode: PermissionMode }> => {
    const mode = await getMode(permission.sessionID, directory)
    if (mode === 'full-access') {
      return { mode, effect: 'allow', reasonCode: 'full-access.allow', risk: 'routine' }
    }
    if (mode === 'auto') return { mode, ...classifyAutoPermission({ ...permission, directory }) }
    return { mode: 'manual', effect: 'ask', reasonCode: 'manual.ask', risk: 'unknown' }
  }
  ...
  const processPermission = (
    ...
    const task = (async () => {
      const evaluation = await evaluate(permission, directory)
  ...
  return { processPermission, reconcilePending, evaluate }
}

const runtime = createVSCodePermissionAutoAcceptRuntime({
  getPolicy: () => usePermissionStore.getState().modes,
  ...
  recordDecision: (decision) => {
    window.dispatchEvent(new CustomEvent('openchamber:permission-auto-decision', { detail: decision }))
  },
