...
const buildPermission = (id: string, sessionID: string): PermissionRequest => ({
  id,
  sessionID,
  permission: 'edit',
  patterns: [],
...
const buildQuestion = (id: string, sessionID: string): QuestionRequest => ({
  id,
  sessionID,
  questions: [
    {
...
describe('ChildStoreManager.subscribeAllSelected', () => {
  ...
  test('notifies after the child-store registry changes without publishing synchronously', async () => {
    const manager = new ChildStoreManager();
    let notifications = 0;
    const unsubscribe = manager.subscribeAllSelected((state) => state.session, () => {
      notifications += 1;
...
describe('ChildStoreManager permission subscriptions', () => {
  test('does not notify session permission listeners for unrelated high-frequency updates', () => {
    ...
    child.setState({ permission: { 'session-17': [buildPermission('permission-1', 'session-17')] } });
...
describe('ChildStoreManager question subscriptions', () => {
  test('notifies only the owning session and ignores unrelated high-frequency updates', () => {
    ...
    child.setState({ question: { 'session-17': [buildQuestion('question-1', 'session-17')] } });
    ...
    child.setState({ question: { ...child.getState().question, 'session-18': [buildQuestion('question-2', 'session-18')] } });
  ...
  test('notifies subtree and exact-session rows once for each relevant replacement', () => {
    ...
    const parentQuestions = [buildQuestion('question-parent', 'parent')];
    const childQuestions = [buildQuestion('question-child', 'child')];
    ...
    child.setState({
      question: {
        ...
        child: [buildQuestion('question-child-replacement', 'child')],
      },
    });
  ...
  test('aggregates exact question buckets across directory stores', () => {
    ...
    childStore.setState({ question: { child: [buildQuestion('child-question', 'child')] } });
    ...
    childStore.setState({
      question: {
        ...
        unrelated: [buildQuestion('unrelated-question', 'unrelated')],
      },
    });
    expect(questionCount()).toBe(1);
    expect(notifications).toBe(1);

    parentStore.setState({ question: { parent: [buildQuestion('parent-question', 'parent')] } });
