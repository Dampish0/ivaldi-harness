...
describe('sendMessage draft snapshot (issues #2222 / #2315)', () => {
  ...
  beforeEach(() => {
    ...
    globalThis.fetch = async (input, init) => {
      ...
      if (url.includes('/api/fs/home')) {
        return Response.json({ home: '/home/test', dataDirectory: '/home/test/.config/ivaldi' });
      }
      if (url.includes('/api/permission-policy/sessions/session-materialized')) {
        const body = JSON.parse(String(init?.body ?? '{}'));
        const mode = body.mode ?? 'manual';
        return new Response(JSON.stringify({
          modes: { 'session-materialized': mode },
          sessions: { 'session-materialized': mode === 'full-access' },
          revision: 1,
