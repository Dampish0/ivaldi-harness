...
export type SessionUIState = {
  ...
  openNewSessionDraft: (options?: Partial<NewSessionDraftState> & {
    automatic?: boolean
...
export async function materializeOpenDraftSession(selection: {
  ...
  await usePermissionStore.getState().setSessionMode(created.id, draftPermissionMode)
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  createSessionFromAssistantMessage: async (sourceMessageId, execution) => {
    ...
    if (execution.createWorktree) {
      ...
      if (!project?.path) {
        throw new Error("Project is not registered in Ivaldi")
      }
