...
let sessionAbortResult: { data?: unknown; error?: unknown; response?: { status?: number } } = { data: true }
...
describe("abortCurrentOperation", () => {
  ...
  test("targets the session's authoritative directory and reports confirmation", async () => {
    const store = createStore({}, {
      session: [{ id: "session-a", directory: "/target/project" } as Session],
    })
    const { abortCurrentOperation, setActionRefs } = await import("./session-actions")
    setActionRefs(mockSdk as unknown as OpencodeClient, createChildStores([["/target/project", store]]), () => "/other/project")

    expect(await abortCurrentOperation("session-a")).toBe(true)
  ...
  test("does not report success when the server declines the abort", async () => {
    const store = createStore({}, {
      session: [{ id: "session-a", directory: "/target/project" } as Session],
    })
    const { abortCurrentOperation, setActionRefs } = await import("./session-actions")
    ...
    expect(await abortCurrentOperation("session-a")).toBe(false)
  })
})
