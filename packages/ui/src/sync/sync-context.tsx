...
import { permissionPolicyBroadcastEventSchema } from "@/lib/permissionModes"
import {
...
export function handleEvent(
  ...
  const permissionPolicyEvent = permissionPolicyBroadcastEventSchema.safeParse(payload)
  if (permissionPolicyEvent.success) {
    usePermissionStore.getState().applySnapshot(permissionPolicyEvent.data.properties, expectedRuntimeKey)
    return
  }
...
export function useDirectoryStore(
  ...
  const priority = options?.priority ?? "selected"
  const reason = options?.reason ?? "action-demand"
  // Store acquisition is allowed during render, but starting bootstrap is not:
  // bootstrap state is observed by useSyncExternalStore consumers elsewhere in
  // the tree. Queue it from the committed effect instead of synchronously
  // notifying those consumers while another component is rendering.
  const store = system.childStores.ensureChild(dir, { bootstrap: false })

  useEffect(() => {
    system.childStores.pin(dir)
    if (shouldBootstrap && store.getState().status === "loading") {
      system.childStores.requestBootstrap({
        directory: dir,
        priority,
        reason,
      })
  ...
  }, [dir, priority, reason, shouldBootstrap, store, system.childStores])
