...
mock.module('@/lib/runtime-fetch', () => ({
  runtimeFetch: mock(async (_path: string, init?: RequestInit) => {
    if (_path === '/api/fs/home') {
      return Response.json({ home: '/Users/tester', dataDirectory: '/Users/tester/.config/ivaldi' });
    }
...
describe('chat directories', () => {
  ...
  test('creates one isolated directory beneath the dated chats root', async () => {
    ...
    expect(directory.startsWith('/Users/tester/.config/ivaldi/chats/2026-08-21/session-')).toBe(true);
  ...
  test('deletes managed chat directories but leaves project directories alone', async () => {
    await deleteChatDirectory('/Users/tester/.config/ivaldi/chats/2026-08-21/session-a');
    await deleteChatDirectory('/Users/tester/project');
    expect(deletedDirectories).toEqual(['/Users/tester/.config/ivaldi/chats/2026-08-21/session-a']);
  });
});
