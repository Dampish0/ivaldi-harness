...
export const BUILTIN_STARTERS: readonly BuiltInStarter[] = [
    ...
    { name: 'research-topic', icon: 'search-eye', labelKey: 'chat.draftPresets.researchTopic.label', command: 'Research this topic thoroughly. Compare reliable sources, explain what matters, and cite the evidence.' },
    { name: 'draft-document', icon: 'file-text', labelKey: 'chat.draftPresets.draftDocument.label', command: 'Draft a clear, polished document for this project. Use the available context and ask only for details that are genuinely missing.' },
    { name: 'analyze-files', icon: 'folders', labelKey: 'chat.draftPresets.analyzeFiles.label', command: 'Analyze the relevant files in this project and explain the important findings, risks, and next actions.' },
    { name: 'project-plan', icon: 'survey', labelKey: 'chat.draftPresets.projectPlan.label', command: 'Create a practical project plan with goals, milestones, dependencies, responsibilities, risks, and concrete next actions.' },
    { name: 'summarize-project', icon: 'booklet', labelKey: 'chat.draftPresets.summarizeProject.label', command: 'Summarize this project for someone joining it now: purpose, current state, important context, open questions, and next actions.' },
];
...
// Fresh installs start with a short, high-signal row. Every built-in remains
// available from the add picker, and persisted/customized starter lists are
// never rewritten to this subset.
const DEFAULT_GLOBAL_STARTER_NAMES = ['explore', 'catch-up', 'debug', 'review'] as const;

export const DEFAULT_GLOBAL_STARTERS: readonly DraftStarterRef[] = DEFAULT_GLOBAL_STARTER_NAMES.map((name) => ({
    type: 'command' as const,
    name,
}));

const DEFAULT_WORK_GLOBAL_STARTER_NAMES = [
    'research-topic',
    'draft-document',
    'analyze-files',
    'project-plan',
...
export const DEFAULT_WORK_GLOBAL_STARTERS: readonly DraftStarterRef[] = DEFAULT_WORK_GLOBAL_STARTER_NAMES.map((name) => ({
    type: 'command' as const,
    name,
}));
