...
export const reopenLastClosedSessionTab = (): boolean => {
  const sessionId = useSessionTabsStore.getState().reopenLastClosedTab();
  if (!sessionId) return false;

  const session = useGlobalSessionsStore.getState().activeSessions.find((candidate) => candidate.id === sessionId);
  if (session) {
    useSessionUIStore.getState().setCurrentSession(session.id, resolveGlobalSessionDirectory(session));
  }
  return true;
