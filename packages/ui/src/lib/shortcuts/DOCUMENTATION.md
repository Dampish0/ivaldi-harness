...
Ivaldi retains `reopen_session_tab`, defaulting to `alt+shift+w` alongside `alt+w` for closing a tab. The old `mod+shift+t` default collides with the thinking-level action. Work mode filters developer-only actions and does not dispatch terminal or worktree creation commands.
