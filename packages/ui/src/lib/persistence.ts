...
const applyDesktopUiPreferences = (settings: DesktopSettings) => {
  ...
  if (Array.isArray(settings.draftStarters)) {
    ...
    // No customized starter list exists yet. Mark the historical one-time
    // additions as handled without expanding the intentionally short fresh
    // default; both commands remain available from the starter picker.
