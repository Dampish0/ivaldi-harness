...
export const DEFAULT_LIGHT_THEME_ID = 'ivaldi-light' as const;
export const DEFAULT_DARK_THEME_ID = 'ivaldi-dark' as const;
...
export const themes: Theme[] = [
  ...
  flexokiDarkTheme,
  ...presetThemes.filter(
    (theme) => theme.metadata.id !== 'ivaldi-light' && theme.metadata.id !== 'ivaldi-dark',
  ),
];
...
export function getThemeById(id: string): Theme | undefined {
  ...
  const resolvedId =
    ...
    id === 'openchamber-light' ? 'ivaldi-light' :
    id === 'openchamber-dark' ? 'ivaldi-dark' :
    id;
