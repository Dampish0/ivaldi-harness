...
/**
 * Ivaldi project-level configuration service.
 * Stores per-project settings in the runtime data directory under projects/<projectId>.json.
...
const getUserProjectsDirectory = async (): Promise<string> => {
  return joinPath(await getRuntimeDataDirectory(), 'projects');
};
