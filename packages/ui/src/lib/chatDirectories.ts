import { getRuntimeDataDirectory } from './runtimeDataDirectory';
...
const IVALDI_CHATS_PATH_SEGMENT = '/.config/ivaldi/chats/';
...
export function isChatDirectoryForHome(directory: string | null | undefined, home: string | null | undefined): boolean {
  ...
  if (normalized?.includes(MANAGED_CHATS_PATH_SEGMENT) || normalized?.includes(IVALDI_CHATS_PATH_SEGMENT)) return true;
...
export function isChatDirectoryPath(directory: string | null | undefined): boolean {
  const normalized = normalizePath(directory ?? null);
  return normalized?.includes(MANAGED_CHATS_PATH_SEGMENT) === true || normalized?.includes(IVALDI_CHATS_PATH_SEGMENT) === true;
}

export function getChatsRootFromDirectory(directory: string | null | undefined): string | null {
  const normalized = normalizePath(directory ?? null);
  const segment = normalized?.includes(IVALDI_CHATS_PATH_SEGMENT) ? IVALDI_CHATS_PATH_SEGMENT : MANAGED_CHATS_PATH_SEGMENT;
  const index = normalized?.indexOf(segment) ?? -1;
  return normalized && index >= 0
    ? normalized.slice(0, index + segment.length - 1)
    : null;
}
...
export async function getChatsRootDirectory(): Promise<string> {
  const runtimeKey = getRuntimeKey();
  const existing = chatsRootByRuntime.get(runtimeKey);
  if (existing) return existing;

  const pending = getRuntimeDataDirectory().then((directory) => {
    return joinPath(directory, 'chats');
  }).catch((error) => {
