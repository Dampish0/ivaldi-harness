...
const WORK_MODE_HIDDEN_SETTINGS_SEARCH_ITEMS = new Set([
  'appearance.terminal-font-size',
  'appearance.terminal-shell',
  'appearance.terminal-quick-keys',
  'chat.render-mode',
  'chat.message-transport',
  'chat.session-recap',
  'chat.session-assistance',
  'chat.session-suggestion',
  'chat.session-goal',
  'chat.session-goal-budget',
  'chat.code-block-line-wrap',
  'chat.inline-assistant-actions',
  'chat.draft-starters-visible',
  'chat.subagent-read-only-banner',
  'chat.tool-file-icons',
  'chat.tools-and-files',
  'chat.changed-files',
  'chat.dotfiles',
  'chat.follow-up-behavior',
  'appearance.file-editor-keymap',
  'appearance.session-tabs',
  'sessions.deletion-dialog',
  'sessions.small-model',
  'sessions.walkthrough-model',
  'sessions.opencode-binary',
  'sessions.opencode-update-notifications',
  'sessions.agent-control-tool',
  'sessions.agent-web-tool',
  'sessions.agent-memory-tool',
]);

const WORK_MODE_SETTINGS_SEARCH_PAGE_OVERRIDES = new Map<string, SettingsPageSlug>([
  ['sessions.auto-cleanup', 'chat'],
  ['sessions.retention-period', 'chat'],
  ['sessions.retention-action', 'chat'],
]);
...
const SETTINGS_SEARCH_ITEMS: readonly SettingsSearchItem[] = [
  ...
  },
  {
    ...
    page: 'appearance',
    titleKey: 'settings.openchamber.visual.field.interfaceFont',
  ...
  {
    id: 'remote-instances.header-switcher',
    page: 'remote-instances',
    titleKey: 'settings.remoteInstances.headerSwitcher.label',
    descriptionKey: 'settings.remoteInstances.headerSwitcher.info',
    keywords: ['header', 'current instance', 'instance switcher', 'toolbar'],
    isAvailable: (ctx) => ctx.isDesktop,
  },
  {
    id: 'remote-instances.switch-instance',
    page: 'remote-instances',
    titleKey: 'desktopHostSwitcher.actions.switchInstance',
    descriptionKey: 'desktopHostSwitcher.description',
    keywords: ['current instance', 'local instance', 'remote instance', 'host switcher'],
    isAvailable: (ctx) => ctx.isDesktop,
  },
  {
  ...
  {
    id: 'lifecycle-hooks.list',
    page: 'lifecycle-hooks',
    titleKey: 'settings.page.lifecycleHooks.title',
    descriptionKey: 'settings.page.lifecycleHooks.description',
    keywords: ['lifecycle', 'hooks', 'automation', 'preflight', 'policy', 'prompt submit'],
    isAvailable: (ctx) => !ctx.isVSCode,
  },
  {
    id: 'lifecycle-hooks.recent',
    page: 'lifecycle-hooks',
    titleKey: 'settings.lifecycleHooks.recent.title',
    descriptionKey: 'settings.lifecycleHooks.recent.info',
    keywords: ['hook output', 'history', 'test', 'stderr', 'stdout'],
    isAvailable: (ctx) => !ctx.isVSCode,
  },
  {
    id: 'mcp.create',
  ...
  },
  {
    ...
    keywords: ['claude', 'anthropic', 'claude code', 'pro', 'max', 'agent sdk', '@ivaldi/opencode-claude'],
  },
  {
    ...
    keywords: ['cursor', 'oauth', 'subscription', 'openai compatible', '@ivaldi/opencode-cursor'],
  },
] as const;
...
export function buildSettingsSearchResults({
  ...
  return SETTINGS_SEARCH_ITEMS.flatMap((item) => {
    const page = runtimeCtx.productMode === 'work'
      ? WORK_MODE_SETTINGS_SEARCH_PAGE_OVERRIDES.get(item.id) ?? item.page
      : item.page;
    ...
    if (runtimeCtx.productMode === 'work' && WORK_MODE_HIDDEN_SETTINGS_SEARCH_ITEMS.has(item.id)) {
      return [];
    }

    const pageMeta = getSettingsPageMeta(page);
