...
import { formatSessionCompactDateLabel, resolveWorkChatTitle } from '@/components/session/sidebar/utils';
...
const SwitcherRow: React.FC<{
  ...
  active: boolean;
  onSelect: () => void;
}> = ({ session, meta, untitledLabel, active, onSelect }) => {
  ...
  return (
    <button
      type="button"
      className={cn(
        'flex w-full items-center gap-3 rounded-md px-2 py-2 text-left transition-colors active:bg-interactive-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--interactive-focus-ring)]',
    ...
    >
      <span className="flex min-w-0 flex-1 flex-col">
        <span className="block truncate typography-ui-label text-foreground">
          {isWorkMode
            ? resolveWorkChatTitle(session.title, untitledLabel)
      ...
      {isStreaming || showUnreadDot ? (
        <span
          className="size-1.5 shrink-0 rounded-full bg-[var(--status-info)]"
          aria-hidden
        />
      ) : null}
  ...
  return (
    <div ref={wrapperRef} className="fixed inset-x-0 bottom-0 top-[calc(var(--oc-safe-area-top,0px)+var(--oc-header-height,56px))] z-20 pointer-events-none">
      <div
        ...
        className={cn(
          'flex flex-col overflow-hidden rounded-md border border-border/50 bg-[var(--surface-elevated)] p-1.5 will-change-transform',
          isPopover ? 'absolute origin-top-left' : 'mx-3 mt-1.5',
        ...
        <div className="oc-hide-scrollbar min-h-0 flex-1 space-y-0.5 overflow-y-auto overscroll-contain">
          {items.length === 0 ? (
            <p className="px-3 py-6 text-center typography-small text-muted-foreground">
              {t(isWorkMode ? 'chat.work.noRecentChats' : 'sessions.switcher.empty')}
            </p>
          ) : (
            items.map((item) => {
              const session = item.node.session;
              const meta = [
                item.secondaryMeta?.projectLabel,
                isWorkMode ? null : item.secondaryMeta?.branchLabel,
              ]
                .filter(Boolean)
                .join(' · ');
              return (
                <SwitcherRow
                  ...
                  session={session}
                  meta={meta}
                  untitledLabel={t(isWorkMode ? 'chat.work.untitledChat' : 'sessions.sidebar.session.untitled')}
