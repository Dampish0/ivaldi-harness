...
export const MobileConnectionWelcome: React.FC<{
  ...
  return (
    ...
    <main className="oc-keyboard-fill-screen flex min-h-dvh flex-col overflow-y-auto bg-background px-6 pb-[calc(var(--safe-area-inset-bottom,env(safe-area-inset-bottom,0px))+28px)] pt-[calc(var(--safe-area-inset-top,env(safe-area-inset-top,0px))+28px)] text-foreground">
      <div className="m-auto flex w-full max-w-[360px] shrink-0 flex-col items-center gap-7 py-8">
        <div className="flex flex-col items-center gap-4 text-center">
          <span {...debugLongPress} className="select-none" style={{ touchAction: 'manipulation' }}>
            <OpenChamberLogo width={56} height={56} className="size-14" />
          </span>
        ...
        {notice ? (
          <div
            role="status"
            className="flex w-full items-center gap-3 rounded-lg border border-[color-mix(in_srgb,var(--status-warning)_35%,transparent)] bg-[color-mix(in_srgb,var(--status-warning)_10%,transparent)] px-3 py-2.5"
          >
        ...
        {pendingConnection ? (
          <form className="flex w-full flex-col gap-3" onSubmit={handlePasswordSubmit}>
            <div className="flex items-center gap-3 border-y border-border/60 px-1 py-3">
          ...
          <div className="flex w-full flex-col gap-6">
            ...
            {connections.length > 0 ? (
              <section className="flex w-full flex-col gap-2.5">
                <h2 className="px-1 text-left typography-meta text-muted-foreground">
                  ...
                  {connections.map((connection) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        disabled={isBusy}
                        className="flex min-h-12 w-full items-center gap-2.5 border-b border-border/60 px-1 py-2.5 text-left last:border-b-0 hover:bg-interactive-hover/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--interactive-focus-ring)] disabled:opacity-70"
                        onClick={() => {
