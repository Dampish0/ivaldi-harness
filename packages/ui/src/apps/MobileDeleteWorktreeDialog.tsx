...
export const MobileDeleteWorktreeDialog: React.FC<MobileDeleteWorktreeDialogProps> = ({
  ...
  const toggle = (checked: boolean, onChange: (value: boolean) => void, label: string, disabled?: boolean) => (
    <label
      className={cn(
        'flex min-h-11 w-full items-center justify-between gap-3 border-b border-border/50 px-1 py-2.5 text-left last:border-b-0',
        disabled && 'opacity-40',
      )}
    >
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 px-1 py-1">
        ...
        {isDirty ? (
          <p className="rounded-md border border-[color-mix(in_srgb,var(--status-warning)_40%,transparent)] bg-[color-mix(in_srgb,var(--status-warning)_10%,transparent)] px-3 py-2.5 typography-meta text-foreground">
