...
export const MobileChangesSurface: React.FC<MobileChangesSurfaceProps> = ({ onClose, initialDiffPath, initialDiffStaged = false, active = true }) => {
  ...
  const renderListState = (state: React.ReactNode) => (
    <div className="flex h-full flex-col overflow-hidden bg-background text-foreground">
      <header className="flex h-[var(--oc-header-height,56px)] shrink-0 items-center gap-2 border-b border-border/60 px-3 text-foreground">
        {onClose ? (
          <Button
            type="button"
            variant="ghost"
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background text-foreground">
      <header
        className={cn(
          'flex shrink-0 items-center gap-2 border-b border-border/60 px-3 text-foreground',
          onClose ? 'h-[var(--oc-header-height,56px)]' : 'h-11',
        )}
      >
        {onClose ? (
          <Button
        ...
        <div className="min-w-0 flex-1 px-1">
          {onClose ? (
            ...
            <div className="flex min-w-0 items-center gap-1.5 text-muted-foreground">
              ...
              <span className="truncate typography-meta">{status?.current || currentDirectory}</span>
            </div>
          )}
        </div>
        <SyncActions
