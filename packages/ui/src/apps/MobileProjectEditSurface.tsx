...
const SortableWorktreeRow: React.FC<{
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'flex min-h-10 items-center gap-1 border-b border-border/60 px-1 py-1 transition-colors last:border-b-0',
        isDragging && 'bg-[var(--surface-elevated)] shadow-md',
      )}
    >
      <Button
        type="button"
        variant="ghost"
        size="icon"
        className="size-8 shrink-0 cursor-grab touch-none rounded-md text-muted-foreground/70 active:cursor-grabbing"
      ...
      <Button
        ...
        variant="ghost"
        size="icon"
        className="size-8 shrink-0 rounded-md text-muted-foreground hover:text-[var(--status-error)]"
...
export const MobileProjectEditSurface: React.FC<MobileProjectEditSurfaceProps> = ({
  ...
  React.useEffect(() => {
    if (!isDeveloperMode) setWorktreeToDelete(null);
  }, [isDeveloperMode]);
  ...
  return (
    <>
      <MobileFullscreenSurface
        open={open}
        onClose={onClose}
        variant={isTabletLayout ? 'dialog' : 'fullscreen'}
        dialogAlign="app"
      ...
      >
        {project ? (
          <div className="h-full overflow-y-auto px-4 pb-8">
            ...
            {/* Color */}
            <div className="flex flex-col gap-3 border-t border-border/60 py-5">
              <label className="typography-ui-label font-medium text-foreground">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className={cn(
                    'flex size-8 items-center justify-center rounded-md border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]',
                    color === null
                      ? 'border-foreground bg-interactive-selection'
                ...
                {PROJECT_COLORS.map((c) => (
                  <button
                    ...
                    title={c.label}
                    className={cn(
                      'size-8 rounded-md border-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]',
            ...
            <div className="flex flex-col gap-3 border-t border-border/60 py-5">
              ...
              <div className="flex flex-wrap gap-1">
                <Button
                  ...
                  aria-label={t('projectEditDialog.option.none')}
                  className={cn(
                    'size-8 rounded-md border border-transparent text-muted-foreground',
                    icon === null && 'border-foreground bg-interactive-selection text-foreground hover:bg-interactive-selection',
                  )}
                  style={{ touchAction: 'manipulation' }}
                >
                  <span className="h-0.5 w-4 rotate-45 rounded-full bg-muted-foreground/40" />
                ...
                {PROJECT_ICONS.map((i) => (
                  <Button
                    ...
                    className={cn(
                      'size-8 rounded-md border border-transparent text-muted-foreground',
                      icon === i.key && 'border-foreground bg-interactive-selection text-foreground hover:bg-interactive-selection',
                    )}
                    style={{ touchAction: 'manipulation' }}
                  >
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                {hasImageIcon ? (
                  <span
                    className="mr-1 flex size-8 items-center justify-center overflow-hidden rounded-md border border-border/60 bg-[var(--surface-elevated)]"
                  ...
                  >
                    <ProjectIconImage
                      ...
                      fallback={<Icon name="folder-6" className="size-4 text-muted-foreground" />}
                    />
                  </span>
                ) : previewIconName ? (
                  <span className="mr-1 flex size-8 items-center justify-center text-muted-foreground">
                    <Icon name={previewIconName} className="size-4" style={currentColorVar ? { color: currentColorVar } : undefined} />
                  </span>
                ) : null}
            ...
            {isDeveloperMode && project.isGitRepo ? (
              <div className="flex flex-col gap-2 border-t border-border/60 py-5">
                <label className="typography-ui-label font-medium text-foreground">
