...
const MobileProjectIcon: React.FC<{
  ...
  const containerClasses = size === 'sm' ? 'size-4' : 'size-[18px]';
  ...
  const fallbackIcon = ProjectIcon ? (
    ...
    <Icon name="folder-6" className={innerClasses} style={iconColor ? { color: iconColor } : undefined} />
  );

  return (
    <span
      className={cn('flex shrink-0 items-center justify-center text-muted-foreground', containerClasses)}
    >
      {project.iconImage ? (
        <span
          className="flex size-full items-center justify-center overflow-hidden rounded-[3px]"
...
const SessionRow: React.FC<{
  ...
  const title = isWorkMode
    ? resolveWorkChatTitle(session.title, t('chat.work.untitledChat'))
    : session.title?.trim() || t('mobile.sessions.untitled');
    ...
    >
      {swipeEnabled ? (
        ...
        >
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center text-muted-foreground transition-colors active:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--interactive-focus-ring)]"
            aria-label={t('mobile.sessions.renameSessionAria', { title })}
            onClick={onRequestRename}
            style={{ touchAction: 'manipulation' }}
          >
            <Icon name="edit-2" className="size-[18px]" />
          </button>
          <button
            type="button"
            tabIndex={revealed ? 0 : -1}
      ...
      >
        ...
        {isStreaming || showUnreadDot || (hasChildren && onToggleChildren) ? (
          <button
            type="button"
            className="absolute z-10 flex w-6 items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
...
const ShowMoreRow: React.FC<{
  ...
  return (
    <button
      type="button"
      className="flex min-h-9 w-full items-center gap-2 py-1 pr-3 text-left text-muted-foreground transition-colors hover:bg-interactive-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] focus-visible:ring-inset"
      style={{ paddingLeft: indent, touchAction: 'manipulation' }}
      onClick={onClick}
    >
      <Icon name="arrow-down-s" className="size-4" />
      <span className="typography-micro">
        {t(isDeveloperMode ? 'sessions.sidebar.group.showMore' : 'chat.work.showMore')}
      </span>
    </button>
  );
};
...
const ShowFewerRow: React.FC<{
  ...
  return (
    ...
    >
      ...
      <span className="typography-micro">
        {t(isDeveloperMode ? 'sessions.sidebar.group.showFewer' : 'chat.work.showFewer')}
      </span>
    </button>
  );
};
...
const SortableWorktreeReorderRow: React.FC<{ worktree: WorktreeMetadata }> = ({ worktree }) => {
  ...
  return (
    <div
      ...
      className={cn(
        'flex items-center gap-1 bg-background px-1 py-1',
        isDragging && 'shadow-md',
      )}
    >
      <button
        type="button"
        className="flex size-8 shrink-0 cursor-grab touch-none items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-interactive-hover hover:text-foreground active:cursor-grabbing focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
...
const SortableProjectRow: React.FC<{
  ...
  return (
    <div
      ...
      style={{ transform: CSS.Translate.toString(transform), transition, zIndex: isDragging ? 10 : 1 }}
      className={cn(
        'border-y border-border/60 bg-background py-1 transition-colors',
        isDragging && 'shadow-md',
      )}
    >
      <div className="flex items-center gap-1">
        <button
          type="button"
          className="flex size-9 shrink-0 cursor-grab touch-none items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-interactive-hover hover:text-foreground active:cursor-grabbing focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
        ...
        <button
          type="button"
          className="flex min-h-9 min-w-0 flex-1 items-center gap-2 rounded-md px-1 text-left transition-colors hover:bg-interactive-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--interactive-focus-ring)]"
...
export const MobileSessionsSheet: React.FC<MobileSessionsSheetProps> = ({ open, onOpenChange, variant = 'drawer', footer }) => {
  ...
  }, [git, isDeveloperMode, open, projects, worktreeRefreshKey]);
  ...
  const handleSubmitRename = async (sessionId: string, title: string) => {
    ...
    try {
      await updateSessionTitle(sessionId, title);
        ...
        toast.error(t(isDeveloperMode ? 'mobile.sessions.renameError' : 'chat.work.mobile.renameError'));
    }
  };
  ...
  const editToggle = canEditOrder ? (
    <Button
      ...
      style={{ touchAction: 'manipulation' }}
    >
      <Icon name={editingOrder ? 'check' : 'edit-2'} className="size-[18px]" />
    </Button>
  ) : null;
  ...
  const addProjectButton = !editingOrder ? (
    <Button
      ...
      style={{ touchAction: 'manipulation' }}
    >
      <Icon name="folder-add" className="size-[18px]" />
    </Button>
  ) : null;
      ...
      <div ref={contentRootRef} className="flex min-h-0 flex-1 flex-col">
        <ScrollShadow className="min-h-0 flex-1 overflow-y-auto pb-4">
          ...
          <div className={cn('px-3 pb-2 pt-1', editingOrder && 'hidden')}>
            <div className="relative">
              ...
              {query ? (
                <button
                  type="button"
                  className="absolute right-1 top-1/2 flex size-8 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-interactive-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
          ...
          {projectsMeta.length === 0 ? (
            ...
            <div className="flex flex-col gap-3 px-3 pt-2">
              {searchSessionMatches.length > 0 ? (
                <section>
                  ...
                  <div className="overflow-hidden rounded-md bg-[var(--surface-elevated)]">
                    {searchSessionMatches.map((session, index) => (
                      <div key={session.id} className={cn(index > 0 && 'border-t border-border/40')}>
                        <SessionRow
                          session={session}
                          active={currentSessionId === session.id}
                          indent={12}
                ...
                <section>
                  ...
                  <div className="overflow-hidden rounded-md bg-[var(--surface-elevated)]">
                    {searchProjectMatches.map((project, index) => (
                      <div
                        key={project.id}
                        className={cn('flex items-center', index > 0 && 'border-t border-border/40')}
                      >
                        <button
                          type="button"
                          className="flex min-h-12 min-w-0 flex-1 items-center gap-3 px-3 py-1.5 text-left transition-colors hover:bg-interactive-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] focus-visible:ring-inset"
                        ...
                        {isDeveloperMode && project.isGitRepo ? (
            ...
            {t(isDeveloperMode ? 'mobile.sessions.editOrderHint' : 'chat.work.mobile.editOrderHint')}
              </p>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {projectsMeta.map((project) => {
                      ...
                      return (
                        <SortableProjectRow
                          key={project.id}
                          project={isDeveloperMode ? project : { ...project, worktrees: [] }}
                          totalSessions={node?.totalSessions ?? 0}
                          expanded={isDeveloperMode && reorderExpandedProjects.has(project.id)}
            ...
            <div className="flex flex-col">
              {orderedNodes.map((node, nodeIndex) => {
                ...
                const buckets = normalizedQuery
                  ...
                  : node.buckets;
                const activeWorktreePath = isDeveloperMode ? findActiveWorktreePath(node) : null;
                return (
                  <section
                    key={node.project.id}
                    className={cn(nodeIndex > 0 && 'border-t border-border/40')}
                  >
                    ...
                    >
                      <div data-active-project={node.isActive || undefined} className="flex min-h-12 w-full items-center">
                        <button
                          type="button"
                          className="flex min-h-12 min-w-0 flex-1 items-center gap-2 px-3 py-1.5 text-left transition-colors hover:bg-interactive-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] focus-visible:ring-inset"
                          onClick={() => {
                            if (revealedRowId) {
                              ...
                              {worktreeBuckets.map((bucket) => {
                                ...
                                return (
                                  <div key={bucket.key}>
                                    ...
                                    <button
                                      type="button"
                                      className="flex min-h-10 w-full items-center gap-2 px-3 py-1 text-left transition-colors hover:bg-interactive-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] focus-visible:ring-inset"
                                      onClick={() => {
                                        if (revealedRowId) {
                                          handleRowKeyRevealedChange(revealedRowId, false);
                                          return;
            ...
            <div className="flex shrink-0 items-center gap-1">
              {footer.onOpenUpdate ? (
                ...
                >
                  ...
                  <span className="absolute right-2 top-2 inline-flex size-2 rounded-full bg-[var(--status-info)]" aria-hidden />
                </Button>
              ) : null}
              <Button
                type="button"
        ...
        />
        {isDeveloperMode && worktreeToDelete ? (
  ...
  if (variant === 'sidebar') {
    ...
    return (
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex h-[var(--oc-header-height,56px)] shrink-0 items-center justify-between gap-2 border-b border-border/40 px-3">
