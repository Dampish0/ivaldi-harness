...
/** The fullscreen app-level surfaces, reachable from the sessions drawer
    ...
    closing returns to the chat. The sessions drawer and workspace drawer are
    separate layers. Developer exposes Changes / Files / Terminal / Notes / MCP;
...
const MobileShell: React.FC<{ onActiveConnectionDeleted: () => void }> = ({ onActiveConnectionDeleted }) => {
  ...
  const [workspaceTab, setWorkspaceTab] = React.useState<MobileWorkspaceTab>(() => (
    isDeveloperMode ? 'changes' : 'files'
  ));
  ...
  React.useEffect(() => {
    if (!isDeveloperMode && (workspaceTab === 'changes' || workspaceTab === 'terminal')) {
      setPendingChangesDiff(null);
      setWorkspaceTab('files');
    }
  }, [isDeveloperMode, workspaceTab]);
  ...
  const openChangesSurface = React.useCallback((diff: { path: string; staged: boolean } | null = null) => {
    ...
    setWorkspaceOpen(true);
  }, [isDeveloperMode, openFilesSurface]);
...
export function MobileApp({ apis }: MobileAppProps) {
  ...
  React.useEffect(() => {
    if (!isConnected || !isDeveloperMode || projects.length === 0) return;
    let cancelled = false;
  ...
  }, [isConnected, isDeveloperMode, projects]);
  ...
  if (!fontsReady) {
    return (
      <main className="flex min-h-dvh items-center justify-center bg-background text-foreground">
        <OpenChamberLogo width={64} height={64} isAnimated />
      </main>
    );
  }
      ...
      return (
        <main className="flex min-h-dvh items-center justify-center bg-background px-6 text-center text-foreground">
          <div className="flex max-w-sm flex-col items-center gap-4">
            <OpenChamberLogo width={64} height={64} isAnimated={!showConnectionRecovery} />
