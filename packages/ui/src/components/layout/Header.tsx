...
import { DesktopComputerUseStatus } from '@/components/desktop/DesktopComputerUseStatus';
...
import { buildLocalDesktopHost, getLocalDesktopOrigin, resolveCurrentDesktopHost } from '@/lib/desktopCurrentHost';
...
const DESKTOP_HEADER_ICON_BUTTON_CLASS = 'app-region-no-drag inline-flex h-8 w-8 items-center justify-center gap-2 rounded-md typography-ui-label font-medium text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] disabled:pointer-events-none disabled:opacity-50 hover:bg-interactive-hover transition-colors';
...
const DesktopCurrentInstanceControl = React.memo(function DesktopCurrentInstanceControl() {
  const { t } = useI18n();
  const [isOpen, setIsOpen] = React.useState(false);
  const [currentInstanceLabel, setCurrentInstanceLabel] = React.useState('');

  const refreshCurrentInstanceLabel = React.useCallback(async () => {
    try {
      const config = await desktopHostsGet();
      const localOrigin = getLocalDesktopOrigin();
      const current = resolveCurrentDesktopHost([
        buildLocalDesktopHost(localOrigin),
        ...config.hosts,
      ]);
      setCurrentInstanceLabel(
        redactSensitiveUrl(current.label.trim() || t('desktopHostSwitcher.instance.fallback')),
      );
    } catch {
  ...
  const label = currentInstanceLabel || t('desktopHostSwitcher.instance.local');
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            className={cn(
              DESKTOP_HEADER_ICON_BUTTON_CLASS,
              'w-auto max-w-[12rem] justify-start gap-1.5 px-2.5',
            )}
            onClick={() => setIsOpen(true)}
          >
            <Icon name="server" className="h-[18px] w-[18px] shrink-0" />
            <span className="truncate typography-ui-label font-medium text-foreground">{label}</span>
          </button>
        </TooltipTrigger>
        <TooltipContent>
          <p>{t('desktopHostSwitcher.actions.switchInstance')}</p>
        </TooltipContent>
      </Tooltip>
...
export const Header: React.FC = () => {
  ...
  const showCurrentInstanceInHeader = useUIStore((state) => state.showCurrentInstanceInHeader);
  ...
  const gitBranchForDirectory = useGitBranchLabel(isDeveloperMode ? openDirectory || null : null);
  ...
  // Work mode keeps the project identity in the header even while the status
  // panel is visible. That panel intentionally omits repository/project rows in
  // Work, so suppressing the header there leaves the active project unnamed.
  // Developer can still dedupe all project/repository metadata against the
  // richer status panel.
  const showWorkProjectLabel = !isDeveloperMode && Boolean(activeProjectLabel);
  const showDeveloperHeaderMeta = isDeveloperMode
    && !workStatusPanelVisible
    && Boolean(activeProjectLabel || currentBranchLabel || (!isNewSessionDraftOpen && worktreeBadgeKind));
  const showHeaderMetaRow = !isChatContext && (showWorkProjectLabel || showDeveloperHeaderMeta);
  ...
  const webWindowControlsOverlayStyle = React.useMemo<React.CSSProperties | undefined>(() => {
    ...
    return {
      ...
      // browser/PWA Window Controls Overlay needs the right safe-area inset.
      // Electron renders its own controls, so they should meet the frame edge.
      paddingRight: isDesktopApp && usesFramelessChrome
        ? '0px'
  ...
  const openRemoteInstancesSettings = React.useCallback(() => {
    ...
    ui.setSettingsPage('remote-instances');
    ui.setSettingsDialogOpen(true);
  }, []);

  useKeybind('toggle_services_menu', () => {
    if (!isDesktopApp) return false;
    openRemoteInstancesSettings();
  });
  const desktopSidebarActions = (
    <>
      {isDeveloperMode ? <OpenInAppButton directory={actionDirectory} className="mr-1" /> : null}
      {isDesktopApp && showCurrentInstanceInHeader ? <DesktopCurrentInstanceControl /> : null}
      {isDeveloperMode ? (
  ...
  const renderSessionTabMenu = React.useCallback(({ session, isActive, select, closeOtherTabs, components }: SessionTabMenuArgs) => {
    ...
    const canMoveToWorktree = isDeveloperMode && isActive && !isVSCode && !isChatContext && currentSession && !currentSession.parentId;
    return (
      <>
  ...
  }, [copySessionIdFor, copySessionShareUrl, currentSession, exportCurrentSession, isChatContext, isCurrentSessionActive, isCurrentSessionMovingToWorktree, isDeveloperMode, isVSCode, moveCurrentSessionToWorktree, sessionDirectory, shareSessionFor, t, unshareSessionFor]);

  const renderDesktop = () => (
    <div
      data-ivaldi-header="true"
      onMouseDown={handleDragStart}
      className={cn(
        'app-region-drag relative flex h-[var(--oc-header-height)] select-none items-center',
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 items-center">
            ...
            <div className="flex min-w-0 flex-col justify-center px-1">
              ...
              {showHeaderMetaRow ? (
                <span className="flex min-w-0 max-w-full items-center gap-1.5 truncate typography-micro text-[10.5px] font-normal leading-tight text-muted-foreground/75">
                  ...
                  {isDeveloperMode && !isNewSessionDraftOpen && worktreeBadgeKind ? (
                    <span className={cn(
              ...
              {currentSessionId && !isNewSessionDraftOpen && !isRenamingHeaderSession ? (
                ...
                >
                  ...
                  <DropdownMenuContent align="end" className="min-w-[190px]">
                    ...
                    {isDeveloperMode && !isVSCode && !isChatContext && currentSession && !currentSession.parentId ? (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="block">
                            <DropdownMenuItem
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <ContextPanelRail
            workStatusAction={!isVSCode && currentSessionId ? {
              active: workStatusToggleActive,
              onSelect: handleWorkStatusToggle,
              label: workStatusPanelEnabled && !workStatusPanelFits
