...
export const ProjectContextPanel: React.FC<{
  ...
  const [runtimeChatsRoot, setRuntimeChatsRoot] = React.useState<string | null>(null);
  React.useEffect(() => {
    let generation = 0;
    const refresh = () => {
      const request = ++generation;
      setRuntimeChatsRoot(null);
      void getChatsRootDirectory().then((root) => {
        if (request === generation) setRuntimeChatsRoot(root);
      }).catch(() => {});
    };
    refresh();
    const unsubscribe = subscribeRuntimeEndpointChanged(refresh);
    return () => { generation++; unsubscribe(); };
  }, []);
  const chatsRoot = getChatsRootFromDirectory(chatSessionDirectory) ?? runtimeChatsRoot;
