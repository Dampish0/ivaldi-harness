...
// Context surfaces stay on-demand. This component is mounted with the main
// shell even while the panel is closed, so a static surface import becomes
// cold-start work for every launch.
const PullRequestView = lazyWithChunkRecovery(() => import('@/components/views/PullRequestView').then((m) => ({ default: m.PullRequestView })));
...
const ProjectContextPanel = lazyWithChunkRecovery(() => import('./RightSidebarTabs').then((m) => ({ default: m.ProjectContextPanel })));
const SidebarFilesTree = lazyWithChunkRecovery(() => import('./SidebarFilesTree').then((m) => ({ default: m.SidebarFilesTree })));
const ContextPanelContent = lazyWithChunkRecovery(() => import('./ContextSidebarTab').then((m) => ({ default: m.ContextPanelContent })));
const BrowserPane = lazyWithChunkRecovery(() => import('@/components/browser/BrowserPane').then((m) => ({ default: m.BrowserPane })));
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-hidden={!visible}
      >
        <React.Suspense fallback={null}><SidebarFilesTree /></React.Suspense>
      </div>
    </div>
  );
};
...
export const ContextPanel: React.FC = () => {
  ...
  const chatTabs = React.useMemo(
    () => visibleTabs.filter((tab) => tab.mode === 'chat'),
    [visibleTabs],
  );
  ...
  React.useLayoutEffect(() => {
    const hasAnyChatTab = visibleTabs.some((tab) => tab.mode === 'chat');
    if (!hasAnyChatTab) {
      return;
    }
  ...
  }, [darkThemeId, lightThemeId, postChatSettingsSyncToEmbeddedChat, postEmbeddedVisibilityToChats, postThemeSyncToEmbeddedChat, themeMode, visibleTabs]);
  ...
  const activeModeTabs = React.useMemo(
    () => (activeTab ? visibleTabs.filter((tab) => tab.mode === activeTab.mode) : []),
  ...
  const tabItems = React.useMemo(() => activeModeTabs.map((tab) => {
    ...
    return {
      ...
      closable: activeModeTabs.length > 1,
  ...
  const activeNonChatContent = activeTab?.mode === 'context'
        ? <React.Suspense fallback={null}><ContextPanelContent /></React.Suspense>
        : activeTab?.mode === 'git'
            ? <React.Suspense fallback={null}><GitView isActive={isOpen} /></React.Suspense>
            : activeTab?.mode === 'pr'
                ? <React.Suspense fallback={null}><PullRequestView /></React.Suspense>
            : activeTab?.mode === 'notes'
                ? <React.Suspense fallback={null}><ProjectContextPanel /></React.Suspense>
        : activeTab?.mode === 'plan'
  ...
  const browserTabs = React.useMemo(
    () => visibleTabs.filter((tab) => tab.mode === 'browser'),
    [visibleTabs],
  );
  const diffTabs = React.useMemo(
    () => visibleTabs.filter((tab) => tab.mode === 'diff'),
    [visibleTabs],
  );
  const hasTerminalTab = React.useMemo(
    () => visibleTabs.some((tab) => tab.mode === 'terminal'),
    [visibleTabs],
  );
  ...
  const hasWalkthroughTab = React.useMemo(
    () => visibleTabs.some((tab) => tab.mode === 'walkthrough'),
    [visibleTabs],
  );
  const hasFileTabs = React.useMemo(
    () => visibleTabs.some((tab) => tab.mode === 'file'),
    [visibleTabs],
  );
  const isFileTabActive = activeTab?.mode === 'file';
  const isActiveFileEditorOpen = Boolean(isFileTabActive && activeTab?.targetPath);
  const showTabStrip = isMultiInstanceMode && activeModeTabs.length > 1;

  const header = (
    <header className="flex h-10 items-stretch border-b border-border/60">
      {showTabStrip ? (
        <SortableTabsStrip
          items={tabItems}
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2 px-3">
          ...
          <span className="truncate typography-ui-label font-medium text-foreground">
            {activeTab ? getTabLabel(activeTab, sessionTitleById, t) : null}
          </span>
        </div>
      )}
      <div className="flex items-center gap-1 px-1.5">
        ...
        {isFileTabActive && isActiveFileEditorOpen ? (
          <Button
            type="button"
            variant="ghost"
            size="sm"
      ...
      <div className={cn('relative min-h-0 flex-1 overflow-hidden', isResizing && 'pointer-events-none')}>
        ...
        {hasTerminalTab ? (
          <div className={cn('absolute inset-0', activeTab?.mode === 'terminal' ? 'block' : 'hidden')}>
            <React.Suspense fallback={null}>
              <TerminalView visible={isOpen && activeTab?.mode === 'terminal'} contextPanelChrome />
            </React.Suspense>
          </div>
        ) : null}
        {hasWalkthroughTab ? (
