...
export const ProjectActionsButton = ({
  ...
  const displayActions = React.useMemo(
    () => canUseAutoDiscover ? [...actions, autoDiscoverAction] : actions,
  ...
  const resolvedSelected = selectedAction ?? (compact ? actions[0] : displayActions[0]) ?? null;
  if (compact && !resolvedSelected && canUseAutoDiscover) {
    const autoDiscoverRunKey = toProjectActionRunKey(normalizedDirectory, autoDiscoverAction.id);
    const autoDiscoverRun = projectActionRuns[autoDiscoverRunKey];
    const autoDiscoverStopping = autoDiscoverRun?.status === 'stopping';
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className={cn(
              'app-region-no-drag inline-flex h-8 w-6 items-center justify-center rounded-md text-muted-foreground',
        ...
        <DropdownMenuContent align="start" className="w-52">
          ...
          <DropdownMenuItem
            ...
            disabled={isLoading || autoDiscoverStopping}
            onClick={() => {
              if (autoDiscoverRun) {
                void stopAction(autoDiscoverAction);
                return;
              }
              void runAction(autoDiscoverAction);
            }}
          >
            <Icon name="scan-2" className="h-4 w-4" />
            <span className="typography-ui-label text-foreground truncate">{autoDiscoverAction.name}</span>
            {autoDiscoverStopping || autoDiscoverRun?.status === 'waiting-for-preview'
        ...
        <TooltipTrigger asChild>
          <button
            ...
            className={cn(
              ...
              'transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] disabled:cursor-not-allowed'
            )}
            aria-label={selectedRunning
              ? t('projectActions.actions.stopNamedAria', { name: resolvedSelected.name })
              : t('projectActions.actions.runNamedAria', { name: resolvedSelected.name })}
          ...
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                ...
                'hover:bg-interactive-hover transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]'
              )}
              aria-label={t('projectActions.actions.openPreview')}
            >
              <Icon name="global" className="h-4 w-4" />
        ...
        <DropdownMenuTrigger asChild>
          <button
            ...
            className={cn(
              ...
              'hover:bg-interactive-hover hover:text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]'
            )}
            aria-label={t('projectActions.actions.chooseActionAria')}
          >
