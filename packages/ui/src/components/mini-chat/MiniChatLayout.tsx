...
const MiniChatHeader: React.FC<{ mode: MiniChatMode }> = ({ mode }) => {
  ...
  const worktreeMetadataBranch = useSessionUIStore((state) => (
    isDeveloperMode && currentSessionId
      ? state.worktreeMetadata.get(currentSessionId)?.branch?.trim() ?? null
      : null
  ));
  ...
  React.useEffect(() => {
    if (!isDeveloperMode || !openDirectory || isChatContext) return;
    void ensureGitStatus(openDirectory, runtimeApis.git).catch(() => {});
  }, [ensureGitStatus, isChatContext, isDeveloperMode, openDirectory, runtimeApis.git]);
  ...
  const projectLabel = React.useMemo(() => {
    ...
    if (!project) return directoryLabel || 'Ivaldi';
  ...
  const rawBranchLabel = isDeveloperMode
    ? gitBranchForDirectory || worktreeMetadataBranch || sessionWorktreeMetadata?.branch?.trim() || worktreeAttachment?.branch?.trim() || catalogWorktreeBranch
    : null;
