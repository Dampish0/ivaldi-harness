...
export function ForkSessionDialog(props: ForkSessionDialogProps) {
  ...
  const showCreateWorktree = isDeveloperMode && !isVSCodeRuntime();
  ...
  }, [isDeveloperMode, open, loadProviders, loadConfigAgents, loadAgentsStoreAgents, projectDirectory]);
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!submitting) onOpenChange(nextOpen); }}>
      <DialogContent className="max-w-md overflow-visible">
        <DialogHeader>
          <DialogTitle>
            {t(isDeveloperMode ? 'chat.messageBody.actions.startNewSession' : 'chat.work.startNewFromAnswer')}
          </DialogTitle>
        </DialogHeader>
            ...
            {showRunAsGoal ? (
              <div className="flex min-w-0 items-center gap-2">
                <Checkbox
                  ...
                  disabled={submitting}
                  ariaLabel={t(isDeveloperMode
                    ? 'sessions.scheduledTasks.editor.goal.aria'
                    : 'chat.work.keepWorkingUntilComplete')}
                />
                <button
                  type="button"
                  className="truncate typography-ui-label text-muted-foreground transition-colors hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
                ...
                >
                  {t(isDeveloperMode
                    ? 'sessions.scheduledTasks.editor.goal.label'
                    : 'chat.work.keepWorkingUntilComplete')}
                </button>
              </div>
            ) : null}
