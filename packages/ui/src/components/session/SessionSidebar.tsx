...
const SessionSidebarComponent: React.FC<SessionSidebarProps> = ({
  ...
  const gitBranches = useGitAllBranches(isVisible && isDeveloperMode);
  ...
  const isWorktreeTopologyLoading = isDeveloperMode && !isVSCode && resolvedWorktreeTopologyKey !== projectWorktreeDiscoveryKey;
  ...
  React.useEffect(() => {
    ...
    const discoverWorktrees = async () => {
      ...
      if (projectEntries.length === 0 || isVSCode || !isDeveloperMode) {
        if (!cancelled) {
  ...
  }, [isDeveloperMode, isVSCode, projectWorktreeDiscoveryKey, runtimeKey, worktreeDiscoveryRevision]);
  ...
  const gitRepoStatus = useGitRepoStatusMap(isVisible && isDeveloperMode ? normalizedProjectPaths : EMPTY_STRING_ARRAY);
  useProjectRepoStatus({
    enabled: isVisible && isDeveloperMode,
    normalizedProjects,
    gitRepoStatus,
  ...
  const useGroupedSections = isDeveloperMode && sessionGroupingMode === 'by-worktree' && !isVSCode;
