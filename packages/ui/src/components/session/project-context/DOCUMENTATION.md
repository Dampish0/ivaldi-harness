...
| `KnowledgeCard.tsx` | the shared row shell and expand interaction every entry list uses |
| `useProjectTodoSend.ts` | sending a todo to a current/new/worktree session |

The worktree target is a Developer-mode action. Ivaldi Work mode keeps sending
todos to the current or a new normal session, but does not expose a worktree
destination.
...
## One row, one interaction

Every entry list renders `KnowledgeCard`. The name is historical; visually it is
a flat list row separated by a divider. Notes and memories had drifted into two
different interactions in the same panel, so they still share one shell even
though the shell no longer needs card chrome.

A collapsed row opens on a click anywhere on it. An expanded row closes only
