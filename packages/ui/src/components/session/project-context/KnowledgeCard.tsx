 ...
 *
 * A collapsed row opens on a click anywhere on it — the whole row is the
 * target, not a chevron the user has to aim at. An expanded row closes only
...
export const KnowledgeCard: React.FC<{
  ...
  return (
    <li
      className={cn(
        'flex flex-col gap-1 border-b border-[var(--interactive-border)] px-1 py-2 last:border-b-0',
        !expanded && 'cursor-pointer hover:bg-interactive-hover/30',
      )}
    ...
    >
      <div className="flex min-w-0 items-start gap-2">
        ...
        {/* Stopped here rather than on each control: every action is a click on
            the row too, and without this each one would also toggle it. */}
        <div
