...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  return (
    <div className={cn('flex h-full min-h-0 w-full min-w-0 flex-col', className)}>
      ...
      <div className="flex flex-shrink-0 items-center gap-2 px-3 py-2">
        ...
        <div className="relative w-40 flex-shrink-0">
          ...
          <Input
            ...
            className="h-8 rounded-md bg-transparent pl-7 pr-7 ring-0 shadow-none hover:[&:not(:focus)]:bg-interactive-hover/30 focus:bg-[var(--surface-elevated)] focus:ring-1 focus:ring-[var(--interactive-focus-ring)]"
          />
          {query ? (
            <button
              type="button"
              onClick={() => setQuery('')}
              className="absolute right-1.5 top-1/2 inline-flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-interactive-hover/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
      ...
      {contextEntry.error && !contextEntry.loading ? (
        <div className="mx-3 mb-2 flex flex-shrink-0 items-center justify-between gap-3 rounded-md border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-3 py-2">
          <span className="typography-meta text-[var(--status-error)]">
            {t('rightSidebar.contextNotesTodo.toast.loadNotesFailed')}
          </span>
          <Button
            size="xs"
            variant="outline"
            onClick={() => void loadProjectContext(projectRef, { force: true })}
          >
      ...
      {/* Mobile: a half-width panel has no room for a side column, so the
          sections use the same compact flat tabs as the mobile workspace
          header. The active tab carries the label; the rest stay icon-first. */}
      {isMobile ? (
        <nav
          className="flex flex-shrink-0 items-center gap-0.5 overflow-x-auto px-3 pb-2"
          ...
          {sections.map((section) => {
            ...
            return (
              <Button
                ...
                className={cn(
                  'h-8 min-w-8 flex-shrink-0 gap-1.5 rounded-md px-2 text-muted-foreground',
                  isActive
              ...
              >
                ...
                {isActive ? (
                  <span className="whitespace-nowrap typography-ui-label font-medium">{section.label}</span>
                ) : null}
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className={cn('min-h-0 min-w-0 flex-1 px-3 pb-3 pt-2', openPlan ? 'overflow-hidden' : 'overflow-y-auto')}>
        {activeTab === 'notes' ? (
          <NotesSection
        ...
        {isMobile ? null : (
        <nav
          className="relative flex flex-shrink-0 flex-col gap-0.5 overflow-y-auto border-l border-[var(--interactive-border)] py-2 pl-2 pr-2"
