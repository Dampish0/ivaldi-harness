...
- `shell/SidebarFooter` owns the profile menu, with Edit profile, Usage and Settings links. Its single-line trigger shows the saved local name and an avatar. Edit profile offers Work and Developer mode cards, applied with Save. The menu shows the GitHub username or explicit disconnected state in both product modes. Connect and Disconnect open the shared GitHub account dialog and start the requested action once. CLI connections use the existing Ivaldi-only disable action. The compact download button opens the Update menu for confirmed Ivaldi and independently upgradable OpenCode updates. The header has no account control.
