...
import { formatProjectLabel, formatSessionCompactDateLabel, formatSessionDateLabel, normalizePath, renderHighlightedText, resolveWorkChatTitle } from '../utils';
    ...
    <Tooltip delayDuration={500}>
      <TooltipTrigger asChild>
        <button
          type="button"
          className={cn(
            'inline-flex items-center justify-center rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] transition-opacity',
            shiftHeld
...
function SessionNodeItemComponent(props: SessionNodeItemProps): React.ReactNode {
  ...
  const isWorkMode = !isDeveloperMode;
  const {
    node,
    depth = 0,
    groupDirectory,
  ...
  // Project sessions sit one small step deeper than the project header. The
  // row background still spans the full list width, so selection remains easy
  // to scan while the text hierarchy reads as an actual tree.
  const projectRowIndentPx = renderContext === 'project' && Boolean(projectId) && !isVSCode && !mobileVariant ? 32 : 0;
  ...
  const tooltipBranchLabel = isDeveloperMode
    ? secondaryMeta?.branchLabel ?? node.worktree?.branch ?? null
    : null;
  const prLookupKey = React.useMemo(() => {
    if (!isDeveloperMode || isVSCode) return null;
  ...
  }, [isDeveloperMode, isVSCode, node.worktree]);
  ...
  const showInlineBranchMarker = isDeveloperMode
    && Boolean(tooltipBranchLabel)
  ...
  const sessionTitle = isWorkMode
    ? resolveWorkChatTitle(resolvedSession.title, t('chat.work.untitledChat'))
    : resolvedSession.title || t('sessions.sidebar.session.untitled');
  ...
  if (editingId === session.id) {
    return (
      <div
        key={session.id}
        style={{ paddingLeft: ROW_TEXT_LEFT_PX + projectRowIndentPx + depth * ROW_DEPTH_STEP_PX }}
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0">
          ...
          >
            ...
            <button
              ...
              className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
            >
  ...
  const leadingIndicators = isMovingToWorktree || showStatusMarker || showPinnedMarker ? (
    <span
      style={{ left: ROW_GUTTER_LEFT_PX + projectRowIndentPx + depth * ROW_DEPTH_STEP_PX }}
      className={cn(
    ...
    >
      {isMovingToWorktree ? (
        <Icon
          name="loader-4"
          className="h-3 w-3 animate-spin text-[var(--status-info)]"
  ...
  const subsessionChevron = hasChildren ? (
    <span
      ...
      style={{ minWidth: 14, minHeight: 14, left: ROW_GUTTER_LEFT_PX + projectRowIndentPx + depth * ROW_DEPTH_STEP_PX }}
      className={cn(
        'absolute top-1/2 inline-flex h-3.5 w-3.5 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] transition-opacity',
  ...
  const handleOpenInEditorClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault();
    event.stopPropagation();
    void runtimeApis?.vscode?.executeCommand('ivaldi.openSessionInEditor', session.id, sessionTitle);
  };
  ...
  const renderSessionMenuItems = ({
    ...
    <>
      ...
      {isDeveloperMode && !isSubtaskSession && !archivedBucket && !isVSCode && !isChatDirectoryPath(sessionDirectory) ? (
        <Tooltip>
          <TooltipTrigger asChild>
                  ...
                  folderEntries.map(({ scope, folder }) => {
                    ...
                    return (
                      <Item key={folder.id} onClick={() => {
                        ...
                        {isCurrent ? <Icon name="check" className="ml-2 h-3.5 w-3.5 text-foreground flex-shrink-0" /> : null}
                      </Item>
                    );
                  })
                )}
            ...
            render={
              <div
                ...
                className={cn(
                  ...
                  // The open session and multi-select use the same neutral
                  // selection language instead of borrowing CTA color.
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={cn(
	                      'flex min-w-0 flex-1 cursor-pointer flex-col gap-0 overflow-hidden text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] text-foreground select-none transition-[padding]',
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    className={cn(
                      'inline-flex items-center justify-center rounded-md text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] transition-opacity',
