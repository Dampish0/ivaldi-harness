...
const VisibleSessionProjects: React.FC<SessionProjectCollectionProps> = ({ topology, view, actions }) => {
  ...
  React.useEffect(() => {
    if (!isDeveloperMode || !github || !githubAuthChecked || !githubAuthStatus?.connected) return;
  ...
  }, [isDeveloperMode, ensureEntry, github, githubAuthChecked, githubAuthStatus?.connected, projectSections, projectView.collapsedProjects, refreshTargets, setParams, topology.gitBranches]);
  ...
  const chatGroup = React.useMemo<SessionGroup | null>(() => {
    ...
    const chatsRoot = collection.chatSessions.map((session) => getChatsRootFromDirectory(session.directory)).find(Boolean)
      ?? getChatsRootForHome(view.homeDirectory)
      ?? null;
  ...
  const scrollerModel = React.useMemo(() => ({
    ...
    hasManagedChats: collection.chatSessions.length > 0,
    ...
    recentSection,
    collection.chatSessions.length,
    singleProjectMode,
