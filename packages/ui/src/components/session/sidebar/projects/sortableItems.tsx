          ...
          >
            <ProjectIconImage
              ...
              fallback={projectIconName ? (
                ...
                <Icon name="folder" className="h-3.5 w-3.5 text-muted-foreground/80" />
              )}
            />
          </span>
        ) : projectIconName ? (
...
export const SortableProjectItem: React.FC<SortableProjectItemProps> = ({
  ...
  const renderProjectMenuItems = (Item: React.ElementType) => (
    <>
      ...
      {isDeveloperMode && isRepo && !hideDirectoryControls && onManageWorktrees && (
      ...
      {!hideHeader ? (
        <>
          ...
          <ContextMenu open={isContextMenuOpen} onOpenChange={setIsContextMenuOpen}>
            ...
            >
            <div
              className="relative flex items-center gap-1 py-1.5 pl-4 pr-3.5 group-hover/project:bg-interactive-hover/30 group-focus-within/project:bg-interactive-hover/30"
              {...attributes}
            >
              {isProjectPicker ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button
                      ...
                      className={cn(
                        'flex-1 min-w-0 flex items-center gap-1.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] rounded-md transition-[padding]',
                        isDeveloperMode && isRepo && !hideDirectoryControls
                  ...
                  <DropdownMenuContent align="start" className="max-h-[70vh] min-w-[220px] overflow-y-auto">
                    {projectPickerOptions?.map((option) => (
                      ...
                      >
                        ...
                        {option.id === id ? <Icon name="check" className="h-4 w-4 flex-shrink-0 text-foreground" /> : null}
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>
                ...
                <Tooltip delayDuration={800}>
                  <TooltipTrigger asChild>
                      <button
                        ...
                        {...listeners}
                        className={cn(
                          'flex-1 min-w-0 flex items-center gap-1.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] rounded-md cursor-grab active:cursor-grabbing transition-[padding]',
                          isDeveloperMode && isRepo && !hideDirectoryControls
                            ? (alwaysShowActions ? 'pr-20' : 'pr-7 group-hover/project:pr-20 group-focus-within/project:pr-20')
                            : (alwaysShowActions ? 'pr-14' : 'pr-7 group-hover/project:pr-14 group-focus-within/project:pr-14'),
                        )}
                      ...
                      {statusIndicator ? (
                        <span className={cn(
                          'ml-auto inline-flex flex-shrink-0 items-center transition-opacity',
                          alwaysShowActions
                            ? 'opacity-100'
              ...
              <div className={cn(
                ...
                {isDeveloperMode && showCreateButtons && isRepo && !hideDirectoryControls && onNewWorktreeSession ? (
                  <Tooltip delayDuration={500}>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={cn(
                        'inline-flex h-6 w-6 items-center justify-center rounded-md text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] hover:text-foreground transition-opacity',
                ...
                >
                    <DropdownMenuTrigger asChild>
                      <button
                        type="button"
                        className={cn(
                          'inline-flex h-6 w-6 items-center justify-center rounded-md text-muted-foreground transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] hover:text-foreground',
                          isMenuOpen
