  ...
  return (
    ...
    >
            ...
            {renderedSections.map((section) => {
              ...
              return (
                <SortableProjectItem
                 ...
                 >
                  {!isCollapsed ? (
                    <div className="relative space-y-0 pt-0.5 pb-1 before:pointer-events-none before:absolute before:bottom-1 before:left-3 before:top-0 before:w-px before:bg-border/30">
                      {(() => {
      ...
      {enableStickyFade && (leadingProject || model.hasSharedSessions) ? (
        <div
          className="oc-sticky-fade-overlay pointer-events-none absolute inset-x-0 top-0 z-30 flex items-center gap-1 py-1 pl-4 pr-5"
          aria-hidden="true"
        >
          {leadingProject && leadingProjectLabel ? (
            ...
            <>
              <span className="h-3.5 w-3.5 flex-shrink-0" />
              <span className="typography-micro truncate font-medium uppercase tracking-[0.1em] text-muted-foreground/60">
                {t(model.hasManagedChats
