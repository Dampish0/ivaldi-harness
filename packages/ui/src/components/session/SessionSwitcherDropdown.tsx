...
import { formatSessionCompactDateLabel, resolveWorkChatTitle } from './sidebar/utils';
      ...
      <div className="space-y-0.5">
        ...
        {items.length === 0 ? (
          <div className="px-3 py-4 text-center typography-meta text-muted-foreground">
            {isWorkMode ? t('chat.work.noRecentChats') : t('sessions.switcher.empty')}
          </div>
        ) : (
          items.map((item) => (
            <SwitcherNode
...
function SwitcherRow({ session, depth, variant, secondaryMeta, hasChildren, isExpanded, onToggleExpand, closeDropdown }: SwitcherRowProps): React.ReactElement {
  ...
  const sessionTitle = isWorkMode
    ...
    : session.title?.trim() || t('sessions.sidebar.session.untitled');
  ...
  const branchLabel = isDeveloperMode && rawBranchLabel && rawBranchLabel !== 'HEAD' ? rawBranchLabel : null;
    ...
    >
      ...
      {isStreaming || showUnreadDot ? (
        <span className="flex h-3 w-3 flex-shrink-0 items-center justify-center self-center">
          {isStreaming ? (
            <span
              className="h-1.5 w-1.5 rounded-full bg-[var(--status-success)] animate-busy-pulse"
