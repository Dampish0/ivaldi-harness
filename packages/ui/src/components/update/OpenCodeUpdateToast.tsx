...
export const OpenCodeUpdateToast: React.FC = () => {
  ...
  const runUpgrade = React.useCallback(async () => {
    if (upgradingRef.current || useProductModeStore.getState().mode !== 'developer') return;
    ...
    try {
      ...
      if (useProductModeStore.getState().mode !== 'developer') {
        toast.dismiss(UPGRADE_TOAST_ID);
        return;
      }
  ...
  React.useEffect(() => {
    const showUpdateAvailableToast = (version: string) => {
      // Work mode keeps OpenCode maintenance details out of the everyday UI.
      // Developer mode still follows the user's notification preference.
      if (
    ...
    const onUpdateAvailable = (event: Event) => {
      ...
      if (version && useProductModeStore.getState().mode === 'developer') {
