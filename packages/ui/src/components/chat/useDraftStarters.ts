...
export function useDraftStarters(): UseDraftStartersResult {
    ...
    const resolve = React.useCallback((ref: DraftStarterRef, group: StarterGroup): ResolvedStarter | null => {
        ...
        if (ref.type === 'command' && !isComposerCommandVisibleInProductMode(productMode, ref.name)) return null;
        if (ref.type === 'skill' && !isComposerSkillVisibleInProductMode(productMode, ref.name)) return null;
        if (ref.type === 'command') {
    ...
    }, [t, commandNames, skillNames, isVSCode, productMode]);

    const globalRefs = React.useMemo<readonly DraftStarterRef[]>(
        () => globalRaw ?? (productMode === 'work' ? DEFAULT_WORK_GLOBAL_STARTERS : DEFAULT_GLOBAL_STARTERS),
        [globalRaw, productMode],
    );
    ...
    const pinnable = React.useMemo<PinnableItem[]>(() => {
        ...
        for (const b of BUILTIN_STARTERS) {
            ...
            if (!isComposerCommandVisibleInProductMode(productMode, b.name)) continue;
        ...
        for (const c of commands) {
            ...
            if (!isComposerCommandVisibleInProductMode(productMode, c.name)) continue;
        ...
        for (const sk of skills) {
            if (!isComposerSkillVisibleInProductMode(productMode, sk.name)) continue;
    ...
    }, [t, commands, skills, pinnedKeys, isVSCode, productMode]);
    ...
    const addStarter = React.useCallback((item: PinnableItem) => {
        ...
        if (item.scope === 'project') {
            ...
            const base = globalRaw ?? (productMode === 'work' ? DEFAULT_WORK_GLOBAL_STARTERS : DEFAULT_GLOBAL_STARTERS);
            if (base.some((r) => sameStarter(r, ref))) return;
            persistGlobal([...base, ref]);
        }
    }, [projectRef, projectStarters, globalRaw, persistProject, persistGlobal, productMode]);
    ...
    }, [projectStarters, globalRaw, persistProject, persistGlobal, productMode]);

    const reorder = React.useCallback((group: StarterGroup, fromId: string, toId: string) => {
        const base = group === 'project'
            ? projectStarters
            : (globalRaw ?? (productMode === 'work' ? DEFAULT_WORK_GLOBAL_STARTERS : DEFAULT_GLOBAL_STARTERS));
