...
import { isFullySyntheticMessage, isLegacyGoalCommandMessage, isLegacyGoalContinuationMessage } from '@/lib/messages/synthetic';
...
const ChatViewport = React.memo(({
    ...
    const hideProtocolMessages = useProductModeStore((state) => state.mode === 'work');
    const promptPreviewsByTurnId = React.useMemo(() => {
        ...
        for (let index = 0; index < renderedMessages.length; index += 1) {
            ...
            if (isFullySyntheticMessage(message.parts) || (hideProtocolMessages && (isLegacyGoalCommandMessage(message.parts) || isLegacyGoalContinuationMessage(message.parts)))) {
                continue;
            }
        ...
        return next;
    }, [hideProtocolMessages, renderedMessages]);
...
const DraftWelcome: React.FC<{ exiting?: boolean }> = ({ exiting = false }) => {
    ...
    return (
        <div className={cn(
            'oc-draft-center flex min-h-0 flex-1 flex-col items-center justify-center text-center transition-opacity duration-[120ms] ease-out motion-reduce:transition-none',
            isMobileSurface ? 'px-5' : 'px-6',
            exiting && 'pointer-events-none opacity-0',
        )}>
            <h1 className={cn(
                'text-balance font-normal tracking-tight text-foreground',
                isMobileSurface ? 'max-w-[21rem] text-[26px] leading-[1.12]' : 'text-3xl',
            )}>
                {renderDraftTitle(
                    isWorkMode
                        ? projectLabel
                            ? t('chat.emptyState.workDraftTitleWithProject', { project: projectLabel })
...
export const ChatContainer: React.FC<ChatContainerProps> = ({
    ...
    const workStatusPanelMountable = Boolean(currentSessionId) && !draftOpen && !isMobile
        && !isVSCode
