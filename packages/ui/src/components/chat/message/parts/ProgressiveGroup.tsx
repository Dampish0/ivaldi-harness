...
const TOOL_ACTIVITY_ROW_CLASS = 'flex min-h-7 w-full min-w-0 items-center gap-2 rounded-md px-1.5 py-1';
...
const getActivityStatus = (activity: TurnActivityPart): string => {
    ...
    return activity.part.state.status.toLowerCase();
};
...
const StaticToolRowInner: React.FC<{
    ...
    const hasErrorActivity = React.useMemo(() => activities.some((activity) => {
    ...
    return (
        <div
            ...
            className={cn('oc-static-tool-row', TOOL_ACTIVITY_ROW_CLASS)}
        >
            <div className="inline-flex size-4 flex-shrink-0 items-center justify-center" style={{ color: 'var(--tools-icon)' }}>
                {hasErrorActivity ? (
            ...
            <MinDurationShineText
                ...
                minDurationMs={1000}
                className={cn(TOOL_ROW_TITLE_CLASS, 'inline-flex flex-shrink-0 items-center')}
                style={{ color: 'var(--tools-title)' }}
                title={displayName}
            >
                {displayName}
...
const ProgressiveGroup: React.FC<ProgressiveGroupProps> = ({
    ...
    if (!showHeader) {
        return (
            <FadeInOnReveal>
                <div className="my-1 space-y-0.5">{renderedRows}</div>
            </FadeInOnReveal>
        );
    }
