...
- Completed tool rows keep their tool-specific icon. Only running and failed
  states replace that icon with a spinner or error marker, so a finished turn
  remains scannable by tool type instead of becoming a column of success
  checkmarks.
- In sorted mode, short activity segments render their rows directly. The
  generic collapsible Activity header appears only when a visible segment is
...
- Work presentation also treats the narrowly recognized legacy `/goal`
  ...
  so Work removes OpenCode control copy without changing conversation history.
  Developer continues to render those protocol messages.
