...
const TOOL_ACTIVITY_ROW_CLASS = 'group/tool flex min-h-7 w-full min-w-0 items-center gap-2 rounded-md px-1.5 py-1 text-left transition-colors hover:bg-interactive-hover';
...
const ToolPartContent: React.FC<ToolPartProps> = ({
    ...
    const statusGlyph = isError ? (
        <Icon name="error-warning" className="size-3.5 text-status-error" />
    ) : isActive ? (
        ...
        <div>
            ...
            >
                    ...
                    {isMultiFileApplyPatch ? (
                        <>
                            <div className="flex h-5 flex-shrink-0 items-center gap-2">
                                <span className="inline-flex size-4 flex-shrink-0 items-center justify-center" style={iconStyle}>
                                    {statusGlyph}
                                </span>
                            ...
                            <Icon
                                ...
                                className="ml-auto size-3.5 flex-shrink-0 text-muted-foreground/60 transition-opacity group-hover/tool:text-muted-foreground"
                            />
                        </>
                    ) : (
                        <>
                            <div
                                className="inline-flex size-4 flex-shrink-0 cursor-pointer items-center justify-center"
                ...
                {!isMultiFileApplyPatch ? (
                    <Icon
                        name={isExpanded ? 'arrow-down-s' : 'arrow-right-s'}
                        className="size-3.5 flex-shrink-0 text-muted-foreground/60 transition-opacity group-hover/tool:text-muted-foreground"
                    />
                ) : null}
            </div>
            ...
            {!isTaskTool ? (
                ...
                >
                    {shouldRenderExpandedContent ? (
                        <div
                            className="relative ml-[7px] pt-1 pl-4"
                        >
                            <span
                                aria-hidden="true"
