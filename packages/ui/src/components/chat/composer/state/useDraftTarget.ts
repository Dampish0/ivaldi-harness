...
export function useDraftTarget(enabled: boolean, options: { includeWorktrees?: boolean } = {}) {
    ...
    const worktreeLookupPath = includeWorktrees ? selectedDraftProjectPath : null;
    const selectedDraftProjectBranches = useGitBranches(worktreeLookupPath);
    const selectedDraftProjectBranchesFetchedAt = useGitStore(
        (s) => (worktreeLookupPath ? s.directories.get(worktreeLookupPath)?.lastBranchesFetch ?? 0 : 0),
    );
    const selectedDraftProjectIsGitRepo = useIsGitRepo(worktreeLookupPath);
    ...
    React.useEffect(() => {
        if (!enabled || !includeWorktrees || !selectedDraftProjectPath || !runtimeGit || selectedDraftProjectIsGitRepo !== null) {
            return;
        }

        void fetchGitStatus(selectedDraftProjectPath, runtimeGit, { silent: true });
    }, [fetchGitStatus, includeWorktrees, runtimeGit, selectedDraftProjectIsGitRepo, selectedDraftProjectPath, enabled]);

    React.useEffect(() => {
        if (!enabled || !includeWorktrees || !selectedDraftProjectPath || !selectedDraftProject || !runtimeGit || selectedDraftProjectIsGitRepo !== true) {
    ...
    }, [fetchBranches, includeWorktrees, runtimeGit, selectedDraftProject, selectedDraftProjectBranchesFetchedAt, hasDraftBranchList, selectedDraftProjectIsGitRepo, selectedDraftProjectPath, enabled]);
    ...
    const projectRootBranchOption = React.useMemo(() => {
        if (!includeWorktrees || !selectedDraftProject) {
            return null;
        }
    ...
    }, [includeWorktrees, selectedDraftProject, selectedDraftProjectCurrentBranch]);
    ...
    }, [availableWorktreesByProject, includeWorktrees, newSessionDraft?.bootstrapPendingDirectory, selectedDraftProject, selectedDraftProjectCurrentBranch, selectedDraftProjectPath]);
    ...
    }, [includeWorktrees, isDiscoveringDraftBranches, projectRootBranchOption, selectedDraftProjectIsGitRepo, worktreeBranchOptions.length]);
