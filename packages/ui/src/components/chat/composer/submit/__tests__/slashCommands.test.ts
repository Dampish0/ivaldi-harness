...
describe('findMagicPromptCommand', () => {
    ...
    test('/plan is the friendly alias for the existing planning command', () => {
        expect(findMagicPromptCommand('plan')?.name).toBe('plan-feature');
        expect(findMagicPromptCommand('plan')).toBe(findMagicPromptCommand('plan-feature'));
    });
...
describe('buildCommandVariables', () => {
    ...
    test('a /plan argument is preserved as the planning idea', () => {
        const variables = buildCommandVariables(findMagicPromptCommand('plan')!, 'migrate the database');
