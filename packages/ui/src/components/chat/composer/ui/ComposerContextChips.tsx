...
const ENTRY_LABEL_CLASS = 'text-[10px] font-medium text-[var(--surface-mutedForeground)] opacity-60';
...
}> = ({ draft, index, title, isMobile, editing, onStartEdit, onEndEdit, onRemove, onSaveComment }) => {
    const { t } = useI18n();
    ...
    return (
        <div>
            ...
            <div className="space-y-2 px-3 py-2">
                {draft.code.trim() ? (
                    <div>
                        <div className={cn(ENTRY_LABEL_CLASS, !isMobile && 'uppercase tracking-wide')}>
...
export function ComposerContextChips({ draftTarget, colors, isMobile = false }: ComposerContextChipsProps) {
    const { t } = useI18n();
            ...
            <div className="flex flex-wrap items-center gap-2 pb-2">
                {groups.map((group) => (
                    <button
                        ...
                        type="button"
                        className={cn(
                            'inline-flex max-w-full items-center gap-1.5 border px-2.5 py-1 text-left',
