...
export const RevertedMessageDock: React.FC<RevertedMessageDockProps> = React.memo(({ sessionId, directory, isMobile = false }) => {
    const { t } = useI18n();
            ...
            <div className={cn(
                ...
                >
                    <span className="typography-ui-label font-medium text-foreground flex-shrink-0">
                        {t('chat.revertPopover.title')}
                    </span>
                    <span className="typography-meta tabular-nums text-muted-foreground">{items.length}</span>
                    <Icon
                        name="arrow-down-s"
