...
export function MobileDraftTargetTriggers(
    ...
    return (
        <div className="mb-1.5 flex min-w-0 items-center gap-x-2 px-0.5">
            <button
                type="button"
                className={`inline-flex h-7 min-w-0 flex-shrink cursor-pointer items-center gap-1 rounded-md px-1.5 typography-micro font-medium text-foreground/80 hover:bg-[var(--interactive-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)] ${showBranchSelector ? 'max-w-[42vw]' : 'max-w-[70vw]'}`}
            ...
            {showBranchSelector ? (
                <button
                    type="button"
                    className="inline-flex h-7 min-w-0 max-w-[48vw] flex-shrink cursor-pointer items-center gap-1 rounded-md px-1.5 typography-micro font-medium text-foreground/80 hover:bg-[var(--interactive-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
                        ...
                        {rankByQuery(projects, query, (project) => [getProjectDisplayLabel(project), project.path])
                            .map((project) => (
                                <button
                                    key={project.id}
                                    type="button"
                                    className={`flex w-full cursor-pointer items-center gap-2 rounded-md px-2 py-2.5 text-left typography-ui-label transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--interactive-focus-ring)] ${project.id === selectedProject.id ? 'bg-interactive-selection text-interactive-selection-foreground' : 'hover:bg-interactive-hover'}`}
                                    onClick={() => {
                        ...
                        {(() => {
                            ...
                            const renderRow = (value: string, label: React.ReactNode, key?: string) => {
                                ...
                                <button
                                    key={key ?? value}
                                    type="button"
                                    className={`flex w-full cursor-pointer items-center gap-2 rounded-md px-2 py-2.5 text-left typography-ui-label transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--interactive-focus-ring)] ${selected ? 'bg-interactive-selection text-interactive-selection-foreground' : 'hover:bg-interactive-hover'}`}
                                    onClick={() => {
                            ...
                            return (
                                <>
                                    ...
                                    {rankByQuery(worktreeBranchOptions, query, (option) => [option.label])
                                        .map((option) => renderRow(
                                            option.value,
                                            <span className="flex min-w-0 items-center gap-1.5">
                                                {option.pending ? <Icon name="loader-4" className="size-3.5 shrink-0 animate-spin text-muted-foreground" /> : null}
