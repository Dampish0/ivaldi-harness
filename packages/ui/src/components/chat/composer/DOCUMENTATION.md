...
- Ivaldi Work mode calls the same draft-target hook without worktree discovery.
  A project draft targets the project root and shows no branch selector, while
  Developer mode keeps the branch/worktree target flow. This is presentation
  only; agent, model and tool capability are unchanged.
- Work mode also uses progressive disclosure in the composer. Its resting
  footer keeps attachment, model, thinking, and one primary action slot. Focus,
  goal, agent, and other operator controls remain
  available to the underlying product but are not permanent Work chrome. The model picker shows
  every available model, grouped by provider so identical model names stay
  distinguishable; provider configuration still lives in Settings. A
  non-default agent may surface as a removable context chip rather than
  restoring the full agent selector. Work-only command modes such as `/plan`
  and `/goal` become removable composer chips after selection instead of
  leaving implementation syntax in the editable prompt.
- `/plan` is the Work-facing alias for the existing guided feature-planning
  prompt pair. `planningIntent.ts` may advertise that command with a small chip
  while the draft looks planning-oriented; the heuristic is presentation only
  and never auto-sends or changes agent capability. Clicking the chip prefixes
  the current draft with `/plan`, preserving the user's planning idea as the
  command argument.
