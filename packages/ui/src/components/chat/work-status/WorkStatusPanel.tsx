...
/**
 * Work-status panel reporting the state of the session, its branch and its
 * subagents.
 *
 ...
 *
 * The scroller lives inside the panel so the same top/bottom scroll shadows the
 * transcript uses stay within its bounds. The scrollbar itself is hidden — at
 * this width it would eat a visible slice of every row's trailing value, and
 * the shadows already say there is more to see.
 */
export const WorkStatusPanel: React.FC<Props> = ({ sessionId, directory, visible, repositoryEnabled = true, overlay = false }) => {
  ...
  // nothing on the way in would make the panel flash out and back on arrival.
  ...
  // Content is dropped only after the collapse finishes, so the panel animates
  ...
  // Hidden or mid-collapse: the panel is not something the user can act on.
  ...
  // for the transient "no data yet" state so the panel doesn't flash an empty
  // shell on first mount.
  ...
  React.useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      // The header control closes it on its own; letting this fire too would
      // close before its menu action can run.
  ...
  return (
    <aside
      ...
      // The panel stays mounted while hidden so it can animate its own collapse,
      ...
      className={cn(
        ...
        overlay ? [
          ...
          // Inline status uses the same visual language as the other docked
          // right panes: full height, one divider, no card inside the workspace.
          'h-full max-h-full self-stretch',
          'border-l border-[var(--interactive-border)]',
          'bg-[var(--surface-muted)]/20',
        ],
      ...
      style={{
        ...
        transform: visible
          ...
          : overlay
            ...
            // Inline: leaves to the right and arrives from it, so the pane
