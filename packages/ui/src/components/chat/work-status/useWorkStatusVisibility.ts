...
/**
 * Fixed panel width. The status pane is not a context-panel surface, so it has
 * no resizer and no persisted width.
 */
...
/** Inline mode is flush with the chat row; overlay spacing consumes no layout width. */
const WORK_STATUS_PANEL_GUTTER = 0;
...
export const useWorkStatusVisibility = ({ isMobile, isVSCode }: Options): Result => {
  ...
  const canMeasure = !isMobile && !isVSCode;
  const layoutAllows = canMeasure && !contextPanelOpen;
  ...
  React.useEffect(() => {
    if (!canMeasure || !rowNode || typeof ResizeObserver === 'undefined') return undefined;
