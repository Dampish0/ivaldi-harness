...
export const WorkStatusPrimaryGroup: React.FC<Props> = ({ sessionId, directory, goalRow, showSession, showRepository }) => {
  ...
  const gitStatus = useGitStore(
    React.useCallback(
      (state) => (isDeveloperMode && directory ? state.directories.get(directory)?.status ?? null : null),
      [directory, isDeveloperMode],
    ),
  );
  ...
  React.useEffect(() => {
    if (!isDeveloperMode || !showRepository || !directory || !git) return;
    void runBackgroundNetworkTask(() => ensureStatus(directory, git));
  }, [directory, git, ensureStatus, isDeveloperMode, showRepository]);
  ...
  }, [clearDiffCache, directory, fetchStatus, git, isDeveloperMode, showRepository]);
  ...
  const prSummary = useFreshestPrVisualSummaryForBranch(
    isDeveloperMode ? directory : null,
    isDeveloperMode ? branch : null,
  );
  ...
  // Keep ordinary consumption neutral. Status color only appears once the
  // context window is genuinely under pressure.
  ...
  return (
    <>
      {hasSession ? (
        <WorkStatusSection title={t(isDeveloperMode ? 'chat.workStatus.section.session' : 'layout.mainTab.chat')}>
          {usagePercent !== null ? (
            <>
              <WorkStatusRow
                icon="donut-chart"
