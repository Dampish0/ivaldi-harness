...
A fixed-width status pane rendered to the right of the transcript inside
`ChatContainer`. It reports the state of the current session, its branch, its
quotas and its subagents.
...
resizer. Inline, it uses the same flat right-pane language as the context panel:
a fixed width, full height and one divider against the transcript. When the
chat is too narrow, the same content becomes a transient rounded overlay using
the shared `oc-glass-panel` surface.
...
- `WorkStatusPanel`, a fixed-width full-height `shrink-0` sibling.
...
The inline pane has no outer horizontal margin, so only its fixed width counts
toward the fit threshold. Overlay spacing is visual only and never consumes
transcript width.
...
Ivaldi Work mode treats the repository block the same way: branch, changed-file
and PR rows do not render, and the panel does not warm Git/PR caches for them.
Session, usage, subagent, task, MCP and context data remain available. The main
header therefore keeps the active project label visible in Work even while this
panel is open. Developer mode preserves the full repository block and may hide
duplicate header metadata while the panel carries it.
...
1. **Chat** in Work / **Session** in Developer (goal, context, cost), **Project** (attention, branch,
...
A persisted preference (`workStatusPanelEnabled`) drives the Work Status action
in the header's contextual-workspace menu, and a dialog behind the equalizer
icon switches individual sections off. The menu action is only present once a
real session exists because a draft cannot render the panel. Hidden sections are
stored rather than visible ones, so a section added later appears for everyone
instead of staying invisible to whoever had saved settings before it existed.
Both preferences travel the full settings pipeline, including the server
...
The panel does not mount on the new-session draft. Project, provider and
context data can already exist there, but there is no session work to report
until the first message materializes a session. This keeps the draft focused on
the composer and prevents a technically valid but semantically premature
status pane from appearing in the corner.
...
An empty pane showing only a settings icon reads as a fault. Each
...
There is one deliberate exception: when the user hides every section, the pane
