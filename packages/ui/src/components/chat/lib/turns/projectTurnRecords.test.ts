...
describe('projectTurnRecords', () => {
    ...
    test('merges legacy goal protocol turns only when the presentation asks to hide them', () => {
        const user = createMessageEntry({ id: 'u1', role: 'user', createdAt: 1 });
        user.parts = [{ id: 'p1', type: 'text', text: 'visible prompt' } as Part];
        const assistant1 = createMessageEntry({ id: 'a1', role: 'assistant', parentID: 'u1', createdAt: 2 });
        const goalContinuation = createMessageEntry({ id: 'u2', role: 'user', createdAt: 3 });
        goalContinuation.parts = [{
            id: 'p2',
            type: 'text',
            text: [
        ...
        const developerProjection = projectTurnRecords([user, assistant1, goalContinuation, assistant2], {
            mergeHiddenUserTurns: { planModeEnabled: false },
        });
        const workProjection = projectTurnRecords([user, assistant1, goalContinuation, assistant2], {
            mergeHiddenUserTurns: { planModeEnabled: false, hideGoalProtocolMessages: true },
        });

        expect(developerProjection.turns.map((turn) => turn.turnId)).toEqual(['u1', 'u2']);
        expect(workProjection.turns).toHaveLength(1);
        expect(workProjection.turns[0]?.assistantMessageIds).toEqual(['a1', 'a2']);
        expect(workProjection.ungroupedMessageIds.has('u2')).toBe(false);
    });

    test('merges compaction protocol turns only when the presentation asks to hide them', () => {
        const user = createMessageEntry({ id: 'u1', role: 'user', createdAt: 1 });
        user.parts = [{ id: 'p1', type: 'text', text: 'visible prompt' } as Part];
        const assistant1 = createMessageEntry({ id: 'a1', role: 'assistant', parentID: 'u1', createdAt: 2 });
        const compact = createMessageEntry({ id: 'u2', role: 'user', createdAt: 3 });
        compact.parts = [{ id: 'p2', type: 'text', text: '/compact' } as Part];
        const compactionSummary = createMessageEntry({ id: 'a2', role: 'assistant', parentID: 'u2', createdAt: 4 });

        const developerProjection = projectTurnRecords([user, assistant1, compact, compactionSummary], {
            mergeHiddenUserTurns: { planModeEnabled: false },
        });
        const workProjection = projectTurnRecords([user, assistant1, compact, compactionSummary], {
            mergeHiddenUserTurns: { planModeEnabled: false, hideCompactionProtocolMessages: true },
        });
