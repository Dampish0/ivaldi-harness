...
import { getDefaultPermissionMode, type PermissionMode } from '@/lib/permissionModes';
...
import { shouldSuggestPlanningCommand } from './composer/planningIntent';
...
type WorkComposerCommand = 'plan' | 'goal';
...
const ChatInputComponent: React.FC<ChatInputProps> = ({
    ...
    const isDeveloperMode = productMode === 'developer';
    const restoredGoalArmed = useSessionGoalArmStore((state) => state.armed);
    ...
    const [workComposerCommand, setWorkComposerCommand] = React.useState<WorkComposerCommand | null>(null);
    const visibleWorkComposerCommand: WorkComposerCommand | null = workComposerCommand ?? (restoredGoalArmed ? 'goal' : null);
    ...
    React.useEffect(() => {
        setWorkComposerCommand(null);
    }, [chatDraftIdentity?.runtimeKey, chatDraftIdentity?.directory, chatDraftIdentity?.sessionId]);
    React.useEffect(() => {
        if (isDeveloperMode) setWorkComposerCommand(null);
    }, [isDeveloperMode]);
    ...
    const isGitRepo = useIsGitRepo(isDeveloperMode ? currentDirectory : null);
    const currentGitStatus = useGitStore((state) =>
        isDeveloperMode && currentDirectory ? state.directories.get(currentDirectory)?.status ?? null : null,
    );
    const ensureGitStatus = useGitStore((state) => state.ensureStatus);
    const fetchGitStatus = useGitStore((state) => state.fetchStatus);
    const clearGitDiffCache = useGitStore((state) => state.clearDiffCache);
    const setSessionPermissionMode = usePermissionStore((state) => state.setSessionMode);
    ...
    React.useEffect(() => {
        if (!isDeveloperMode || !currentDirectory || !runtimeGit) return;
        void ensureGitStatus(currentDirectory, runtimeGit);
    }, [currentDirectory, ensureGitStatus, isDeveloperMode, runtimeGit]);
    ...
    }, [clearGitDiffCache, currentDirectory, fetchGitStatus, isDeveloperMode, runtimeGit]);
    ...
    const chatInputRadius = isMobile ? '1.125rem' : 'var(--radius-xl)';
    ...
    const knownSlashNames = React.useMemo(() => {
        const names = new Set<string>([
            'init', 'review', 'undo', 'redo', 'timeline', 'compact', 'btw', 'summary', 'workspace-review', 'plan', 'plan-feature', 'craft-goal', 'schedule-task', 'catch-up', 'debug', 'weigh', 'explore',
        ]);
    ...
    React.useEffect(() => {
        if (isDeveloperMode || inputMode !== 'shell') return;
        setInputMode('normal');
    }, [inputMode, isDeveloperMode]);
    ...
    const handleQueueMessage = React.useCallback(() => {
        ...
        const commandPrefix = !isDeveloperMode && workComposerCommand ? `/${workComposerCommand} ` : '';
        const messageToQueue = `${commandPrefix}${inputSnapshot.message}`.replace(/^\n+|\n+$/g, '');
        ...
        setMessage('');
        if (workComposerCommand) setWorkComposerCommand(null);
    ...
    }, [getCurrentInputSnapshot, currentSessionId, messageQueueTarget, isDeveloperMode, workComposerCommand, attachedFiles, sanitizeAttachmentsForSend, addToQueue, clearAttachedFiles, isMobile, currentProviderId, currentModelId, currentAgentName, currentVariant, scrollToLatest]);
    ...
    const handleSubmit = async (options?: SubmitOptions) => {
        ...
        const capturedWorkCommand = !queuedOnly && options?.presetText == null && !isDeveloperMode
            ? workComposerCommand
            : null;
        ...
        const availableSkillNames = new Set(
            useSkillsStore.getState().skills
                .filter((skill) => isComposerSkillVisibleInProductMode(productMode, skill.name))
                .map((skill) => skill.name),
        );

        const outgoing = buildOutgoingMessage({
            queued: queuedMessagesToSend,
            composerText: !queuedOnly && inputSnapshot.hasContent
                ? `${capturedWorkCommand ? `/${capturedWorkCommand} ` : ''}${inputSnapshot.message}`
                : null,
            ...
            linkedIssue: linkedIssue
                ...
                : null,
            linkedPr: isDeveloperMode && linkedPr
        ...
        if (!queuedOnly) {
            setMessage('');
            if (capturedWorkCommand) {
                setWorkComposerCommand((current) => current === capturedWorkCommand ? null : current);
            }
        ...
        const parsedCommandCandidate = inputMode === 'normal' ? parseSlashCommand(primaryText) : null;
        const parsedCommand = parsedCommandCandidate
            && isComposerCommandVisibleInProductMode(productMode, parsedCommandCandidate.name)
        ...
        if (parsedCommand) {
            ...
            if (commandName === 'goal' && productMode === 'work' && !isVSCodeRuntime()) {
                const objective = argument.trim();
                if (objective) {
                    shouldArmWorkGoal = true;
        ...
        if (shouldArmWorkGoal) {
            // Arm immediately before dispatch. sendMessage consumes the flag
            // synchronously, so Work never flashes a transient goal pill while
            // response-style/snippet preparation is still running.
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        // command character.
        if (isDeveloperMode && inputMode === 'normal' && e.key === '!') {
    ...
    const handleComposerChange = ({ value, selection, fromPaste, insertedText }: ComposerChange) => {
        ...
        if (isDeveloperMode && inputMode === 'normal' && value.startsWith('!')) {
    ...
    const handleCommandSelect = (command: CommandInfo) => {
        if (!isDeveloperMode && (command.name === 'plan' || command.name === 'goal')) {
            const cursorPosition = composerRef.current?.getSelection().start ?? message.length;
            const textBeforeCursor = message.substring(0, cursorPosition);
            const slashIndex = textBeforeCursor.lastIndexOf('/');
            const nextMessage = slashIndex >= 0
                ? `${message.substring(0, slashIndex)}${message.substring(cursorPosition)}`
                : message;
            const nextCursor = slashIndex >= 0 ? slashIndex : cursorPosition;

            if (command.name === 'plan' && restoredGoalArmed) {
                useSessionGoalArmStore.getState().setArmed(false);
            }
            setWorkComposerCommand(command.name);
            setMessage(nextMessage);
            closeAutocomplete();
            requestAnimationFrame(() => {
                composerRef.current?.focus({ preventScroll: true });
                composerRef.current?.setSelection(nextCursor, nextCursor);
            });
            return;
        }
        ...
        if (!isDeveloperMode) {
            setWorkComposerCommand(null);
            if (restoredGoalArmed) useSessionGoalArmStore.getState().setArmed(false);
        }
    ...
    const handleApplyPlanSuggestion = React.useCallback(() => {
        const idea = message.trim();
        if (!idea) return;
        if (restoredGoalArmed) useSessionGoalArmStore.getState().setArmed(false);
        setWorkComposerCommand('plan');
        closeAutocomplete();
        requestAnimationFrame(() => {
            composerRef.current?.focus({ preventScroll: true });
            composerRef.current?.setSelection(message.length, message.length);
        });
    }, [closeAutocomplete, message, restoredGoalArmed]);

    const showWorkPlanSuggestion = !isDeveloperMode
        && workComposerCommand !== 'plan'
        && shouldSuggestPlanningCommand(message);
    ...
    } = useDraftTarget(showDraftTargetSelectors, { includeWorktrees: isDeveloperMode });
    ...
    const hasPendingChanges = React.useMemo(() => {
        if (!isDeveloperMode || isMiniChatSurface) {
            return false;
        }
    ...
    }, [currentDirectory, currentGitStatus, isDeveloperMode, isGitRepo, isMiniChatSurface]);
    ...
    }, [draftBranchItems, isDeveloperMode, newSessionDraft?.bootstrapPendingDirectory, newSessionDraft?.pendingWorktreeRequestId, newSessionDraft?.preserveDirectoryOverride, selectedDraftDirectory, selectedDraftProject, setNewSessionDraftTarget, showDraftTargetSelectors]);

    React.useEffect(() => {
        if (isDeveloperMode || !showDraftTargetSelectors || !selectedDraftProject || selectedDraftProject.kind === 'chat') {
            return;
        }
        if (newSessionDraft?.pendingWorktreeRequestId || newSessionDraft?.bootstrapPendingDirectory || newSessionDraft?.preserveDirectoryOverride) {
            return;
        }
        const projectRoot = normalizePath(selectedDraftProject.path);
        if (!projectRoot || normalizePath(selectedDraftDirectory) === projectRoot) {
            return;
        }
        setNewSessionDraftTarget({
            projectId: selectedDraftProject.id,
    ...
    }, [isDeveloperMode, newSessionDraft?.bootstrapPendingDirectory, newSessionDraft?.pendingWorktreeRequestId, newSessionDraft?.preserveDirectoryOverride, selectedDraftDirectory, selectedDraftProject, setNewSessionDraftTarget, showDraftTargetSelectors]);
    ...
    const permissionModeCapabilities = usePermissionStore((state) => state.capabilities);
    const permissionPolicyLoaded = usePermissionStore((state) => state.loaded);
    const isPermissionModeInteractive = newSessionDraftOpen
        || (Boolean(permissionScopeSessionId) && permissionPolicyLoaded);
    ...
    const handlePermissionModeChange = React.useCallback((mode: PermissionMode) => {
        ...
        void setSessionPermissionMode(permissionScopeSessionId, mode).catch(() => {
            toast.error(t('chat.permissionMode.updateFailed'));
        });
    }, [
        newSessionDraftOpen,
    ...
    useKeybind('toggle_permission_auto_accept', () => {
        if (!isPermissionModeInteractive) return false;
        handlePermissionModeChange(permissionMode === 'manual' ? 'auto' : 'manual');
    });
    ...
    return (
        ...
        >
                ...
                <div className={cn('chat-input-column mb-5 text-center', draftPresentationClassName)}>
                    <h1 className="text-balance text-[28px] font-medium leading-tight tracking-[-0.025em] text-foreground">
                        {renderDraftTitle(
                            !isDeveloperMode
                                ? draftProjectLabel
                                    ? t('chat.emptyState.workDraftTitleWithProject', { project: draftProjectLabel })
                                    : t('chat.emptyState.workDraftTitle')
                                : draftProjectLabel
            ...
            <div className={cn('chat-input-column relative overflow-visible', isComposerExpanded && 'flex flex-1 min-h-0 flex-col')}>
                ...
                <QueuedMessageChips
                    ...
                    isMobile={isMobile}
                />
                <AutoReviewBanner isMobile={isMobile} />
                {hasDrafts ? (
                ...
                {isDeveloperMode && linkedPr && !isVSCode ? (
                    <LinkedReferenceRow
                ...
                <WorkPlanSuggestionChip
                    visible={showWorkPlanSuggestion}
                    onApply={handleApplyPlanSuggestion}
                    className="mb-1.5 px-1"
                />
                <div
                    data-ivaldi-composer="true"
                    className={cn(
                    ...
                    style={{
                        ...
                        backgroundColor: isMobile
                            ? currentTheme?.colors?.surface?.elevated
                ...
                >
                    ...
                    {!isMobile && (showDraftTargetSelectors || draftPresentationExiting) && selectedDraftProject ? (
                        <div className={cn('px-2.5 pt-2', draftPresentationClassName)}>
                            <DraftTargetSelectors
                                projects={draftProjects}
                                selectedProject={selectedDraftProject}
                    ...
                    <div className={cn("overflow-hidden", isComposerExpanded && 'flex flex-1 min-h-0 flex-col')}>
                        <div className={cn(
                            'flex items-center gap-1.5 px-3 flex-wrap relative z-10',
                            visibleWorkComposerCommand ? 'pt-2' : 'pt-1',
                        )}>
                            {!isDeveloperMode && visibleWorkComposerCommand ? (
                                <Button
                                    type="button"
                                    variant="secondary"
                                    size="xs"
                                    aria-label={t('chat.chatInput.commandChip.removeAria', {
                                        command: t(visibleWorkComposerCommand === 'plan'
                                            ? 'chat.chatInput.commandChip.planLabel'
                                            : 'chat.chatInput.commandChip.goalLabel'),
                                    })}
                                    onClick={() => {
                                        setWorkComposerCommand(null);
                                        if (visibleWorkComposerCommand === 'goal' && restoredGoalArmed) {
                                            useSessionGoalArmStore.getState().setArmed(false);
                                        }
                                    }}
                                    className="rounded-md border border-border/50 bg-interactive-hover/70 px-2 text-foreground/80 hover:bg-interactive-active hover:text-foreground"
                                >
                                    <span>{t(visibleWorkComposerCommand === 'plan'
                                        ? 'chat.chatInput.commandChip.planLabel'
                                        : 'chat.chatInput.commandChip.goalLabel')}</span>
                                    <Icon name="close" className="size-3 opacity-60" />
                                </Button>
                            ) : null}
                                ...
                                className={cn(
                                    ...
                                    isComposerExpanded
                                        ? cn(
                                            'h-full min-h-0',
                                            isMobile
                                                ? (visibleWorkComposerCommand ? 'pt-1.5 pb-2.5' : 'py-2.5')
                                                : (visibleWorkComposerCommand ? 'pt-2 pb-4' : 'py-4'),
                                        )
                                        : isMobile
                                            ? (visibleWorkComposerCommand ? 'pt-1.5 pb-2.5' : 'py-2.5')
                                            : (visibleWorkComposerCommand ? 'pt-2 pb-2' : 'pt-4 pb-2'),
            ...
            {showDesktopDraftPresentation ? (
                <DraftPresetChips
                    ...
                    className={cn('chat-input-column mt-3', draftPresentationClassName)}
                />
            ) : null}
        ...
        />
        {isDeveloperMode ? <GitHubPrPickerDialog
            open={prPickerOpen}
            ...
            >
                <div className="flex flex-col px-3 pb-4 pt-1">
                    <Button
                        type="button"
                        variant="ghost"
                        className="h-auto w-full justify-start gap-2.5 rounded-md px-2 py-3 text-left typography-ui-label"
                        onClick={() => {
