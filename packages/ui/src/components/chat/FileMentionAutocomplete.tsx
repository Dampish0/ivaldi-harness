...
export const FileMentionAutocomplete = React.forwardRef<FileMentionHandle, FileMentionAutocompleteProps>(({
  ...
  const getFileIcon = (file: FileInfo) => {
    ...
    switch (ext) {
      ...
      case 'js':
      case 'jsx':
        return <Icon name="code" className="size-3.5 text-current opacity-70" />;
      case 'json':
        return <Icon name="code" className="size-3.5 text-current opacity-70" />;
      case 'md':
      case 'mdx':
        return <Icon name="file" className="size-3.5 text-current opacity-70" />;
      case 'png':
      case 'jpg':
      case 'jpeg':
      case 'gif':
      case 'svg':
        return <Icon name="file-image" className="size-3.5 text-current opacity-70" />;
      default:
        return <Icon name="file-pdf" className="size-3.5 text-current opacity-70" />;
    }
  };

  const selectedAgent = selectedIndex < visibleAgents.length
    ? visibleAgents[selectedIndex]
    : undefined;

  return (
      <div
        ref={containerRef}
        className="absolute bottom-full left-0 z-[100] mb-2 flex max-h-64 w-full min-w-0 max-w-[640px] flex-col overflow-hidden rounded-lg border border-[var(--interactive-border)]/70 bg-[var(--surface-elevated)] shadow-md"
        ...
        {loading ? (
          ...
          <div>
            {visibleAgents.map((agent, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'flex cursor-pointer items-start gap-2 rounded-md px-2.5 py-2 typography-ui-label transition-colors',
                    isSelected
            ...
            {visibleAgents.length > 0 && (visibleRecentFiles.length > 0 || visibleResults.length > 0) && (
              <div className="my-1 border-t border-[var(--interactive-border)]/60" />
            )}
            ...
            {visibleResults.map((file, index) => {
              ...
              const item = (
                ...
                >
                  {file.kind === 'directory'
                    ? <Icon name="folder-3-fill" className="size-3.5 text-current opacity-70" />
                    : getFileIcon(file)}
                  <span
                    ref={(el) => { labelRefs.current[rowIndex] = el; }}
                    className="relative flex-1 min-w-0 overflow-hidden file-mention-marquee-container"
