...
export const useTurnRecords = (
    ...
    const previousHideGoalProtocolMessagesRef = React.useRef(options.hideGoalProtocolMessages === true);
    const previousHideCompactionProtocolMessagesRef = React.useRef(options.hideCompactionProtocolMessages === true);
    const hideGoalProtocolMessages = options.hideGoalProtocolMessages === true;
    const hideCompactionProtocolMessages = options.hideCompactionProtocolMessages === true;
    ...
    if (
        ...
        || previousHideGoalProtocolMessagesRef.current !== hideGoalProtocolMessages
        || previousHideCompactionProtocolMessagesRef.current !== hideCompactionProtocolMessages
    ) {
        previousSessionKeyRef.current = options.sessionKey;
        previousShowTextJustificationActivityRef.current = options.showTextJustificationActivity;
        previousShowTurnChangedFilesRef.current = options.showTurnChangedFiles;
    ...
    React.useEffect(() => {
        ...
        staticTurnsRef.current = [];
        streamingTurnRef.current = undefined;
    }, [hideCompactionProtocolMessages, hideGoalProtocolMessages, options.sessionKey, options.showTextJustificationActivity, options.showTurnChangedFiles, options.planModeEnabled]);

    const projection = React.useMemo(() => {
        const sessionKey = options.sessionKey ?? '';
        const mergeKey = `${options.planModeEnabled ? 'merge:plan' : 'merge'}${hideGoalProtocolMessages ? ':hide-goal' : ''}${hideCompactionProtocolMessages ? ':hide-compact' : ''}`;
        ...
        return streamPerfMeasure('ui.turns.projection_ms', () => {
            const nextProjection = projectTurnRecords(messages, {
                ...
                mergeHiddenUserTurns: {
                    ...
                    hideCompactionProtocolMessages,
                },
            });
    ...
    }, [hideCompactionProtocolMessages, hideGoalProtocolMessages, messages, options.showTextJustificationActivity, options.showTurnChangedFiles, options.sessionKey, options.planModeEnabled]);
