...
export const useUsageProviderGroups = (): UsageProviderGroup[] => {
  ...
  return React.useMemo<UsageProviderGroup[]>(() => {
    ...
    return QUOTA_PROVIDERS
      ...
      .map((providerMeta) => {
        ...
        const rawStatus = !result.ok && result.error
          ? result.error
          : rows.length === 0
            ? t('header.services.noRateLimitsReported')
            : null;
        const status = isWorkMode && rawStatus && isLikelyProviderAuthFailure(rawStatus)
  ...
  }, [dropdownProviderIds, isWorkMode, quotaResults, selectedQuotaModels, t]);
};
