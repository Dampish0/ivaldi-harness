...
const TrafficLightButton: React.FC<TrafficLightButtonProps> = ({ action, isMaximized, onActivate }) => {
  ...
  return (
    <button
      ...
      className="app-region-no-drag flex h-8 w-[24px] items-center justify-center rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
    >
      <span
...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({
  ...
  const buttonClassName = cn(
    'app-region-no-drag inline-flex items-center justify-center text-muted-foreground transition-colors hover:bg-interactive-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]',
  ...
  const renderControl = (action: DesktopWindowControlAction) => {
    ...
    return (
      <button
        ...
        className={cn(
          buttonClassName,
          // Windows caption chrome uses its own fixed hover treatment. Do not
          // inherit the app theme's muted error-card colors here.
          'hover:bg-[#c42b1c] hover:text-white active:bg-[#a92318] active:text-white',
        )}
