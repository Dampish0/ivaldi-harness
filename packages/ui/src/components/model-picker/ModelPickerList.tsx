...
interface ModelPickerListProps {
  ...
  /** Hide provider headings/logos when the caller has already fixed provider choice elsewhere. */
  showProviderIdentity?: boolean;
  /** Optional caller-owned recovery UI when filtering leaves no model rows. */
...
export const ModelPickerList: React.FC<ModelPickerListProps> = ({
  ...
  const visibleSectionKeys = React.useMemo(() => [
    ...
    ...(showProviderIdentity ? filteredProviders.map((provider) => `provider:${provider.id}`) : []),
  ], [filteredFavorites.length, filteredProviders, filteredRecents.length, showProviderIdentity]);
  ...
  const providerSortingEnabled = showProviderIdentity && Boolean(onReorderProvider) && searchQuery.trim().length === 0 && !allowedProviderSet && filteredProviders.length > 1;
        ...
        {(isHighlighted) => {
          const rowElement = (
            ...
            >
              ...
              {isSelected ? <Icon name="check" className="h-4 w-4 text-foreground flex-shrink-0" /> : null}
              {onToggleFavorite ? (
                <button type="button" disabled={disabled} onClick={(event) => { event.preventDefault(); event.stopPropagation(); onToggleFavorite(entry); }} className={cn('model-favorite-button flex h-4 w-4 items-center justify-center hover:text-foreground flex-shrink-0 disabled:pointer-events-none', favorite ? 'text-foreground' : 'text-muted-foreground')} aria-label={favorite ? labels.unfavorite : labels.favorite} title={favorite ? labels.unfavorite : labels.favorite}>
  ...
  const renderSectionIdentity = (sectionKey: string): React.ReactNode => {
    if (sectionKey === 'favorites') {
      return <><Icon name="star-fill" className="h-4 w-4 flex-shrink-0 text-muted-foreground" /><span className="min-w-0 truncate">{labels.favorites}</span></>;
    }
        ...
        >
          ...
          {includeNotSelected ? (
            <>
              ...
              >
                ...
                {!selectedModel ? <Icon name="check" className="h-4 w-4 text-foreground ml-auto" /> : null}
              </button>
          ...
          {filteredFavorites.length > 0 ? (
            <div className="relative">
              ...
              {renderSectionHeader('favorites', <Icon name="star-fill" className="h-4 w-4 text-muted-foreground" />, labels.favorites)}
                    ...
                    {filteredFavorites.map((entry) => {
                      ...
                      return (
                        <SortableFavoriteModelRow key={`fav-sortable-${entry.providerID}-${entry.modelID}`} id={`${entry.providerID}:${entry.modelID}`} disabled={disabled}>
                          {(dragHandleProps) => renderRow(entry, 'fav', showProviderIdentity, rowIndex, dragHandleProps)}
                        </SortableFavoriteModelRow>
                      );
                    })}
                  </SortableContext>
                </DndContext>
              ) : filteredFavorites.map((entry) => renderRow(entry, 'fav', showProviderIdentity, currentFlatIndex++)))}
            </div>
          ) : null}
          ...
          {filteredRecents.length > 0 ? (
            <>
              ...
              <div className="relative">
                ...
                {!isSectionCollapsed('recent') ? filteredRecents.map((entry) => renderRow(entry, 'recent', showProviderIdentity, currentFlatIndex++)) : null}
              </div>
            </>
          ) : null}
          ...
          {!showProviderIdentity ? (
            filteredProviders.flatMap((provider) => provider.models.map((model) => {
              const modelID = model.id;
              if (!modelID) return null;
              return renderRow({ model, providerID: provider.id, modelID }, 'model', false, currentFlatIndex++);
            }))
