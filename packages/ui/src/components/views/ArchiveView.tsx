...
import { formatSessionDateLabel, normalizePath, resolveWorkChatTitle } from '@/components/session/sidebar/utils';
...
export function ArchiveView(): React.ReactNode {
  ...
  const getDisplayTitle = React.useCallback((session: Session) => (
    isWorkMode
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              aria-label={isWorkMode
                ? t('chat.work.archive.deleteProjectAria', { label })
                : t('sessions.archivePage.deleteProjectAria', { label })}
            >
              <Icon name="delete-bin" className="h-3.5 w-3.5" />
            </button>
          </TooltipTrigger>
          <TooltipContent side="bottom" sideOffset={4}>
            {isWorkMode ? t('chat.work.archive.deleteProject') : t('sessions.archivePage.deleteProject')}
          </TooltipContent>
        </Tooltip>
      ) : null}
    </div>
          ...
          <div className="flex items-center gap-3 px-6 pt-3">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder={isWorkMode
                  ? t('chat.work.archive.searchPlaceholder')
                  : t('sessions.archivePage.searchPlaceholder')}
            ...
            <span className="flex-shrink-0 typography-micro text-muted-foreground">
              {filteredSessions.length === 1
                ? t(isWorkMode ? 'chat.work.archive.countSingle' : 'sessions.archivePage.countSingle', { count: filteredSessions.length })
                : t(isWorkMode ? 'chat.work.archive.countPlural' : 'sessions.archivePage.countPlural', { count: filteredSessions.length })}
            </span>
          </div>
              ...
              {visibleSessions.length === 0 ? (
                <div className="py-10 text-center text-muted-foreground">
                  <p className="typography-ui-label font-semibold">
                    {normalizedQuery
                      ? t(isWorkMode ? 'chat.work.archive.emptyNoMatches' : 'sessions.archivePage.empty.noMatches')
                      : t(isWorkMode ? 'chat.work.archive.emptyNoArchived' : 'sessions.archivePage.empty.noArchived')}
                  </p>
                </div>
