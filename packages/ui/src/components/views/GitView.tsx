...
export const GitView: React.FC<GitViewProps> = ({ isActive }) => {
  ...
  const gitRefreshError = useGitStore((state) => {
    if (!currentDirectory) return null;
    const directoryState = state.directories.get(currentDirectory);
    return directoryState?.statusError ?? directoryState?.branchesError ?? null;
  });
  ...
  const refreshStatusAndBranches = React.useCallback(
    async (showErrors = true) => {
      ...
      const directoryState = useGitStore.getState().directories.get(currentDirectory);
      if (showErrors && (directoryState?.statusError || directoryState?.branchesError)) {
  ...
  return (
    <div className={cn('flex h-full flex-col overflow-hidden')}>
      {gitRefreshError ? (
        <div className="mx-4 mt-3 flex items-center justify-between gap-3 rounded-md border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-3 py-2 text-[var(--status-error-foreground)]">
          <span className="typography-meta">{t('gitView.toast.refreshRepositoryFailed')}</span>
          <Button
            size="xs"
            variant="outline"
            onClick={() => void refreshStatusAndBranches()}
            disabled={isLoading}
          >
