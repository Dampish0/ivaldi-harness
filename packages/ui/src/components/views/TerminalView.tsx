...
type TerminalViewProps = {
    visible?: boolean;
    contextPanelChrome?: boolean;
};

const FALLBACK_TERMINAL_SIZE = { cols: 80, rows: 24 } as const;

export const TerminalView: React.FC<TerminalViewProps> = ({ visible, contextPanelChrome = false }) => {
    const { t } = useI18n();
    ...
    const showTerminalTabStrip = enableTabs
        && Boolean(directoryTerminalState)
        && (!contextPanelChrome || terminalTabItems.length > 1);
        ...
        <div className="flex h-full flex-col overflow-hidden bg-[var(--surface-background)]">
            <div className={cn(
                ...
                {enableTabs && directoryTerminalState ? (
                    <div className={cn('flex items-center gap-2', contextPanelChrome ? 'justify-end' : 'pl-1 pr-1')}>
                        {showTerminalTabStrip ? (
