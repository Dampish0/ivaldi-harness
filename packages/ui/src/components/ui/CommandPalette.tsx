...
export const CommandPalette: React.FC = () => {
  ...
  React.useEffect(() => {
    if (productMode === 'work' || !isCommandPaletteOpen || !gitApi) return;
  ...
  }, [isCommandPaletteOpen, activeSessions, gitApi, ensureGitStatus, productMode]);
  ...
  const settingsEntries = React.useMemo<CommandEntry[]>(() => {
    return SETTINGS_PAGE_METADATA
      ...
      .filter((p) => isSettingsPageVisibleInProductMode(productMode, p.slug))
      .filter((p) => isSettingsPageAvailableInProductMode(
        productMode,
        p.slug,
        p.isAvailable ? p.isAvailable(settingsRuntimeCtx) : true,
        settingsRuntimeCtx,
      ))
      .map((page) => {
        const iconName = getSettingsNavIcon(page.slug) ?? 'settings-3';
        const keywords = getSettingsPageKeywordsInProductMode(
          productMode,
          page.slug,
          page.keywords ?? [],
        ).join(' ');
  ...
  }, [productMode, settingsRuntimeCtx, run, setSettingsPage, setSettingsDialogOpen, t]);
  ...
  const allBranches = useGitAllBranches(productMode === 'developer');
                ...
                return (
                  <CommandGroup key="sessions">
                    {visibleSessions.map((session) => {
                      ...
                      const dir = resolveGlobalSessionDirectory(session);
                      const branch = productMode === 'developer' ? branchForSession(session.id, dir) : null;
                      return (
                        <CommandItem
                          key={session.id}
