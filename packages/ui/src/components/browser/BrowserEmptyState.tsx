...
export const BrowserEmptyState: React.FC<{
  ...
  return (
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 overflow-hidden bg-background p-6 text-center">
      <div className="flex max-w-sm shrink-0 flex-col gap-1">
        <span className="typography-meta text-muted-foreground">{t('contextPanel.browser.emptyHint')}</span>
      </div>
          ...
          <div className="flex min-h-0 flex-col overflow-y-auto pr-0.5">
            {candidates.map((candidate) => (
              <Button
                ...
                variant="ghost"
                size="sm"
                className="w-full shrink-0 justify-start gap-2 px-2 text-left"
                onClick={() => {
