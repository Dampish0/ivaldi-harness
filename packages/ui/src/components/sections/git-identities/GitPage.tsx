...
export const GitPage: React.FC = () => {
  ...
  const hasIdentityLoadError = Object.values(loadErrors).some(Boolean);

  const retryIdentityLoads = React.useCallback(() => {
    void Promise.all([
      loadProfiles(),
      loadGlobalIdentity(),
      loadDiscoveredCredentials(),
  ...
  }, [loadDefaultGitIdentityId, loadDiscoveredCredentials, loadGlobalIdentity, loadProfiles]);
      ...
      >
        ...
        >
          {hasIdentityLoadError && (
            <div className="mb-4 flex items-center justify-between gap-3 rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-4 py-3">
              <p className="typography-meta text-[var(--status-error)]">
                {t('settings.gitIdentities.page.state.loadFailed')}
              </p>
              <Button size="sm" variant="outline" onClick={retryIdentityLoads}>
          ...
          <div className={cn(
            ...
            {/* Empty state */}
            {!hasIdentityLoadError && !globalIdentity && profiles.length === 0 && unimportedCredentials.length === 0 && (
    ...
    <ContextMenu open={contextMenuOpen} onOpenChange={setContextMenuOpen}>
      <ContextMenuTrigger
        render={
          <div
            className={cn(
              'group flex items-center justify-between gap-3 py-2.5 transition-colors cursor-pointer',
              isMobile ? 'px-0 active:bg-interactive-hover/50' : 'px-4 hover:bg-[var(--interactive-hover)]/30',
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {isDefault && (
              <span className="typography-micro bg-interactive-selection text-interactive-selection-foreground px-1 rounded flex-shrink-0 leading-none pb-px border border-border/50">
  ...
  return (
    <div
      className={cn(
        ...
        isMobile ? 'px-0' : 'px-4 hover:bg-[var(--interactive-hover)]/30',
        hasBorder && 'border-b border-[var(--surface-subtle)]'
      )}
    >
      <div className="min-w-0">
