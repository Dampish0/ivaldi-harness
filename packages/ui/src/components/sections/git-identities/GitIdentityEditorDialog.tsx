    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className={cn(
          ...
          <div className={cn('space-y-5 py-2', isMobile && 'px-4 py-4')}>
            {/* Profile Display */}
            {!isGlobalProfile && (
          ...
          <DialogFooter className={cn('gap-2', isMobile && 'flex-row flex-wrap justify-end border-t border-border/40 px-4 py-3')}>
            ...
            <Button variant="ghost" size={isMobile ? 'default' : 'sm'} onClick={() => onOpenChange(false)} className="text-foreground hover:bg-interactive-hover hover:text-foreground">
              {isGlobalProfile ? t('settings.gitIdentities.editor.actions.close') : t('settings.common.actions.cancel')}
            </Button>
            {!isGlobalProfile && (
              <Button size={isMobile ? 'default' : 'sm'} onClick={handleSave} disabled={isSaving}>
