...
export const InstallSkillDialog: React.FC<InstallSkillDialogProps> = ({ open, onOpenChange, item }) => {
  ...
  const locationLabelText = React.useCallback((value: SkillLocationValue) => {
    if (isWorkMode) {
      const { scope: locationScope } = locationPartsFrom(value);
      return t(locationScope === 'project' ? 'settings.common.scope.project' : 'settings.common.scope.global');
    }
    switch (value) {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className={cn('max-w-md', isMobile && 'max-w-none gap-0 rounded-md p-0')}>
          ...
          <div className={cn('space-y-3', isMobile ? 'px-4 py-4' : 'mt-2')}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                <SelectTrigger className={cn('w-fit gap-1.5', isMobile && 'rounded-md')}>
            ...
            {scope === 'project' && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                {projects.length === 0 ? (
                  ...
                  >
                    <SelectTrigger className={cn('w-fit', isMobile && 'rounded-md')}>
