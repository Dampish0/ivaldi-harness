...
import { selectCommandsForDirectory, selectCommandsLoadErrorForDirectory, useCommandsStore, isCommandBuiltIn, type Command } from '@/stores/useCommandsStore';
...
export const CommandsSidebar: React.FC<CommandsSidebarProps> = ({ onItemSelect }) => {
  ...
  const loadError = useCommandsStore((state) => selectCommandsLoadErrorForDirectory(state, settingsDirectory));
      ...
      <ScrollableOverlay outerClassName="flex-1 min-h-0" className="space-y-1 px-3 py-2">
        {loadError ? (
          <div className="flex items-center justify-between gap-2 px-2 py-3 text-[var(--status-error)]">
            ...
            <Button size="xs" variant="outline" onClick={() => void loadCommands(settingsDirectory, { force: true })}>
              {t('settings.common.actions.retry')}
            </Button>
          </div>
        ) : null}
        {commandOnlyItems.length === 0 && !loadError ? (
          <div className="px-2 py-4">
            <p className="typography-ui-label font-medium text-foreground">{t('settings.commands.sidebar.empty.title')}</p>
            <p className="typography-meta mt-1 text-muted-foreground">{t('settings.commands.sidebar.empty.description')}</p>
          </div>
