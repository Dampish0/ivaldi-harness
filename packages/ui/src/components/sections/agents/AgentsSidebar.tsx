...
import { selectAgentsForDirectory, selectAgentsLoadErrorForDirectory, useAgentsStore, isAgentBuiltIn, isAgentHidden, type AgentScope, type AgentDraft } from '@/stores/useAgentsStore';
...
export const AgentsSidebar: React.FC<AgentsSidebarProps> = ({ onItemSelect }) => {
  ...
  const loadError = useAgentsStore((state) => selectAgentsLoadErrorForDirectory(state, settingsDirectory));
      ...
      <div className="border-b px-3 pt-4 pb-3">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Button size="sm"
            ...
            title={t('settings.agents.page.title.new')}
          >
      ...
      <ScrollableOverlay outerClassName="flex-1 min-h-0" className="space-y-1 px-3 py-2 overflow-x-hidden">
        {loadError ? (
          <div className="flex items-center justify-between gap-2 px-2 py-3 text-[var(--status-error)]">
            ...
            <Button size="xs" variant="outline" onClick={() => void loadAgents(settingsDirectory, { force: true })}>
        ...
        {visibleAgents.length === 0 && !loadError ? (
          <div className="px-2 py-4">
            <p className="typography-ui-label font-medium text-foreground">{t('settings.agents.sidebar.empty.title')}</p>
            <p className="typography-meta mt-1 text-muted-foreground">{t('settings.agents.sidebar.empty.description')}</p>
          </div>
          ...
          <>
            ...
            {customAgents.length > 0 && (
              <>
                <div className="px-2 pb-1.5 pt-3 typography-meta font-medium text-muted-foreground">
