...
export const BehaviorPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setLoadError(hadLoadFailure ? t('settings.common.state.loadFailed') : null);
      } catch (error) {
        if ((error as Error).name !== 'AbortError') {
          console.warn('Failed to load behavior settings:', error);
          setLoadError(t('settings.common.state.loadFailed'));
        }
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    ...
    >
      {loadError ? (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-4 py-3">
          <span className="typography-meta text-[var(--status-error)]">{loadError}</span>
          <Button size="sm" variant="outline" onClick={() => setLoadRevision((value) => value + 1)} disabled={isLoading}>
