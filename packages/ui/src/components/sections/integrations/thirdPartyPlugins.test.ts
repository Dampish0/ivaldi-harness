...
const claudePackage = '@ivaldi/opencode-claude';
...
describe('third-party plugin catalog helpers', () => {
  test('derives catalog row status with explicit transient-state priority', () => {
  ...
  test('matches only a package or its versioned spec', () => {
    ...
    expect(specMatchesPackage('@ivaldi/opencode-claude-extra@0.6.0', claudePackage)).toBe(false);
  });
