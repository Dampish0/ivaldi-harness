      ...
      <ScrollableOverlay outerClassName="flex-1 min-h-0" className="space-y-1 px-3 py-2 overflow-x-hidden">
        {providers.length === 0 ? (
          <div className="px-2 py-4">
            <p className="typography-ui-label font-medium text-foreground">{t('settings.providers.sidebar.empty.title')}</p>
            <p className="typography-meta mt-1 text-muted-foreground">{t('settings.providers.sidebar.empty.description')}</p>
          </div>
        ) : (
          <>
            ...
            {projectProviders.length > 0 && (
              <>
                <div className={cn('px-2 pb-1.5 typography-meta font-medium text-muted-foreground', userProviders.length > 0 ? 'pt-3' : 'pt-2')}>
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onSelect}
        className="flex min-w-0 flex-1 items-center gap-2 rounded-sm text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--interactive-focus-ring)]"
        tabIndex={0}
      >
