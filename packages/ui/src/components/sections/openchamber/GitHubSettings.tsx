...
export const GitHubSettings: React.FC = () => {
  ...
  return (
    <>
      ...
      >
      {loadError && (
        <div className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-4 py-3">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => void refreshStatus(runtimeGitHub, { force: true })}
            disabled={isBusy}
          >
      ...
      <div className={cn(
        ...
        !isMobile && 'rounded-lg bg-[var(--surface-elevated)]/70',
      )}>
        {connected ? (
          <div className={cn(isMobile ? 'flex flex-col gap-3 py-2' : 'flex items-center justify-between gap-4 px-4 py-3')}>
            ...
            {ghCli?.active ? (
              <Button size="sm" variant="outline" onClick={() => toggleGhCli(true)} disabled={isBusy} className={cn(isMobile ? 'self-start' : undefined)}>
                {t('settings.github.page.ghCli.actions.disable')}
              </Button>
            ) : (
              <Button size="sm" variant="outline" onClick={disconnect} disabled={isBusy} className={cn('text-[var(--status-error)] hover:text-[var(--status-error)]', isMobile ? 'self-start' : undefined)}>
                {t('settings.github.page.actions.disconnect')}
              </Button>
            )}
          </div>
        ) : loadError ? null : (
          <div className={cn('flex items-center justify-between gap-4 py-4', !isMobile && 'px-4')}>
        ...
        {otherAccounts.length > 0 && (
          <div className={cn('mt-2 border-t border-[var(--surface-subtle)] pt-2 pb-1', !isMobile && 'px-2')}>
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isMobile ? 'px-0' : 'rounded-md border border-[var(--surface-subtle)] bg-[var(--surface-muted)] px-3',
                    )}
                  >
      ...
      {connected && (
        <div className={cn('mt-2 pb-2', !isMobile && 'px-2')}>
          <Button size="sm"
            variant="outline"
      ...
      {ghCli?.available && !ghCli?.active && (!ghCli.user || ghCli.disabled) && (
        <SettingsSection title={t('settings.github.page.ghCli.title')}>
          <div className={cn('overflow-hidden', !isMobile && 'rounded-lg bg-[var(--surface-elevated)]/70')}>
            <div className={cn(isMobile ? 'flex flex-col gap-3 py-2' : 'flex items-center justify-between gap-4 px-4 py-3')}>
              <div className={cn("flex min-w-0 items-center gap-4", isMobile ? "w-full" : undefined)}>
              ...
              <Button
                ...
                disabled={isBusy}
                className={cn(isMobile ? 'self-start' : undefined)}
              >
                {ghCli.disabled
