...
export const PluginsSidebar: React.FC<PluginsSidebarProps> = ({
  ...
  const { entries, files, selectedId, loadError, setSelected, deleteEntry, deleteFile, loadPlugins } =
    usePluginsStore(
      useShallow((s) => ({
        entries: s.entries,
        files: s.files,
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  {!isDeveloperMode && <span>{t('settings.plugins.sidebar.actions.addTitle')}</span>}
                </Button>
              </div>
            </div>
          </div>
      ...
      >
        {loadError ? (
          <div className="flex items-center justify-between gap-2 px-2 py-3 text-[var(--status-error)]">
            ...
            <Button size="xs" variant="outline" onClick={() => void loadPlugins({ force: true })}>
