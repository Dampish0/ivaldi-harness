...
export const AddPluginDialog: React.FC<AddPluginDialogProps> = ({
  ...
  const tabs = React.useMemo<Array<SortableTabsStripItem & { id: TabKey }>>(() => [
    {
      id: 'npm',
      label: t(isWorkMode ? 'settings.plugins.sidebar.kind.npm' : 'settings.plugins.dialog.add.tab.npm'),
    },
    {
      id: 'path',
      label: t(isWorkMode ? 'settings.plugins.sidebar.kind.path' : 'settings.plugins.dialog.add.tab.path'),
    },
    {
      id: 'file',
      label: t(isWorkMode ? 'settings.plugins.sidebar.kind.file' : 'settings.plugins.dialog.add.tab.file'),
    },
  ], [isWorkMode, t]);
  ...
  return (
    ...
    >
      <DialogContent
        className={cn(
          'max-w-xl',
          isMobile && 'max-h-[calc(100dvh-1rem)] max-w-none gap-0 rounded-md p-0',
        )}
      >
        <DialogHeader className={cn(isMobile && 'border-b border-border/40 px-4 pb-3 pt-4 pr-10 text-left')}>
        ...
        {isMobile ? (
          <div className="mx-4 mt-3 flex items-center gap-1" role="tablist" aria-label={t('sortableTabsStrip.aria.tabs')}>
            {tabs.map((item) => {
              ...
              return (
                <Button
                  ...
                  className={cn(
                    'min-w-0 px-2 text-muted-foreground',
        ...
        <div className={cn('flex flex-col gap-4', isMobile && 'px-4 py-4')}>
          {(tab === 'npm' || tab === 'path') && (
            <>
              <div data-settings-item="plugins.spec" className="flex flex-col gap-1.5">
                <label htmlFor="plugin-spec" className="typography-ui-label text-foreground">
                  {isWorkMode
                    ? t(tab === 'npm' ? 'settings.plugins.sidebar.kind.npm' : 'settings.plugins.sidebar.kind.path')
                ...
                <Input
                  ...
                  className={cn(isMobile && 'rounded-md')}
                  aria-invalid={specEmpty ? false : undefined}
                  disabled={submitting}
                />
                {specEmpty && !isWorkMode && (
              ...
              <div data-settings-item="plugins.options" className="flex flex-col gap-1.5">
                ...
                <Textarea
                  ...
                  rows={5}
                  className="font-mono"
                  outerClassName={cn(isMobile && 'rounded-md')}
        ...
        <DialogFooter className={cn(isMobile && 'flex-row justify-end border-t border-border/40 px-4 py-3')}>
          <Button
