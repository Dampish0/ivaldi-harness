...
import { opencodeClient } from '@ivaldi/ui/lib/opencode/client';
import { sanitizeHeadersForBrowser } from '@ivaldi/ui/lib/runtime-fetch';
import {
  buildVSCodeThemeFromPalette,
  readVSCodeThemePalette,
  type VSCodeThemeKind,
  type VSCodeThemePayload,
} from '@ivaldi/ui/lib/theme/vscode/adapter';
import { getBootstrapMessages, readStoredLocaleForBootstrap } from '@ivaldi/ui/lib/i18n';
import type { VSCodeActiveEditorFile } from '@/sync/input-store';
import { usePermissionStore } from '@ivaldi/ui/stores/permissionStore';
import { processVSCodePermissionAutoAccept } from '@ivaldi/ui/sync/vscode-permission-auto-accept';
import type { PermissionRequest } from '@opencode-ai/sdk/v2/client';
import { focusChatInput } from '@ivaldi/ui/components/chat/composer/editor/dom';
...
const handleLocalApiRequest = async (input: RequestInfo | URL, url: URL, init: RequestInit | undefined, method: string) => {
  ...
  if (normalizedPathname === '/api/permission-policy' && method === 'GET') {
    const snapshot = await sendBridgeMessage('api:permission-policy:get');
    return new Response(JSON.stringify(snapshot), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  ...
  const permissionModeMatch = normalizedPathname.match(/^\/api\/permission-policy\/sessions\/([^/]+)$/);
  if (permissionModeMatch && method === 'PUT') {
    const bodyText = await extractBodyText(url, init, method);
    const body = bodyText ? JSON.parse(bodyText) as { mode?: unknown } : {};
    const snapshot = await sendBridgeMessage('api:permission-policy:set', {
      sessionId: decodeURIComponent(permissionModeMatch[1]),
      mode: body.mode,
    });
    return new Response(JSON.stringify(snapshot), {
      status: 200,
...
onCommand('reloadOpenCode', () => {
  void import('@ivaldi/ui/stores/useAgentsStore').then(({ reloadOpenCodeConfiguration }) => {
...
onCommand('settingsSynced', () => {
  import('@ivaldi/ui/lib/persistence').then(({ syncDesktopSettings }) => {
...
onCommand('permissionAutoAcceptSynced', (payload) => {
  ...
  const snapshot = payload as { sessions?: unknown; modes?: unknown; revision?: unknown };
  const sessions = snapshot.sessions;
  if (!sessions || typeof sessions !== 'object') return;
  usePermissionStore.getState().applySnapshot({
    sessions: sessions as Record<string, boolean>,
    modes: snapshot.modes && typeof snapshot.modes === 'object'
      ? snapshot.modes as Record<string, 'manual' | 'auto' | 'full-access'>
      : undefined,
...
import('@ivaldi/ui/apps/renderVSCodeApp')
