...
const resolveGoogleOAuthClient = (sourceId: GoogleAuthSource['sourceId']) => {
  ...
  const clientId = asNonEmptyString(process.env[`IVALDI_${prefix}_GOOGLE_CLIENT_ID`]);
  const clientSecret = asNonEmptyString(process.env[`IVALDI_${prefix}_GOOGLE_CLIENT_SECRET`]);
