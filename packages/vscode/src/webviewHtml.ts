...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Keep startup branding aligned with the shared Ivaldi glyph.
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    /* A quiet pulse signals loading without introducing extra copy. */
...
<body>
  <!-- Initial loading screen with the Ivaldi glyph -->
  <div id="initial-loading">
    <svg class="logo" width="70" height="70" viewBox="108 105 302 302" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ivaldi">
      <defs>
        <mask id="ivaldi-vscode-gap" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" x="0" y="0" width="512" height="512">
          <rect width="512" height="512" fill="#fff"/>
          <polygon points="252.50,256.00 422.50,133.00 459.50,220.00 454.50,314.00" fill="#000"/>
        </mask>
      </defs>
      <g class="logo-inner logo-fill-solid">
        <circle cx="252.5" cy="256" r="118" fill="none" stroke="var(--vscode-foreground)" stroke-width="48" mask="url(#ivaldi-vscode-gap)"/>
    ...
    if (!devServerUrl) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[Ivaldi] VS Code webview dev bundle unavailable, retrying...', error);
