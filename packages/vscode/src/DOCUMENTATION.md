...
The extension identity is `dampish0.ivaldi`; contributed commands, views, and settings use `ivaldi.*`. Activation copies unset global and workspace API URL and binary settings from the legacy namespace. It does not overwrite Ivaldi settings or migrate extension-specific UI state.

The shared server `ivaldi-bootstrap.js` runs before settings modules initialize. Fresh installs use `~/.config/ivaldi`; existing legacy directories are reused in place unless the new directory or an explicit data-directory override exists. `/api/fs/home` exposes this resolved directory to the webview.
...
- `bridge-permission-auto-accept-runtime.ts`
  - Owns the persisted VS Code permission-mode policy and both the current `/api/permission-policy` bridge contract and legacy boolean compatibility contract.
  - Persists `manual`, `auto`, and `full-access`, mirrors legacy booleans, serializes read-modify-write updates, preserves a monotonic revision, and broadcasts the exact committed snapshot to every active OpenChamber webview.
  - Permission replies remain foreground UI-owned because VS Code does not run the OpenChamber server runtime. Auto uses the shared UI risk classifier and asks on ambiguous or risky actions.
