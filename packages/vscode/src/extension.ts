import '../../web/server/lib/ivaldi-bootstrap.js';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('Ivaldi');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      await vscode.commands.executeCommand('ivaldi.chatView.focus');
      await vscode.commands.executeCommand(moveCommandId);
      return 'moved';
    } catch (error) {
      outputChannel?.appendLine(
        `[Ivaldi] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  // Preserve configured connection values when adopting the Ivaldi namespace.
  const previousConfig = vscode.workspace.getConfiguration('openchamber');
  const currentConfig = vscode.workspace.getConfiguration('ivaldi');
  for (const key of ['apiUrl', 'opencodeBinary']) {
    const current = currentConfig.inspect<string>(key);
    const previous = previousConfig.inspect<string>(key);
    if (current?.globalValue === undefined && previous?.globalValue !== undefined) {
      await currentConfig.update(key, previous.globalValue, vscode.ConfigurationTarget.Global);
    }
    if (current?.workspaceValue === undefined && previous?.workspaceValue !== undefined) {
      await currentConfig.update(key, previous.workspaceValue, vscode.ConfigurationTarget.Workspace);
    }
  }

  // Migration: clear legacy auto-set API URLs (ports 47680-47689 were auto-assigned by older extension versions)
  const config = vscode.workspace.getConfiguration('ivaldi');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.ivaldi');
      } catch (e) {
        outputChannel?.appendLine(`[Ivaldi] workbench.view.extension.ivaldi failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('ivaldi.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[Ivaldi] ivaldi.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('Ivaldi: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[Ivaldi] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('Ivaldi: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    const opened = await vscode.commands.executeCommand<boolean>('ivaldi.openSidebar');
    if (!opened) {
      return false;
    }
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[Ivaldi] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('Ivaldi: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.focusChat', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.internal.permissionAutoAcceptSynced', (snapshot: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('Ivaldi: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('Ivaldi: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('Ivaldi: Failed to restart API - {0}', String(e)));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Ivaldi [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Ivaldi [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('Ivaldi: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('Ivaldi: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Ivaldi [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Ivaldi [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Ivaldi [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('Ivaldi: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`Ivaldi: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.showSettings', () => {
      chatViewProvider?.showSettings();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('ivaldi.showOpenCodeStatus', async () => {
      const config = vscode.workspace.getConfiguration('ivaldi');
      ...
      const lines = [
        ...
        `Ivaldi version: ${extensionVersion || '(unknown)'}`,
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('ivaldi').get<string>('opencodeBinary') || '').trim() || '(none)'}`,
